...
def _get_tunnel_base() -> str:
    """Extract tunnel service base domain from BASE_URL.
    e.g. https://godhostingbot.loca.lt  →  loca.lt
    """
    url = BASE_URL.strip()
    if "://" in url:
        url = url.split("://", 1)[1]
    url = url.split("/")[0].split(":")[0]   # strip path + port
    parts = url.split(".")
...
async def domain_tunnel_monitor():
    """Background task: restart dead domain tunnels every 60 s."""
    while True:
        await asyncio.sleep(60)
        try:
...
async def cb_domain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not assigned_domains:
        text = (
            f"🌐 *Custom Domain — {escape_md(name)}*\n\n"
            f"No domains assigned to you yet\\.\n\n"
            f"Contact the bot owner to get a custom domain\\!\n"
            f"Your User ID: `{uid}`"
        )
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 Back", callback_data=f"proj:{name}")]
