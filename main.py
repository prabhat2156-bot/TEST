...
async def cb_support_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """User clicks Support — generate ticket ID and ask for description."""
    query = update.callback_query
    await query.answer()
    uid = query.from_user.id
    ...
    await safe_edit(
        ...
        ("🎧 *Support Center*\n\n"
         ...
         "_Tip: Include screenshots if needed — send them after this message._"),
...
async def support_describe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive user's problem description and notify owner."""
    uid         = update.effective_user.id
    ...
    if OWNER_ID and notification_bot:
        try:
            ...
            msg = ("🎧 *New Support Ticket*\n\n"
                   "🎫 Ticket ID: `" + ticket_id + "`\n"
                   "👤 User: " + escape_md(us) + " \\(`" + str(uid) + "`\\)\n"
                   "📅 " + now.strftime("%Y-%m-%d %H:%M UTC") + "\n\n"
                   "📝 *Problem:*\n" + escape_md(description[:500]))
            await notification_bot.send_message(OWNER_ID, msg, parse_mode=ParseMode.MARKDOWN, reply_markup=kb)
        except Exception as e:
    ...
    await update.message.reply_text(
        ...
        "You will receive a notification once your issue is resolved. 🙏",
        parse_mode=ParseMode.MARKDOWN,
