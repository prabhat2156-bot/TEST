...
async def safe_edit(query, text: str, reply_markup=None, parse_mode=_SAFE_EDIT_UNSET):
    ...
    if is_media:
        # For photo/media messages, edit caption (clears buttons and updates text)
        try:
            await query.edit_message_caption(caption=text, reply_markup=reply_markup, parse_mode=parse_mode)
            return
        except Exception as e:
            logger.warning(f"safe_edit caption error: {e}")
            # Last resort: reply to the message
            try:
...
async def cb_domain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not assigned_domains:
        ...
        if _can:
            body = (
                "No loca\\.lt domains assigned to you yet\\.\n\n"
                "🌐 *Options:*\n"
                "• **loca\\.lt subdomain** — Buy from bot \\(₹49/30 days\\)\n"
                "• **External domain** — Add your own \\(\\.com/\\.in/\\.net etc\\.\\.\\.\\.\\)\n\n"
