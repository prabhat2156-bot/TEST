import os, asyncio, logging, time, sys, shutil, re, secrets, socket, random, gc
...
async def cb_project_dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check 2: psutil fallback (after bot restart, no proc object in memory)
    if proc_obj is not None:
        actual_alive = proc_obj.returncode is None
    elif pid:
        actual_alive = psutil.pid_exists(pid)
    else:
        actual_alive = False

    if db_status == "running" and not actual_alive:
        # Process died but DB not updated yet (process_monitor hasn't run)
        await projects_col.update_one(
...
async def cb_admin_running(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # context_store check first (most reliable), psutil as fallback
        running = []
        for p in all_db_running:
            ...
            if not alive:
                # Process is dead but DB still says "running" — fix it
                await projects_col.update_one(
...
async def process_monitor():
    while True:
        # FIX: first tick at 60s — gives auto_restart_on_startup (runs at 5s) time to
        ...
        try:
            ...
            for p in running:
                ...
                if proc is not None:
                    ...
                    # No subprocess object (bot restart scenario) — use psutil.
                    if not pid:
                        continue                          # no pid stored, skip
...
async def cb_domain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current:
        text = (
            ...
            f"✅ Domain: `{escape_md(current)}`\n\n"
            f"Your HTML project is accessible at:\n`http://{escape_md(current)}/`\n\n"
            f"File Manager: `http://{escape_md(current)}/fm/`"
        )
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✏️ Change Domain", callback_data=f"domain_set:{name}")],
            [InlineKeyboardButton("🗑 Remove Domain",  callback_data=f"domain_del:{name}")],
            [InlineKeyboardButton("🔙 Back",           callback_data=f"proj:{name}")],
        ])
    else:
        server_ip = BASE_URL.replace("https://", "").replace("http://", "").split(":")[0]
        text = (
            f"🌐 *Custom Domain — {escape_md(name)}*\n\n"
            f"No domain set yet.\n\n"
            f"*Setup:*\n"
            f"1️⃣ Go to your domain's DNS settings\n"
            f"2️⃣ Add an *A Record*:\n"
            f"   • Name: `@` or your subdomain\n"
            f"   • Value: `{escape_md(server_ip)}`\n"
            f"3️⃣ Send your domain below"
        )
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("➕ Set Domain", callback_data=f"domain_set:{name}")],
            [InlineKeyboardButton("🔙 Back",       callback_data=f"proj:{name}")],
        ])
    await safe_edit(query, text, reply_markup=kb, parse_mode=ParseMode.MARKDOWN)
