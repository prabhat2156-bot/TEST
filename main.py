...
async def _find_or_install_mvn(pdir: str = ".") -> str | None:
    ...
    try:
        ...
        mvn_version = "3.9.9"
        # Multiple mirror URLs as fallback
        mvn_urls = [
        ...
        if not downloaded:
            raise RuntimeError("All Maven mirror URLs failed")

        # Extract with proper file handle close before rename
        def _extract_tar():
        ...
        for entry in os.listdir(local_dir):
            if entry.startswith("apache-maven"):
                ...
                if os.path.isdir(extracted) and extracted != mvn_home:
        ...
        if os.path.exists(mvn_bin):
            ...
            logger.info(f"Maven downloaded successfully → {mvn_bin}")
            return mvn_bin
        else:
            logger.error(f"Maven binary not found after extraction at {mvn_bin}")

    except Exception as e:
        logger.error(f"Maven direct download failed: {e}")
...
async def new_project_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"⚠️ JavaScript aur Java *alag* hain\\!\n"
        f"JS project ke liye *Node\\.js / JS* choose karo\\.\n\n"
    ...
    if ptype == "java" and os.path.isdir(pdir):
        ...
        if not has_java_files and not has_pom and not has_gradle and (has_js_files or has_pkg_json):
            ...
            await msg_source_warn.reply_text(
                "⚠️ *Auto-correction:* Aapne *Java* select kiya tha, lekin aapki files JavaScript (Node.js) hain!\n\n"
                "Bot ne automatically *Node.js* mode switch kar diya hai. ✅",
                parse_mode=ParseMode.MARKDOWN,
            )
...
async def cb_help_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "❓ *Help & Support*\n\n"
        "Koi problem hai? Ticket raise karo — owner seedha tumhari help karega.\n\n"
        "📋 *Kya include karo ticket mein:*\n"
        "• Project name (agar related ho)\n"
        "• Kya problem aa rahi hai\n"
        "• Error message (agar koi hai)\n\n"
        "📌 *Common Solutions:*\n"
        "• Project nahi chal raha → `📦 Reinstall Requirements` try karo\n"
        "• Logs nahi dikh rahe → `📋 Logs` button click karo\n"
        "• Files upload nahi ho rahi → 10MB se choti files bhejo\n\n"
        "Ticket raise karne ke liye neeche button dabao 👇"
    )
...
async def cb_raise_ticket(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(
        query,
        "🎫 *New Support Ticket*\n\n"
        "Apni problem likho aur send karo.\n"
        "Owner ko seedha message jayega.\n\n"
        "_Jitna detail likho utna jaldi help milegi._",
        reply_markup=kb,
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def help_ticket_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_text("⚠️ Ticket khali nahi ho sakta. Dobara likho.")
        return HELP_TICKET_MSG
    ...
    if sent:
        await update.message.reply_text(
            "✅ *Ticket bhej diya gaya!*\n\n"
            "Owner ne dekha aur jaldi reply karega.\n"
            "_Patience rakhna — usually 1-24 ghante mein response aata hai._",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="back_start")]]),
        )
    else:
        await update.message.reply_text(
            "❌ Ticket bhejne mein problem aayi. Baad mein try karo.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="back_start")]]),
        )
...
def _analyze_error(log_text: str) -> str:
    """Log text se error samjhao aur Hindi mein simple explanation do."""
    ...
    if "modulenotfounderror" in log_lower or "no module named" in log_lower:
        ...
        return (
            ...
            f"Python package `{mod}` install nahi hai.\n"
            f"Bot automatically install karne ki koshish karega. ⚙️"
        )
    ...
    if "syntaxerror" in log_lower:
        ...
        return (
            ...
            f"Code mein likhne ki galti hai.\n"
            f"📁 Files → apna code open karo aur {line_info} dekho."
        )

    if "permission denied" in log_lower:
        return (
            "🔒 *Permission Denied*\n\n"
            "File ya folder ko access karne ki permission nahi mili.\n"
            "Check karo ki correct path use kar rahe ho."
        )

    if "address already in use" in log_lower or "port" in log_lower and "in use" in log_lower:
        return (
            "🌐 *Port Already In Use*\n\n"
            "Woh port pehle se koi aur process use kar raha hai.\n"
            "Apne code mein alag port number try karo."
        )

    if "connection refused" in log_lower or "connectionrefusederror" in log_lower:
        return (
            "🔌 *Connection Refused*\n\n"
            "Kisi service (database, API) se connection nahi hua.\n"
            "Check karo ki MONGODB_URI ya doosre env vars sahi set hain."
        )

    if "keyerror" in log_lower or "missing key" in log_lower:
        return (
            "🔑 *Missing Config Key*\n\n"
            "Koi required variable ya env key set nahi hai.\n"
            "🔐 Env Vars check karo — shayad koi token ya key missing ho."
        )

    if "timeout" in log_lower or "timed out" in log_lower:
        return (
            "⏱ *Connection Timeout*\n\n"
            "Kisi service ka response nahi aaya.\n"
            "Internet ya database connection slow/unavailable ho sakta hai."
        )

    if "out of memory" in log_lower or "memoryerror" in log_lower:
        return (
            "💾 *Out of Memory*\n\n"
            "Project bahut zyada RAM use kar raha hai.\n"
            "Code optimise karo ya heavy operations reduce karo."
        )

    if "typeerror" in log_lower:
        return (
            "🔴 *Type Error*\n\n"
            "Code mein galat type ka data use ho raha hai.\n"
            "Logs mein line number dekho aur fix karo."
        )

    if "importerror" in log_lower:
        return (
            "📦 *Import Error*\n\n"
            "Koi library import nahi ho rahi.\n"
            "`📦 Reinstall Requirements` try karo."
        )

    return (
        "❌ *Project Crashed*\n\n"
        "Neeche last log lines hain — inhe dekho samajhne ke liye.\n"
        "📁 Files Manager se code check karo."
    )


async def _try_auto_fix(uid: int, name: str, log_text: str) -> bool:
    """
    ModuleNotFoundError detect karo aur automatic pip install karo.
    Returns True agar fix successful raha.
    """
    import re
                        ...
                        if auto_fixed and notification_bot:
                            try:
                                await notification_bot.send_message(
                                    ...
                                    text=(
                                        f"🔧 *Auto-Fix Applied*\n\n"
                                        f"Project `{p['name']}` mein missing package mila.\n"
                                        f"Automatically install karke restart kar raha hoon... ⚙️"
                                    ),
                                    parse_mode=ParseMode.MARKDOWN,
                                )
                            except Exception:
                                ...
                                try:
                                    await notification_bot.send_message(
                                        ...
                                        text=(
                                            ...
                                            f"{smart_msg}\n\n"
                                            f"Manually restart karo bot se."
                                        ),
                                        parse_mode=ParseMode.MARKDOWN,
                                    )
                                except Exception:
