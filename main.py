...
# Admin input state per owner: {OWNER_ID: {"action": str, "step": str, ...}}
admin_state: dict = {}
    ...
    try:
        ...
        for h in captured_handlers:
            if isinstance(h, CommandHandler):
                ...
                pat_str = f" pattern={pat.pattern}" if pat else ""
                print(f"  ├ CallbackQueryHandler{pat_str}")
            else:
...
# ============================================
# ADMIN FLOW FILTER — FIX 2
# ============================================

class AdminFlowFilter(filters.MessageFilter):
    """
    Only matches messages when OWNER_ID is in an active admin input flow.

    This is the key fix for Bug 2: previously a broad User(user_id=OWNER_ID)
    filter was used, which consumed ALL owner messages even when no admin flow
    was active — blocking bot.py's handlers from receiving owner messages.

    Now the filter only matches when admin_state[OWNER_ID] is set (i.e., the
    owner has clicked Add Channel, Add Group, Remove, or Broadcast and is
    actively typing a response to an admin prompt).
    """
    def filter(self, message) -> bool:
        user = message.from_user
        if not user or user.id != OWNER_ID:
            return False
        # Only match when the owner is actively inside an admin flow
        return OWNER_ID in admin_state and admin_state[OWNER_ID] is not None


# Singleton instance — reused in build_application()
...
# ============================================
# CALLBACK HANDLERS — FIX 1: PATTERN FILTERS
# ============================================
# Each handler is registered with a specific pattern so ONLY our callbacks
# are intercepted. bot.py's callback_data values flow freely to group 1.

async def handle_verify_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Handle the 'verify_membership' button press."""
    try:
...
async def handle_no_link_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Handle the 'no_link' placeholder button press."""
    try:
        query = update.callback_query
        await query.answer(
        ...
        logger.error("handle_no_link_callback crashed: %s", e, exc_info=True)


async def handle_admin_callback_router(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """
    Route all admin_* callback data to the appropriate handler.
    Pattern: ^admin_ — only owner can interact; all others silently ignored.
    """
    try:
        query = update.callback_query
        user_id = query.from_user.id

        # Silently ignore if not the owner
        if user_id != OWNER_ID:
            await query.answer()
            return
        ...
        logger.error("handle_admin_callback_router crashed: %s", e, exc_info=True)
        try:
            await update.callback_query.answer(
                "An error occurred. Please try again.", show_alert=True
            )
...
async def handle_admin_callback(
    ...
    """Dispatch admin panel button presses (owner only, already gated above)."""
    try:
        query = update.callback_query
...
async def admin_message_handler(
    ...
    This handler is only registered with AdminFlowFilter, so it ONLY fires
    when the owner is actively inside a multi-step admin flow.
    When the owner is NOT in a flow, this handler never matches — bot.py's
    handlers in group 1 receive the owner's messages normally.
    """
    try:
        ...
        if not state:
            # AdminFlowFilter should have prevented this, but be defensive
            return
...
def make_verified_handler(original_callback):
    ...
    Key fix: when the update is a CallbackQuery (update.message is None),
    we respond with a show_alert popup instead of trying to send a message.

    Owner always passes through. Unverified users see join message or alert.
    """
    async def verified_callback(
        ...
        try:
            ...
            if verified:
                ...
                # FIX 3: callback queries have no update.message — use alert
                if update.callback_query:
                    try:
                        await update.callback_query.answer(
                            "⚠️ Please verify first! Send /start to verify.",
                            show_alert=True,
                        )
                    except Exception as e:
                        logger.warning("Verification alert for CBQ failed: %s", e)
                elif update.message:
                    await send_join_message(update, context)
                # else: some other update type — silently drop
    ...
    verified_callback.__name__ = (
        f"verified_{getattr(original_callback, '__name__', 'handler')}"
    )
...
def build_application() -> Application:
    ...
    Handler groups:
      Group 0 — Our system handlers (specific patterns, specific commands)
      Group 1 — Bot.py handlers wrapped with verification middleware

    All our CallbackQueryHandlers use explicit pattern= filters so that any
    callback_data NOT matching our patterns flows through to bot.py in group 1.
    The AdminFlowFilter ensures admin_message_handler only fires when the owner
    is mid-flow, leaving bot.py's MessageHandlers free to handle owner messages
    at all other times.
    """
    ...
    # FIX 1: Pattern-filtered CallbackQueryHandlers
    # ONLY our three known callback prefixes are intercepted here.
    # Every other callback_data passes through to bot.py handlers in group 1.
    app.add_handler(
    ...
    # FIX 2: AdminFlowFilter — only fires when owner is in an active admin flow
    app.add_handler(
        MessageHandler(
    ...
    # ── Group 1: Bot.py handlers with verification middleware ─────────────
    if original_handlers:
        for handler in original_handlers:
            try:
                # Skip /start — we own it above
