...
async def cb_domreq_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Try to assign domain from pool if it exists
    result = await domains_col.update_one(
        {"subdomain": raw, "active": True, "assigned_to": None},
        {"$set": {"assigned_to": tuid, "assigned_at": now}},
    )
    assigned_from_pool = result.modified_count > 0
    ...
    if notification_bot:
        try:
            await notification_bot.send_message(
                ...
                "✅ `" + escape_md(full_domain) + "` is now ready for you\\!\n\n"
    ...
    status_line = "✅ Domain assigned from pool." if assigned_from_pool else "⚠️ Domain not found in pool — add it manually then re-assign."
    await safe_edit(
        query,
        "✅ *Domain request approved\\!*\n\n"
        "🌐 `" + escape_md(full_domain) + "`\n"
        "👤 User ID: `" + str(tuid) + "`\n\n"
        + escape_md(status_line) + "\n\nUser has been notified\\.",
        parse_mode=ParseMode.MARKDOWN,
    )
