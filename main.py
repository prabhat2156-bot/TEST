...
async def safe_edit(query, text: str, reply_markup=None, parse_mode=_SAFE_EDIT_UNSET):
    ...
    try:
        ...
        err_str = str(e).lower()
        # FIX: When payment notification has a screenshot (photo), edit_message_text fails.
        # Fall back to edit_message_caption for photo/media messages.
        if "there is no text in the message" in err_str or "message can't be edited" in err_str:
            try:
...
async def cb_domreq_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Notify user — tell them to contact owner for setup
    if notification_bot:
        try:
            await notification_bot.send_message(
                tuid,
...
async def cb_domain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"🌐 *Custom Domain — {escape_md(name)}*\n\nSelect a loca\\.lt domain to activate, or add your own external domain:"
    kb_rows = []
