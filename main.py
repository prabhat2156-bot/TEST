...
async def domain_purchase_name_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc:
        # FIX: Domain not in owner's pre-registered pool.
        # Show up to 5 available (unassigned) domains so user can pick one.
        available_docs = await domains_col.find(
            {"active": True, "assigned_to": None}
        ).limit(5).to_list(length=5)
        if available_docs:
            names_list = "\n".join(
                "• `" + escape_md(d["subdomain"] + ".loca.lt") + "`"
                for d in available_docs
            )
            await update.message.reply_text(
                "😔 *`" + escape_md(full_domain) + "`* is not in our domain pool.\n\n"
                "Available domains you can pick:\n" + names_list + "\n\n"
                "Type one of the names above (without .loca.lt):",
                parse_mode=ParseMode.MARKDOWN,
            )
        else:
            await update.message.reply_text(
                "😔 *`" + escape_md(full_domain) + "`* is not available in our pool.\n\n"
                "No free domains in pool right now. Contact the owner to request a new domain name.\n\n"
                "Try a different name or contact @" + escape_md(OWNER_USERNAME) + ":",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup([
