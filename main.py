...
support_col  = db["support_tickets"]  # Support ticket system
...
def gen_inr_amount(base_inr: int) -> str:
    """Add random paise suffix to make each user's INR payment unique for tracking."""
    return f"{base_inr}.{random.randint(1, 99):02d}"

def gen_usdt_amount(base_usdt: str) -> str:
    """Add random extra digits to USDT amount for uniqueness (e.g. 0.55 → 0.5534)."""
    return base_usdt + str(random.randint(10, 99))
...
async def safe_edit(query, text: str, reply_markup=None, parse_mode=_SAFE_EDIT_UNSET):
    ...
    # FIX: Detect photo/media messages upfront — edit_message_text always fails on them.
    # Check message type directly instead of relying on error string matching.
    msg = getattr(query, "message", None)
    is_media = msg and (msg.photo or msg.document or msg.video or msg.audio or msg.voice)

    # FIX: reply_markup=None means "don't change keyboard" in Telegram API.
    # Use empty InlineKeyboardMarkup to actually clear the buttons.
    if reply_markup is None:
        reply_markup = InlineKeyboardMarkup([])

    if is_media:
        # For photo/media messages use edit_message_caption
        try:
            await query.edit_message_caption(caption=text, reply_markup=reply_markup, parse_mode=parse_mode)
            return
        except Exception as e:
            logger.warning(f"safe_edit caption error (markdown): {e}")
            # Retry without parse_mode (plain text) to ensure something is shown
            try:
                await query.edit_message_caption(caption=text, reply_markup=reply_markup)
                return
            except Exception as e2:
                logger.warning(f"safe_edit caption plain error: {e2}")
            # Last resort: send new reply message
            try:
    ...
    try:
        ...
        try:
            ...
            # All edits failed — send new message as fallback
            try:
                await msg.reply_text(text, reply_markup=reply_markup)
            except Exception:
                pass
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb.append([InlineKeyboardButton("🎧 Support", callback_data="support_start")])
...
async def cb_buy_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    INR_TO_USD = 83
    usd_base = round(plan["price"] / INR_TO_USD, 2)

    # Generate unique amounts so owner can match payment to user
    inr_amt  = gen_inr_amount(plan["price"])
    usdt_amt = gen_usdt_amount(str(usd_base))

    # Clear stale user_data from any previous flow
    context.user_data.clear()
    context.user_data["buying_plan"]   = plan_key
    context.user_data["pay_inr_amt"]   = inr_amt
    context.user_data["pay_usdt_amt"]  = usdt_amt
    context.user_data["pay_method"]    = ""
    ...
    text = ("💎 *" + plan["name"] + " Plan*\n\n"
            "📁 Projects: *" + proj + "*\n"
            "🌐 " + dom + "\n\n"
            "💰 Pay exactly your assigned amount:\n"
            "   💸 INR: *₹" + escape_md(inr_amt) + "*\n"
            "   ₿ USDT BEP20: *$" + escape_md(usdt_amt) + "*\n\n"
            "⚠️ _Each user gets a unique amount for tracking._\n\n"
            "Choose payment method:")
    kb = []
    if upi_id or upi_qr:
        kb.append([InlineKeyboardButton("💸 Pay ₹" + inr_amt + " via UPI", callback_data="pay_upi:" + plan_key)])
    if bsc:
        kb.append([InlineKeyboardButton("₿ Pay $" + usdt_amt + " USDT BEP20", callback_data="pay_crypto:" + plan_key)])
    if not kb:
...
async def cb_plans_domain_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Info page for domain — accessible from Plans menu."""
    query = update.callback_query
    await query.answer()
    text = ("🌐 *Custom Domain Hosting*\n\n"
            "Connect your own \.com/\.in/\.net domain to any project\!\n\n"
            "🔗 *How it works:*\n"
            "1\. Open your project → ⚙️ Settings → 🌐 My Domain\n"
            "2\. Click *Add External Domain*\n"
            "3\. Type your domain e\.g\. `mysite\.com`\n"
            "4\. Set the DNS record shown by the bot\n"
            "5\. Wait 24\-48 hours for DNS to propagate\n\n"
            "📩 *Need help with DNS or a custom subdomain?*\n"
            "Contact the owner: @" + escape_md(OWNER_USERNAME))
    kb = InlineKeyboardMarkup([
...
async def domain_purchase_name_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc:
        # Domain not in pool — save a request for the owner to create & assign it.
        doc_user = await get_user(uid)
        uname    = doc_user.get("username", "") if doc_user else ""
        us       = "@" + uname if uname else "ID:" + str(uid)
        now      = datetime.now(timezone.utc)

        existing_req = await settings_col.find_one({
            "type": "domain_request", "user_id": uid,
            "subdomain": raw, "status": "pending",
        })
        if not existing_req:
            await settings_col.insert_one({
                "type": "domain_request", "user_id": uid,
                "subdomain": raw, "full_domain": full_domain,
            ...
            try:
                kb_req = InlineKeyboardMarkup([[
                    InlineKeyboardButton(
                        "✅ Create & Assign",
                        callback_data="domreq_approve:" + str(uid) + ":" + raw,
                    ),
                    InlineKeyboardButton(
                        "❌ Reject",
                        callback_data="domreq_reject:" + str(uid) + ":" + raw,
                    ),
                ]])
                await notification_bot.send_message(
                    OWNER_ID,
                    ...
                    "Click *Create & Assign* after adding this subdomain to the pool\\.",
                    reply_markup=kb_req,
                    parse_mode=ParseMode.MARKDOWN,
                )
            except Exception as e:
                logger.error("Domain request notify error: " + str(e))

        await update.message.reply_text(
            "📩 *Domain request sent to the owner\\!*\n\n"
            "🌐 Requested: `" + escape_md(full_domain) + "`\n\n"
            "The owner will create this domain and assign it to you\\. "
            "You will receive a notification once it's ready\\. ✅\n\n"
            "_If you have an urgent need, contact @" + escape_md(OWNER_USERNAME) + " directly\\._",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📩 Contact Owner", url="https://t.me/" + OWNER_USERNAME)],
                [InlineKeyboardButton("🔙 Back to Plans", callback_data="plans_domain_info")],
            ]),
        )
        context.user_data.clear()
    ...
    # Generate unique amounts for this user's domain purchase
    inr_amt  = gen_inr_amount(49)
    usdt_amt = gen_usdt_amount("0.55")
    ...
    text = ("✅ *`" + escape_md(full_domain) + "` is available\\!*\n\n"
            "⏱ Valid for 30 days\n\n"
            "💰 Pay your assigned unique amount:\n"
            "   💸 INR: *₹" + escape_md(inr_amt) + "*\n"
            "   ₿ USDT: *$" + escape_md(usdt_amt) + "*\n\n"
            "⚠️ _Your amount is unique — helps owner verify your payment._\n\n"
            "Choose payment method:")
    kb_rows = []
    if upi_id or upi_qr:
        kb_rows.append([InlineKeyboardButton("💸 Pay ₹" + inr_amt + " via UPI", callback_data="pay_upi:__domain__")])
    if bsc:
        kb_rows.append([InlineKeyboardButton("₿ Pay $" + usdt_amt + " USDT BEP20", callback_data="pay_crypto:__domain__")])
    if not kb_rows:
        kb_rows.append([InlineKeyboardButton("📩 Contact Owner", url="https://t.me/" + OWNER_USERNAME)])
...
async def cb_pay_upi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if plan_key == "__domain__":
        ...
        # Generate unique INR amount for domain if not already set
        inr_amt = context.user_data.get("pay_inr_amt") or gen_inr_amount(49)
        context.user_data["pay_inr_amt"] = inr_amt
        pname = "Custom Domain — " + full_domain
        cancel_cb = "plans_domain_info"
    elif plan_key in PLANS:
        plan = PLANS[plan_key]
        # Use unique amount already generated in cb_buy_plan, or generate fresh
        inr_amt = context.user_data.get("pay_inr_amt") or gen_inr_amount(plan["price"])
        context.user_data["pay_inr_amt"] = inr_amt
    ...
    context.user_data.pop("pay_screenshot_file_id", None)
    context.user_data["buying_plan"] = plan_key
    context.user_data["pay_method"]  = "upi"

    parts = ["💸 *UPI Payment — " + escape_md(pname) + "*\n\n"]
    parts.append("💰 Amount: *₹" + escape_md(inr_amt) + "* _(your unique amount)_\n")
    if upi_id:
        parts.append("📲 UPI ID: `" + escape_md(upi_id) + "`\n")
    parts.append("\n*Steps:*\n")
    parts.append("1️⃣ Send *₹" + escape_md(inr_amt) + "* to the UPI ID above\n")
    parts.append("2️⃣ Take a screenshot of payment confirmation\n")
    parts.append("3️⃣ Send screenshot here 👇")
    text = "".join(parts)
...
async def pay_upi_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    method = context.user_data.get("pay_method", "upi")
    if method == "usdt":
        next_prompt = "✅ Screenshot received\\!\n\nNow send your *Transaction Hash \\(TxHash\\)*:\n\n_Copy the TxHash from your wallet or BSCScan\\._"
    else:
        next_prompt = "✅ Screenshot received\\!\n\nNow send your *UTR / Transaction ID*:\n\n_This is the 12\\-digit number in your UPI app\\._"
    await update.message.reply_text(next_prompt, parse_mode=ParseMode.MARKDOWN)
    return PAY_UPI_UTR
...
async def pay_upi_utr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    method   = context.user_data.get("pay_method", "upi")   # "upi" or "usdt"
    inr_amt  = context.user_data.get("pay_inr_amt", "")
    usdt_amt = context.user_data.get("pay_usdt_amt", "")
    paid_amt = ("$" + usdt_amt + " USDT") if method == "usdt" else ("₹" + inr_amt)
    ...
    utr_label = "TxHash" if method == "usdt" else "UTR"
    ...
    if plan_key == "__domain__":
        ...
        await settings_col.insert_one({
            ...
            "amount": paid_amt, "method": method, "utr": utr,
        ...
        if OWNER_ID and notification_bot:
            try:
                ...
                cap = ("🌐 *New Domain Payment Request*\n\n"
                       "👤 User: " + escape_md(us) + " \\(`" + str(uid) + "`\\)\n"
                       "🌐 Domain: `" + escape_md(full_domain) + "`\n"
                       "💰 Amount Paid: *" + escape_md(paid_amt) + "*\n"
                       "💳 Method: *" + method.upper() + "*\n"
                       "🔢 " + utr_label + ": `" + escape_md(utr) + "`\n"
                       "📅 " + now.strftime("%Y-%m-%d %H:%M UTC") + "\n\n"
                       "✅ Approve to assign domain to user automatically.")
                if ss:
        ...
        await update.message.reply_text(
            ...
            "🌐 Domain: `" + escape_md(full_domain) + "`\n"
            "💰 Amount: *" + escape_md(paid_amt) + "*\n"
            "🔢 " + utr_label + ": `" + escape_md(utr) + "`\n\n"
            "Owner will verify your payment and assign the domain.\nYou will receive a notification once confirmed. ✅",
            parse_mode=ParseMode.MARKDOWN,
        )
        context.user_data.clear()
        return ConversationHandler.END
    ...
    await settings_col.insert_one({
        ...
        "amount": paid_amt, "method": method, "utr": utr,
        "screenshot": ss, "created_at": now, "status": "pending",
    })
    if OWNER_ID and notification_bot:
        try:
            kb = InlineKeyboardMarkup([[
    ...
    await update.message.reply_text(
        ...
        "💰 Amount: *" + escape_md(paid_amt) + "*\n"
        "🔢 " + utr_label + ": `" + escape_md(utr) + "`\n\n"
        "The owner will review your payment and activate your plan.\nYou will receive a notification once confirmed. ✅",
        parse_mode=ParseMode.MARKDOWN,
    )
    context.user_data.clear()
    return ConversationHandler.END
...
async def cb_support_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """User clicks Support — generate ticket ID, ask for screenshots first."""
    query = update.callback_query
    await query.answer()
    uid = query.from_user.id

    ticket_id = "TKT-" + str(uid)[-4:] + "-" + str(int(time.time()))[-5:]
    context.user_data["support_ticket_id"]     = ticket_id
    context.user_data["support_screenshots"]   = []

    await safe_edit(
        query,
        ("🎧 *Support Center*\n\n"
         "🎫 Your Ticket ID: `" + ticket_id + "`\n\n"
         "📸 *Step 1:* Send screenshots of your problem\\.\n"
         "You can send *multiple screenshots* one by one\\.\n\n"
         "When done sending screenshots, send *done* to continue\\."),
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="back_start")]]),
        parse_mode=ParseMode.MARKDOWN,
    )
    return SUPPORT_SCREENSHOT


async def support_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Collect screenshots one by one; 'done' moves to description."""
    msg = update.message

    # User typed "done" — move to description step
    if msg.text and msg.text.strip().lower() in ("done", "skip", "next"):
        ss_count = len(context.user_data.get("support_screenshots", []))
        ss_line  = f"📸 {ss_count} screenshot(s) received\\." if ss_count else "📸 No screenshots \\(skipped\\)\\."
        await msg.reply_text(
            "✅ " + ss_line + "\n\n"
            "📝 *Step 2:* Now describe your problem in detail\\.\n"
    ...
    if msg.photo or msg.document:
        ...
        context.user_data.setdefault("support_screenshots", []).append(fid)
        count = len(context.user_data["support_screenshots"])
        await msg.reply_text(
            "📸 Screenshot " + str(count) + " received\\!\n\n"
            "Send more screenshots or type *done* to continue\\.",
            parse_mode=ParseMode.MARKDOWN,
        )
        return SUPPORT_SCREENSHOT

    # Not a photo and not "done"
    await msg.reply_text(
        "📸 Please send a *screenshot* or type *done* to skip screenshots\\.",
        parse_mode=ParseMode.MARKDOWN,
    )
    return SUPPORT_SCREENSHOT


async def support_describe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive problem description, save ticket, notify owner with all screenshots."""
    uid           = update.effective_user.id
    description   = update.message.text.strip()
    ticket_id     = context.user_data.get("support_ticket_id", "TKT-UNKNOWN")
    screenshots   = context.user_data.get("support_screenshots", [])
    doc           = await get_user(uid)
    uname         = doc.get("username", "") if doc else ""
    us            = "@" + uname if uname else "ID:" + str(uid)
    now           = datetime.now(timezone.utc)
    ...
    # Notify owner — send all screenshots + description
    if OWNER_ID and notification_bot:
        try:
            resolve_kb = InlineKeyboardMarkup([[
                InlineKeyboardButton("✅ Resolve", callback_data="support_resolve:" + ticket_id + ":" + str(uid)),
            ]])
            ...
            if screenshots:
                # Send first screenshot with header as caption + resolve button
                await notification_bot.send_photo(
                    OWNER_ID, photo=screenshots[0],
                    caption=header, parse_mode=ParseMode.MARKDOWN,
                    reply_markup=resolve_kb,
                )
                # Send extra screenshots silently
                for fid in screenshots[1:]:
                    await notification_bot.send_photo(OWNER_ID, photo=fid)
            else:
                await notification_bot.send_message(
                    OWNER_ID, header, parse_mode=ParseMode.MARKDOWN,
                    reply_markup=resolve_kb,
                )
        except Exception as e:
            logger.error("Support notify error: " + str(e))

    await update.message.reply_text(
        "✅ *Support ticket submitted successfully\\!*\n\n"
        "🎫 Ticket ID: `" + ticket_id + "`\n"
        "📸 Screenshots: *" + str(len(screenshots)) + "* sent\n\n"
        "Our team will review your problem and get back to you shortly\\.\n"
...
async def cb_support_resolve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner clicks Resolve — marks ticket resolved and notifies user."""
    query = update.callback_query
    await query.answer()
    ...
    tuid      = int(parts[2]) if len(parts) > 2 else 0
    ...
    # Notify user
    if tuid and notification_bot:
        try:
            await notification_bot.send_message(
                tuid,
                "✅ *Your Support Ticket Has Been Resolved\\!*\n\n"
                "🎫 Ticket ID: `" + ticket_id + "`\n\n"
                "Thank you for contacting us\\. Your issue has been resolved by our team\\. "
                "If you still face any problem, feel free to open a new support ticket\\. 🙏",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Home", url="https://t.me/" + OWNER_USERNAME)]]),
            )
        except Exception as e:
            logger.error("Support resolve notify error: " + str(e))

    await safe_edit(
        query,
        "✅ Ticket `" + ticket_id + "` marked as resolved. User has been notified.",
        parse_mode=ParseMode.MARKDOWN,
    )


# ─────────────────────────────────────────────────────────────
# 🌐 Domain Request — Owner approve / reject (no payment needed)
# ─────────────────────────────────────────────────────────────

async def cb_domreq_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner clicked 'Create & Assign' on a domain request notification."""
    query = update.callback_query
    await query.answer()
    ...
    if len(parts) < 3:
        await safe_edit(query, "❌ Invalid callback data.")
        return
    tuid = int(parts[1])
    raw  = parts[2]
    full_domain = raw + ".loca.lt"
    ...
    await settings_col.update_one(
        {"type": "domain_request", "user_id": tuid, "subdomain": raw, "status": "pending"},
        {"$set": {"status": "approved", "approved_at": now}},
    )

    # Notify user — use notification_bot if set, else fall back to context.bot
    _notify_bot = notification_bot or context.bot
    try:
        await _notify_bot.send_message(
            tuid,
            "✅ *Domain Request Accepted\\!*\n\n"
            "🌐 Requested: `" + escape_md(full_domain) + "`\n\n"
            "📩 The owner has accepted your request\\. Please contact @" + escape_md(OWNER_USERNAME) + " directly to complete the domain setup\\.\n\n"
            "Mention your requested domain name in the message\\. 🙏",
            parse_mode=ParseMode.MARKDOWN,
        )
    except Exception as e:
        logger.error("domreq approve notify error: " + str(e))
    ...
    await safe_edit(
        ...
        "✅ User has been notified to contact you\\.",
        parse_mode=ParseMode.MARKDOWN,
    )


async def cb_domreq_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner clicked 'Reject' on a domain request notification."""
    query = update.callback_query
    await query.answer()
    if query.from_user.id != OWNER_ID:
        await query.answer("⛔ Owner only", show_alert=True)
    ...
    if notification_bot:
        try:
            await notification_bot.send_message(
                tuid,
                "❌ *Domain Request Not Available*\n\n"
                "Sorry, `" + escape_md(full_domain) + "` could not be created at this time\\.\n\n"
                "Please try a different name or contact @" + escape_md(OWNER_USERNAME) + " for help\\.",
                parse_mode=ParseMode.MARKDOWN,
            )
        except Exception as e:
            logger.error("domreq reject notify error: " + str(e))

    await safe_edit(
        query,
        "❌ Domain request for `" + escape_md(full_domain) + "` rejected\\. User has been notified\\.",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def cb_pay_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if plan_key == "__domain__":
        ...
        paid_amt    = pmt.get("amount", "₹49") if pmt else "₹49"
        method      = pmt.get("method", "UPI").upper() if pmt else "UPI"
        utr_val     = pmt.get("utr", "—") if pmt else "—"

        # Mark payment approved in history
        await settings_col.update_one(
            {"type": "pending_payment", "user_id": tuid, "plan": "__domain__", "status": "pending"},
            {"$set": {"status": "approved", "approved_at": now}},
            sort=[("created_at", -1)],
        ...
        # Notify user — tell them to contact owner for domain setup
        _notify_bot = notification_bot or context.bot
        try:
            await _notify_bot.send_message(
                tuid,
                "✅ *Domain Payment Approved\\!*\n\n"
                "💰 Amount: *" + escape_md(str(paid_amt)) + "*  \\(" + escape_md(method) + "\\)\n"
                "🌐 Requested Domain: `" + escape_md(full_domain) + "`\n\n"
                "📩 Your payment has been verified\\. Please contact the owner directly to get your domain assigned:\n"
                "@" + escape_md(OWNER_USERNAME) + "\n\n"
                "Mention your domain name and this approval message\\. 🙏",
                parse_mode=ParseMode.MARKDOWN,
            )
        except Exception as e:
            logger.error("Domain approve notify error: " + str(e))

        # Confirm to owner — clear buttons
        await safe_edit(
            query,
            "✅ *DOMAIN PAYMENT — ACCEPTED*\n\n"
            "👤 User: `" + str(tuid) + "`\n"
            "🌐 Domain: `" + escape_md(full_domain) + "`\n"
            "💰 Amount: *" + escape_md(str(paid_amt)) + "*\n"
            "💳 Method: *" + escape_md(method) + "*\n"
            "🔢 UTR: `" + escape_md(utr_val) + "`\n"
            "📅 Approved: `" + now.strftime("%Y-%m-%d %H:%M UTC") + "`\n\n"
            "✅ User has been notified\\. Please assign the domain manually\\.",
            parse_mode=ParseMode.MARKDOWN,
        )
        return
    ...
    # Fetch payment record for details
    pmt = await settings_col.find_one(
        {"type": "pending_payment", "user_id": tuid, "plan": plan_key, "status": "pending"},
        sort=[("created_at", -1)],
    )
    paid_amt = pmt.get("amount", "—") if pmt else "—"
    method   = pmt.get("method", "—").upper() if pmt else "—"
    utr_val  = pmt.get("utr", "—") if pmt else "—"
    ...
    pj = "Unlimited" if not plan["projects"] else str(plan["projects"])
    ...
    try:
        await _notify_bot.send_message(
            tuid,
            "🎉 *Payment Approved — Plan Active\\!*\n\n"
            "✅ *Your " + escape_md(plan["name"]) + " plan is now APPROVED\\!*\n\n"
            "💎 Plan: *" + plan["name"] + "*\n"
            "💰 Amount: *" + escape_md(str(paid_amt)) + "*\n"
            "📁 Projects: *" + pj + "*\n"
            "⏳ Valid for: *30 days*\n"
            "📅 Expires: `" + expiry.strftime("%Y-%m-%d %H:%M UTC") + "`\n\n"
            "Your plan is now active\\! Go to /start to use your features\\. 🚀",
            parse_mode=ParseMode.MARKDOWN,
        )
    except Exception as e:
        logger.error("Approve notify user error: " + str(e))

    # Confirm to owner — clear the Approve/Reject buttons
    await safe_edit(
        query,
        "✅ *PAYMENT — ACCEPTED*\n\n"
        "👤 User: `" + str(tuid) + "`\n"
        ...
        "💳 Method: *" + escape_md(method) + "*\n"
        "🔢 UTR: `" + escape_md(utr_val) + "`\n"
        "📅 Approved: `" + expiry.strftime("%Y-%m-%d") + "`\n"
...
async def cb_pay_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await _notify_bot.send_message(
            tuid,
            "❌ *Payment Rejected*\n\n"
            "Your payment for *" + escape_md(pname) + "* was rejected\\.\n\n"
            "💰 Amount: *" + escape_md(str(paid_amt)) + "*\n"
            "💳 Method: *" + escape_md(method) + "*\n"
            "🔢 UTR: `" + escape_md(utr_val) + "`\n\n"
            "Possible reasons:\n"
            "• Wrong amount sent\n"
            "• UTR/TxHash incorrect\n"
            "• Screenshot not clear\n\n"
            "Contact @" + escape_md(OWNER_USERNAME) + " if you think this is a mistake\\.",
            parse_mode=ParseMode.MARKDOWN,
        )
    except Exception as e:
        logger.error("Reject notify user error: " + str(e))
    ...
    await safe_edit(
        ...
        "❌ *PAYMENT — REJECTED*\n\n"
        "👤 User: `" + str(tuid) + "`\n"
        "📦 Plan: *" + escape_md(pname) + "*\n"
        "💰 Amount: *" + escape_md(str(paid_amt)) + "*\n"
        "💳 Method: *" + escape_md(method) + "*\n"
        "🔢 UTR: `" + escape_md(utr_val) + "`\n"
        "📅 Rejected: `" + now.strftime("%Y-%m-%d %H:%M UTC") + "`\n\n"
        "❌ User has been notified\\.",
        parse_mode=ParseMode.MARKDOWN,
    )

async def cb_pay_crypto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """USDT BEP20 payment — now fully manual (screenshot + TxHash), same flow as UPI."""
    query = update.callback_query
    await query.answer()
    uid = query.from_user.id
    plan_key = query.data.split(":", 1)[1]
    ...
    if plan_key == "__domain__":
        domain_name = context.user_data.get("domain_purchase_name", "")
        full_domain = domain_name + ".loca.lt" if domain_name else "your domain"
        usdt_amt = context.user_data.get("pay_usdt_amt") or gen_usdt_amount("0.55")
        context.user_data["pay_usdt_amt"] = usdt_amt
        context.user_data["buying_plan"]  = "__domain__"
        ...
        plan = PLANS[plan_key]
        usdt_base = str(round(plan["price"] / 83, 2))
        usdt_amt = context.user_data.get("pay_usdt_amt") or gen_usdt_amount(usdt_base)
        context.user_data["pay_usdt_amt"] = usdt_amt
        context.user_data["buying_plan"]  = plan_key
        context.user_data["pay_method"]   = "usdt"
        cancel_cb = "plans"
    ...
    if not bsc:
        await safe_edit(
            query,
            "⚠️ USDT payment not configured yet.\nContact the owner to set up a BSC address.",
    ...
    text = ("₿ *USDT BEP20 Payment — " + escape_md(pname) + "*\n\n"
            "💰 Amount: *$" + escape_md(usdt_amt) + " USDT* _(your unique amount)_\n"
            "🌐 Network: *BEP20 \\(BSC\\)*\n\n"
            "📤 Send to:\n`" + escape_md(bsc) + "`\n\n"
            "⚠️ *Send ONLY USDT on BEP20/BSC network\\!*\n\n"
            "*Steps:*\n"
            "1️⃣ Send *$" + escape_md(usdt_amt) + " USDT* to address above\n"
            "2️⃣ Take a screenshot of transaction confirmation\n"
...
async def cb_crypto_verify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Shared helper: notify owner for manual review ──────────────
    async def _notify_owner_manual(reason: str = ""):
        if OWNER_ID and notification_bot:
            try:
                doc = await get_user(uid)
                uname = "@" + doc.get("username","") if doc and doc.get("username") else "ID:" + str(uid)
                note = ("\n❗ Reason: `" + escape_md(reason[:200]) + "`") if reason else "\nVerify on BSCscan manually."
                await notification_bot.send_message(
                    OWNER_ID,
                    "₿ *Crypto Claim — Manual Review*\n\n"
                    "User " + escape_md(uname) + " (`" + str(uid) + "`) claims to have sent *"
                    + str(expected_usdt) + " USDT* for *" + escape_md(plan_label) + "*" + note,
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("✅ Approve", callback_data="pay_approve:" + str(uid) + ":" + plan_key),
                        InlineKeyboardButton("❌ Reject",  callback_data="pay_reject:"  + str(uid) + ":" + plan_key),
    ...
    # ── BscScan API verification (replaces Binance API) ───────────
    # Uses BSC blockchain directly — no Binance account or special
    # permissions needed. BSCSCAN_API_KEY is optional (free at bscscan.com)
    # but recommended to avoid rate limits.
    import urllib.request as _ur, json as _json
    bsc_addr   = settings.get("binance_bsc_address", "")
    bscan_key  = os.getenv("BSCSCAN_API_KEY", "YourApiKeyToken")
    # USDT BEP20 contract on BSC mainnet
    USDT_CONTRACT = "0x55d398326f99059fF775485246999027B3197955"

    if not bsc_addr:
        await _notify_owner_manual("BSC address not configured")
        await safe_edit(
            query,
    ...
    try:
        cutoff_block_time = int(time.time()) - 3600  # last 1 hour
        url = (
        ...
        if isinstance(txs, list):
            for tx in txs:
                ...
                if tx.get("to", "").lower() != bsc_addr.lower():
                    continue
                # Token must be USDT
                if tx.get("tokenSymbol", "") not in ("USDT", "BSC-USD"):
                    continue
                # Amount check (USDT has 18 decimals on BSC)
                try:
                    decimals = int(tx.get("tokenDecimal", "18"))
                    tx_amount = int(tx.get("value", "0")) / (10 ** decimals)
                except Exception:
                    continue
                if tx_amount < expected_usdt * 0.95:
                    continue
                # Time check — within last hour
                try:
                    tx_time = int(tx.get("timeStamp", "0"))
                except Exception:
                    tx_time = 0
                if tx_time < cutoff_block_time:
                    continue
                found = True
                break
        ...
        logger.error("BscScan verify: " + str(e))
        await _notify_owner_manual("BscScan API error: " + str(e)[:150])
        await safe_edit(
            query,
            "⚠️ *Verification temporarily unavailable.*\n\nYour payment claim has been sent to the owner for manual review. You will be notified once approved.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Home", callback_data="back_start")]]),
            parse_mode=ParseMode.MARKDOWN,
        )
...
async def cb_admin_purchase_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in recs:
        ...
        rejected   = r.get("rejected_at")
        rejected_s = rejected.strftime("%d %b %Y  %H:%M UTC") if rejected else "—"
        ...
        if rejected_s != "—":
            lines.append("❌ Rejected: " + rejected_s + "\n")
        if expiry_s != "—":
...
async def admin_remove_premium_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await users_col.update_one(
        {"user_id": uid},
        {"$set": {"is_premium": False, "premium_expiry": None, "plan": "free", "plan_expiry": None}},
    )
    # Lock extra projects
    ...
    try:
        await update.get_bot().send_message(
            ...
            "Your account has been downgraded to the *Free* plan by the owner\\.\n"
            "Extra projects have been locked\\.\n\n"
            "Contact @" + escape_md(OWNER_USERNAME) + " for more info\\.",
            parse_mode=ParseMode.MARKDOWN,
        )
    except Exception:
        pass
    await update.message.reply_text(
        "✅ Premium removed from `" + str(uid) + "`\\.\n"
        "Extra projects locked\\. User notified\\.",
        parse_mode=ParseMode.MARKDOWN,
    )
    return ConversationHandler.END
...
async def cb_domain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not assigned_domains:
        ...
        if _can:
            body = (
                ...
                "🌐 *Option:*\n"
                "• Add your own domain \\(\\.com/\\.in/\\.net etc\\.\\)\n\n"
                "Click below to connect your domain\\.\n\n"
        ...
        kb2 = []
        if _can:
            kb2.append([InlineKeyboardButton("🔗 Add External Domain (.com/.in/etc)", callback_data="domain_set:" + name)])
        else:
    ...
    kb_rows.append([InlineKeyboardButton("🔗 Add External Domain (.com/.in/etc)", callback_data="domain_set:" + name)])
    kb_rows.append([InlineKeyboardButton("🔙 Back", callback_data=f"proj:{name}")])
    await safe_edit(
        query, text,
...
async def domain_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Extract server host from BASE_URL for DNS instructions
    import urllib.parse as _urlparse
    _parsed    = _urlparse.urlparse(BASE_URL)
    server_host = _parsed.hostname or BASE_URL.replace("https://", "").replace("http://", "").split("/")[0]

    await update.message.reply_text(
        "✅ *Domain Saved: `" + escape_md(raw) + "`*\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "📋 *DNS Setup Required*\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        ...
        "TTL  : Automatic / 3600\n"
        "```\n\n"
        "💡 *If your registrar doesn't allow CNAME on root \\(@\\), use A record:*\n"
        "```\n"
        "Type : A\n"
        "Name : @\n"
        "Value: [Server IP from owner]\n"
        "```\n\n"
        "⏳ DNS changes take *24\\-48 hours* to propagate worldwide\\.\n\n"
        "📩 If unsure, contact @" + escape_md(OWNER_USERNAME) + " for the exact server IP\\.  🙏",
        parse_mode=ParseMode.MARKDOWN,
    )
    return ConversationHandler.END
...
def build_application() -> Application:
    ...
    # NOTE: cb_pay_crypto is entry_point in upi_conv below — NOT standalone
    # cb_crypto_verify is dead code (USDT is now fully manual) — NOT registered
    ...
    app.add_handler(CallbackQueryHandler(cb_domreq_approve, pattern=r"^domreq_approve:"))
    app.add_handler(CallbackQueryHandler(cb_domreq_reject,  pattern=r"^domreq_reject:"))
    ...
    domain_purchase_conv = ConversationHandler(
        ...
        states={
            DOM_PURCHASE_NAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, domain_purchase_name_receive),
                # End conversation when user clicks any cancel/back/navigation button
                CallbackQueryHandler(admin_conv_cancel, pattern="^plans_domain_info$"),
                CallbackQueryHandler(admin_conv_cancel, pattern="^plans$"),
                CallbackQueryHandler(admin_conv_cancel, pattern="^back_start$"),
                CallbackQueryHandler(admin_conv_cancel, pattern="^premium$"),
                CallbackQueryHandler(admin_conv_cancel, pattern="^support_start$"),
                CallbackQueryHandler(admin_conv_cancel, pattern="^my_projects$"),
            ],
        },
        fallbacks=[
            CommandHandler("start", start),
            CommandHandler("cancel", start),
            # Any unmatched callback query ends the conversation cleanly
            CallbackQueryHandler(admin_conv_cancel),
        ],
    ...
    # ── UPI + USDT payment conversation ──
    # IMPORTANT: All entry_points here — NOT registered as standalone handlers.
    ...
    upi_conv = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(cb_pay_crypto,     pattern=r"^pay_crypto:"),   # USDT now manual
    ...
    support_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(cb_support_start, pattern="^support_start$")],
        states={
            SUPPORT_SCREENSHOT: [
                MessageHandler(filters.PHOTO | filters.Document.ALL, support_screenshot),
                MessageHandler(filters.TEXT & ~filters.COMMAND,      support_screenshot),
            ],
            SUPPORT_DESCRIBE: [MessageHandler(filters.TEXT & ~filters.COMMAND, support_describe)],
        },
    ...
    app.add_handler(CallbackQueryHandler(cb_support_resolve, pattern=r"^support_resolve:"))
