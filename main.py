...
async def cb_plans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, p in PLANS.items():
        ...
        stop  = "12hr auto-stop" if p["autostop_hours"] else "No auto-stop"
        dom   = "✅ Custom domain" if p["custom_domain"] else "❌ No custom domain"
        price = "Free" if p["price"] == 0 else "₹" + str(p["price"]) + "/month"
        cur   = " ← *You*" if key == plan_key else ""
        icon  = emojis.get(key, "•")
        lines.append(icon + " *" + p["name"] + "* — " + price + cur + "\n")
        lines.append("   • " + proj + " project(s) · " + stop + "\n")
        lines.append("   • " + dom + "\n\n")
    lines.append("*UPI:* Screenshot + UTR → owner approves\n")
    lines.append("*Crypto:* USDT BEP20 auto-verify via Binance API")
    kb_rows = []
    for key, p in PLANS.items():
        if key == "free" or key == plan_key:
            continue
        kb_rows.append([InlineKeyboardButton(
            "💳 Buy " + p["name"] + " — ₹" + str(p["price"]) + "/mo",
...
async def cb_buy_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = []
    if upi_id or upi_qr:
        kb.append([InlineKeyboardButton("💸 Pay via UPI", callback_data="pay_upi:" + plan_key)])
    kb.append([InlineKeyboardButton("₿ Pay via USDT (BEP20)", callback_data="pay_crypto:" + plan_key)])
    kb.append([InlineKeyboardButton("🔙 Back", callback_data="plans")])
    text = "💎 *" + plan["name"] + " Plan* — ₹" + str(plan["price"]) + "/month\n\nChoose payment method:"
    await safe_edit(query, text, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
...
async def cb_pay_upi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "".join(parts)
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="plans")]])
    if upi_qr:
...
async def pay_upi_utr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not plan_key or plan_key not in PLANS:
        await update.message.reply_text("❌ Session expired. Start again from /start.")
...
async def cb_pay_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await settings_col.update_one(
        {"type": "pending_payment", "user_id": tuid, "status": "pending"},
    ...
    if not bkey or not bsec:
        if OWNER_ID and notification_bot:
            try:
                ...
                await notification_bot.send_message(
                    OWNER_ID,
                    "₿ *Crypto Claim*\n\nUser " + escape_md(uname) + " (`" + str(uid) + "`) claims to have sent *" + str(expected_usdt) + " USDT* for *" + plan.get("name","?") + "* plan.\n\nVerify on BSC explorer.",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("✅ Approve", callback_data="pay_approve:" + str(uid) + ":" + plan_key),
                        InlineKeyboardButton("❌ Reject",  callback_data="pay_reject:"  + str(uid) + ":" + plan_key),
    ...
    try:
        ...
        if found:
            ...
            await safe_edit(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔄 Try Again", callback_data="crypto_verify:" + plan_key + ":" + str(expected_usdt))],
...
def build_application() -> Application:
    ...
    app.add_handler(CallbackQueryHandler(cb_pay_upi,     pattern=r"^pay_upi:"))
    ...
    app.add_handler(CallbackQueryHandler(cb_admin_set_bsc_address,  pattern="^admin:set_bsc_address$"))
    app.add_handler(CallbackQueryHandler(cb_domain_buy_localt, pattern=r"^domain_buy_localt:"))
    app.add_handler(CallbackQueryHandler(cb_domain_buy_pay,    pattern=r"^domain_buy_pay:"))

    # ── UPI payment conversation ──
    upi_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(cb_pay_upi, pattern=r"^pay_upi:")],
        states={
