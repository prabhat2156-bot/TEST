...
import os, asyncio, logging, time, sys, shutil, re, secrets, gc, base64, zipfile, signal
...
from emoji_config import (
    premium_emoji, BTN_SUCCESS, BTN_DANGER, BTN_PRIMARY,
    markdown_to_html_auto, emoji_only_html,
)
...
# ─────────────────────────────────────────────────────────────
# Bot-wide premium emoji auto-conversion.
#
# Every text-sending call the bot makes (safe_edit, message.reply_text,
# message.edit_text, bot.send_message, query.edit_message_text, ...) goes
# through python-telegram-bot's Bot.send_message / Bot.edit_message_text
# under the hood. Patching those two methods once here means every single
# message in the bot automatically gets its plain emoji swapped for premium
# emoji (once an ID is filled in emoji_config.py) — no need to touch each of
# the 200+ individual call sites in this file.
#   - parse_mode=MARKDOWN  -> converted to real HTML (bold/code + emoji swap)
#   - parse_mode=HTML      -> assumed already valid HTML, emoji swap only
#   - parse_mode=None/other -> left completely untouched
# ─────────────────────────────────────────────────────────────
_orig_bot_send_message = _TgBot.send_message
_orig_bot_edit_message_text = _TgBot.edit_message_text


def _convert_text_for_parse_mode(text, parse_mode):
    if not isinstance(text, str):
        return text, parse_mode
    if parse_mode == ParseMode.MARKDOWN or parse_mode == ParseMode.MARKDOWN_V2:
        return markdown_to_html_auto(text), ParseMode.HTML
    if parse_mode == ParseMode.HTML:
        return emoji_only_html(text), parse_mode
    return text, parse_mode


async def _patched_send_message(self, chat_id, text, *args, **kwargs):
    parse_mode = kwargs.get("parse_mode")
    text, new_parse_mode = _convert_text_for_parse_mode(text, parse_mode)
...
_TgBot.edit_message_text = _patched_edit_message_text
...
# Free-tier projects auto-stop after this many hours of continuous runtime;
# premium users (and the owner) are exempt and can run 24/7.
FREE_RUNTIME_LIMIT_HOURS   = 12
FREE_RUNTIME_LIMIT_SECONDS = FREE_RUNTIME_LIMIT_HOURS * 3600
...
async def check_premium_expiry(user_id: int, doc: dict | None = None):
    """Strip premium if expired. Also lock extra projects for expired premium users.

    OPT: accepts an already-fetched user doc (e.g. from is_premium) to avoid a
    second round-trip to Mongo — callers that already have the doc should pass
    it in. Falls back to a fresh fetch when not provided.
    """
    if doc is None:
...
async def is_premium(user_id: int) -> bool:
    # OPT: single DB round-trip instead of two (was: check_premium_expiry
    # fetching the doc, then get_user fetching it again right after).
    doc = await get_user(user_id)
    if not doc:
        return False
    await check_premium_expiry(user_id, doc=doc)
    return bool(doc.get("is_premium"))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_banned(user.id):
        await update.message.reply_text("🚫 You are banned. Contact owner.", parse_mode=ParseMode.HTML)
        return

    doc      = await get_user(user.id)
    premium  = doc.get("is_premium", False)
    count    = await project_count(user.id)
    plan_lbl = f"Premium {premium_emoji('star', '✨')}" if premium else "Free"
    limit_lbl = "∞" if premium else str(FREE_LIMIT)

    # Bot lock status line
    lock_line = ""
    if await is_bot_locked() and not premium and user.id != OWNER_ID:
        lock_line = "\n\n🔒 <b>Bot is locked.</b> New projects &amp; running restricted to Premium users."

    text = (
        f"{premium_emoji('star', '🌟')} <b>Welcome to God Hosting Bot!</b>\n\n"
        ...
        f"{premium_emoji('diamond', '💎')} Plan: {plan_lbl}\n"
    ...
    kb = [
        [
            InlineKeyboardButton(f"{BTN_SUCCESS} New Project", callback_data="new_project"),
            InlineKeyboardButton(f"{BTN_PRIMARY} My Projects", callback_data="my_projects"),
        ],
        [
            InlineKeyboardButton("💎 Premium",        callback_data="premium"),
            InlineKeyboardButton(f"{BTN_PRIMARY} My Status", callback_data="my_status"),
        ],
    ]
...
async def cb_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_maintenance_mode() and user.id != OWNER_ID:
        await safe_edit(query, "🔧 <b>Bot is under maintenance.</b> Only the owner can use it right now.", parse_mode=ParseMode.HTML)
        return

    if await is_banned(user.id):
        await safe_edit(query, "🚫 You are banned. Contact owner.", parse_mode=ParseMode.HTML)
        return
    ...
    await safe_edit(query, text, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.HTML)
...
async def cb_bot_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"{BTN_PRIMARY} Refresh", callback_data="bot_status"),
...
async def cb_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if premium:
        text = (
            f"{premium_emoji('diamond', '💎')} <b>Premium Membership</b>\n\n"
            f"{premium_emoji('star', '✨')} <b>You are Premium!</b> {premium_emoji('star', '✨')}\n\n"
            + features +
            f"{premium_emoji('fire', '🌟')} Premium is active!"
        )
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data="back_start")]])
    else:
        text = (
            f"{premium_emoji('diamond', '💎')} <b>Premium Membership</b>\n\n"
            + features +
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"{BTN_SUCCESS} Contact Owner", url=f"https://t.me/{OWNER_USERNAME}")],
            [InlineKeyboardButton(f"{BTN_PRIMARY} Back",          callback_data="back_start")],
        ])
...
async def cb_my_projects(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb_rows.append([InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data="back_start")])
...
async def cb_my_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{BTN_PRIMARY} Refresh",    callback_data="my_status"),
...
def project_dashboard_kb(user_id: int, project_name: str, auto_restart: bool = True,
    ...
    if is_locked:
        ...
        row1 = [
            InlineKeyboardButton(f"{BTN_DANGER} Stop",    callback_data=f"stop:{pn}"),
            InlineKeyboardButton(f"{BTN_PRIMARY} Restart", callback_data=f"restart:{pn}"),
            InlineKeyboardButton(f"{BTN_PRIMARY} Logs",    callback_data=f"logs:{pn}"),
        ]
    else:
        row1 = [
            InlineKeyboardButton(f"{BTN_SUCCESS} Run",     callback_data=f"run:{pn}"),
            InlineKeyboardButton(f"{BTN_PRIMARY} Restart", callback_data=f"restart:{pn}"),
            InlineKeyboardButton(f"{BTN_PRIMARY} Logs",    callback_data=f"logs:{pn}"),
        ]
    ...
    rows = [
        ...
        [
            ...
            InlineKeyboardButton(f"{BTN_PRIMARY} Refresh", callback_data=f"proj:{pn}"),
        ],
        [
            InlineKeyboardButton("✏️ Edit CMD", callback_data=f"editcmd:{pn}"),
            InlineKeyboardButton(f"{BTN_PRIMARY} Files",    callback_data=f"filemgr:{pn}"),
        ...
        [
            InlineKeyboardButton(f"{BTN_DANGER} Delete", callback_data=f"delete:{pn}"),
            InlineKeyboardButton(f"{BTN_PRIMARY} Back",   callback_data="my_projects"),
        ],
    ]
...
async def cb_reinstall_reqs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    back_kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data=f"proj:{name}")]])
    ...
    if os.path.exists(pkg_json) and not os.path.exists(req_path):
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"{BTN_PRIMARY} Restart Project", callback_data=f"restart:{name}")],
            [InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data=f"proj:{name}")],
        ])
...
async def cb_run(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_bot_locked() and not user_premium and uid != OWNER_ID:
        await safe_edit(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data=f"proj:{name}")]]),
            parse_mode=ParseMode.MARKDOWN,
        )
        return
...
def _kill_pid_tree(pid: int) -> None:
    """Kill a PID, its process group, and any descendants.

    FIX: the old code only called proc.kill() on the single stored PID (plus
    psutil-reported children at that instant). Processes are started with
    start_new_session=True, which makes the child its own session/process
    group leader — so killing just the PID can leave grandchildren or
    re-parented processes alive in that group. We now also SIGKILL the whole
    process group via os.killpg, which is the reliable way to take down
    everything started in that session.
    """
    try:
...
async def kill_project(uid: int, name: str) -> bool:
    """Stop a project's process and mark it stopped in the DB.

    Returns True only if the process was actually confirmed dead. FIX: the
    old implementation trusted the DB-stored PID alone, wrapped the kill in a
    broad try/except, and immediately wrote status="stopped" without ever
    checking whether the process actually died. If the stored PID was stale
    (VPS/RDP PID drift, a prior auto-restart, or the psutil.Process() call
    raising NoSuchProcess because the *real* running process had a different
    PID) the kill silently did nothing, yet the UI/DB still reported
    "stopped" while the real process kept running. This version also
    re-discovers the process by project directory (like the rest of the
    codebase already does for start/monitor), retries once, verifies the
    process is actually gone before flipping status, and — if it still can't
    be killed — leaves status as "running" so the UI reflects reality instead
    of lying about it.
    """
    p = await get_project(uid, name)
    ...
    # Re-discover by CWD/cmdline in case the stored PID was stale — this is
    # the same lookup start/monitor code already uses for the same reason.
    _found, _real_pid = find_project_process(pdir)
    if _found and _real_pid and _real_pid not in killed_pids:
        _kill_pid_tree(_real_pid)
        killed_pids.add(_real_pid)

    # Verify the process actually died; retry once before giving up.
    await asyncio.sleep(0.3)
    _still_running, _still_pid = find_project_process(pdir)
    if _still_running and _still_pid:
        _kill_pid_tree(_still_pid)
        await asyncio.sleep(0.3)
        _still_running, _still_pid = find_project_process(pdir)

    stopped = not _still_running
    update_fields = {
        "status": "stopped" if stopped else "running",
        "pid": None if stopped else _still_pid,
        **uptime_update,
    }
    await projects_col.update_one(
        {"user_id": uid, "name": name},
    ...
    # Clean up context_store entry only once actually stopped
    if stopped:
...
async def cb_stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stopped = await kill_project(uid, name)
    if not stopped:
        logger.error(f"Failed to stop project {name} for user {uid} — process still alive after kill attempts")

    p = await get_project(uid, name)
    _up = await is_premium(uid) or uid == OWNER_ID
    await safe_edit(
        query,
        project_dashboard_text(p) + ("" if stopped else "\n\n⚠️ Could not stop the process — it is still running. Please try again."),
        reply_markup=project_dashboard_kb(uid, name, p.get("auto_restart", True),
                                          p.get("status") == "running", p.get("locked", False),
                                          user_premium=_up),
    )
...
async def cb_logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb   = InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data=f"proj:{name}")]])
    await safe_edit(query, text, reply_markup=kb, parse_mode=ParseMode.MARKDOWN)
...
async def cb_editcmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_DANGER} Cancel", callback_data=f"proj:{name}")]])
    await safe_edit(
        query,
...
async def cb_delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb   = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(f"{BTN_DANGER} Yes, Delete", callback_data=f"delete_yes:{name}"),
            InlineKeyboardButton(f"{BTN_DANGER} Cancel",       callback_data=f"proj:{name}"),
        ],
    ])
    await safe_edit(
        query,
...
async def cb_new_project(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_bot_locked() and not user_premium and uid != OWNER_ID:
        await safe_edit(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data="back_start")]]),
            parse_mode=ParseMode.MARKDOWN,
        )
        return ConversationHandler.END
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_DANGER} Cancel", callback_data="back_start")]])
    await safe_edit(
        query,
...
async def new_project_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"{BTN_DANGER} Cancel",          callback_data="back_start")],
    ])
    await update.message.reply_text(
...
async def cb_admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    role_label = f"{premium_emoji('crown', '👑')} Owner" if uid == OWNER_ID else "🛡 Admin"

    text = (
        f"⚙️ <b>Admin Panel</b> ({role_label})\n\n"
        f"👥 Total Users: <code>{total_users}</code>\n"
        f"{premium_emoji('diamond', '💎')} Premium: <code>{premium_count}</code>\n"
        f"🛡 Admins: <code>{admin_count}</code>\n"
        f"🚫 Banned: <code>{banned_count}</code>\n"
        f"📁 Projects: <code>{total_proj}</code>\n"
        f"🟢 Running: <code>{running_proj}</code>"
        f"{db_count_line}"
        f"{backup_info}\n"
        f"🔒 Bot Lock: <code>{lock_icon}</code>\n"
        f"🔧 Maintenance: <code>{maint_icon}</code>\n"
        f"🗄 Active DB: <code>{active_db_label}</code>"
    )

    kb_rows = [
        [InlineKeyboardButton(f"{BTN_PRIMARY} User List",  callback_data="admin:user_list:0"),
         InlineKeyboardButton(f"{BTN_SUCCESS} Running Scripts",  callback_data="admin:running")],
        [InlineKeyboardButton("💎 Give Premium",     callback_data="admin:give_premium"),
         InlineKeyboardButton(f"{BTN_DANGER} Remove Premium",   callback_data="admin:remove_premium")],
        [InlineKeyboardButton(f"{BTN_PRIMARY} Temp Premium",     callback_data="admin:temp_premium"),
         InlineKeyboardButton(f"{BTN_DANGER} Ban User",         callback_data="admin:ban")],
        [InlineKeyboardButton(f"{BTN_SUCCESS} Unban User",       callback_data="admin:unban"),
         InlineKeyboardButton(f"{BTN_PRIMARY} Broadcast",        callback_data="admin:broadcast_menu")],
        [InlineKeyboardButton(f"{BTN_SUCCESS} Backup Now",       callback_data="admin:backup_now"),
         InlineKeyboardButton(f"{BTN_DANGER} Delete All Backup", callback_data="admin:del_backups")],
        [InlineKeyboardButton(f"{BTN_PRIMARY} Bot Status",       callback_data="bot_status"),
         InlineKeyboardButton(f"{BTN_PRIMARY} DB Viewer",       callback_data="db_viewer")],
    ]

    # Owner-only buttons
    if uid == OWNER_ID:
        kb_rows.append([
            InlineKeyboardButton(f"{BTN_SUCCESS} Add Admin",    callback_data="admin:add_admin"),
            InlineKeyboardButton(f"{BTN_DANGER} Remove Admin", callback_data="admin:remove_admin"),
        ])
    ...
    await safe_edit(query, text, reply_markup=InlineKeyboardMarkup(kb_rows), parse_mode=ParseMode.HTML)
...
async def cb_admin_db_switch_to_local(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb_confirm = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"{BTN_DANGER} Cancel", callback_data="admin:db_settings")],
    ])
    await safe_edit(
        query,
...
async def cb_admin_backup_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data="admin_panel")]]),
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def cb_admin_user_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if nav:
        kb_rows.append(nav)
    kb_rows.append([InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data="admin_panel")])
...
async def cb_admin_all_projects(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not page_user_ids:
        await safe_edit(
            query,
            "👥 *All Users & Projects*\n\nNo projects found.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data="admin:running")]]),
            parse_mode=ParseMode.MARKDOWN,
        )
        return
    ...
    kb_rows.append([InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data="admin:running")])
...
async def cb_admin_run_project(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not p:
        await safe_edit(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data="admin:all_projects:0")]]),
        )
        return
...
async def cb_admin_broadcast_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"{BTN_PRIMARY} Back",           callback_data="admin_panel")],
    ])
...
async def cb_envvars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb_rows.append([InlineKeyboardButton(f"{BTN_PRIMARY} Back", callback_data=f"proj:{name}")])
...
async def cb_env_add_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_DANGER} Cancel", callback_data=f"envvars:{name}")]]),
        parse_mode=ParseMode.MARKDOWN,
    )
    return ENV_ADD_KEY
...
async def _enforce_free_runtime_limit(p: dict) -> bool:
    """Auto-stop a free-tier user's project once it has run continuously for
    FREE_RUNTIME_LIMIT_SECONDS. Premium users and the owner are exempt and
    can run 24/7. Returns True if the project was stopped here.
    """
    uid, name = p["user_id"], p["name"]
    if uid == OWNER_ID:
        return False
    ...
    if elapsed < FREE_RUNTIME_LIMIT_SECONDS:
        return False

    stopped = await kill_project(uid, name)
    if stopped:
        logger.info(f"Free-tier runtime limit reached — stopped {uid}:{name} after {elapsed/3600:.1f}h")
        if notification_bot:
            try:
                await notification_bot.send_message(
                    chat_id=uid,
                    text=(
                        f"⏱ *Free Plan Runtime Limit Reached*\n\n"
                        f"Project `{name}` ran for {FREE_RUNTIME_LIMIT_HOURS} hours and has been "
                        f"auto-stopped (free plan limit).\n\n"
                        f"🚀 Upgrade to *Premium* to keep your projects running 24/7."
                    ),
                    parse_mode=ParseMode.MARKDOWN,
                )
            except Exception:
            ...
            for p in running:
                ...
                if proc is not None:
                    ...
                    if proc.returncode is None:
                        # Definitely still running — check free-tier runtime cap first.
                        await _enforce_free_runtime_limit(p)
                        continue
...
async def _install_requirements_for_project(uid: int, name: str) -> tuple:
    ...
    try:
        with open(req_path, "rb") as _f:
            cur_hash = _hashlib.md5(_f.read()).hexdigest()
        if os.path.exists(hash_cache_path):
            with open(hash_cache_path) as _f:
                saved_hash = _f.read().strip()
...
async def cb_github_set(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_DANGER} Cancel", callback_data=f"github:{name}")]]),
        parse_mode=ParseMode.MARKDOWN,
    )
    return GITHUB_URL
...
async def cb_cron_add_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_DANGER} Cancel", callback_data=f"cron:{name}")]]),
        parse_mode=ParseMode.MARKDOWN,
    )
    return CRON_EXPR
...
async def _live_log_task(uid: int, name: str, chat_id: int, message_id: int, bot):
    ...
    last_size = -1
    stop_kb   = InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_DANGER} Stop", callback_data=f"live_stop:{name}")]])
    try:
        while True:
...
async def cb_clone_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{BTN_DANGER} Cancel", callback_data=f"proj:{name}")]]),
        parse_mode=ParseMode.MARKDOWN,
    )
    return CLONE_NAME
...
async def post_init(app: Application):
    ...
    # PERF: ensure Mongo indexes exist so every find_one()/find() on these
    # hot paths (permission checks, project lookups, the 10s process monitor
    # scan) uses an index seek instead of a full collection scan. Idempotent —
    # safe to call on every startup.
    try:
        await users_col.create_index("user_id", unique=True)
        await projects_col.create_index([("user_id", 1), ("name", 1)], unique=True)
