...
async def _find_or_install_mvn(pdir: str = ".") -> str | None:
    ...
    # 4. Try auto-install via apt-get (Debian/Ubuntu servers)
    logger.info("mvn not found — attempting auto-install via apt-get")
    try:
        proc = await asyncio.wait_for(
        ...
        logger.warning(f"apt-get maven install failed: {e}")
