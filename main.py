...
async def cb_pay_upi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX: Clear stale user_data from any previous purchase flow (e.g. a
    # domain purchase attempt that didn't complete) so pay_upi_utr always
    # reads the correct buying_plan for *this* flow.
    context.user_data.pop("pay_screenshot_file_id", None)
    context.user_data.pop("domain_purchase_name", None)
    context.user_data.pop("domain_buy_project", None)
    context.user_data["buying_plan"] = plan_key
    settings = await get_bot_settings()
    upi_id = settings.get("upi_id", "")
    upi_qr = settings.get("upi_qr_file_id", "")
