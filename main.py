...
async def _find_or_install_mvn(pdir: str = ".") -> str | None:
    ...
    try:
        ...
        mvn_version = "3.9.9"
        mvn_url     = (
            f"https://downloads.apache.org/maven/maven-3/{mvn_version}/binaries/"
            f"apache-maven-{mvn_version}-bin.tar.gz"
        )
        tmp_tar = "/tmp/apache-maven.tar.gz"

        logger.info(f"Downloading Maven {mvn_version} from Apache...")
        await asyncio.get_event_loop().run_in_executor(
            None, lambda: urllib.request.urlretrieve(mvn_url, tmp_tar)
        )
        ...
        await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: _tarfile.open(tmp_tar, "r:gz").extractall(local_dir),
        )
