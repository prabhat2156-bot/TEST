...
import os, time, json, shutil, mimetypes, zipfile, io, secrets, stat, subprocess, threading
...
# Shared webhook secrets: {secret: {user_id, project_name, project_dir}}
webhook_secrets: dict = {}

# Shared domain map: {domain: {user_id, project_name, project_dir}}
domain_map: dict = {}

# Pending webhook deploys queue: [{user_id, project_name, project_dir}]
...
def safe_json_body() -> dict:
    """Parse the request JSON body without raising a raw 400/HTML error.
    FIX: request.get_json(force=True) still raises BadRequest on malformed/
    truncated bodies (e.g. flaky mobile connections). Routes used to call this
    unguarded outside their try/except, so a parse failure returned an HTML
    error page instead of JSON — the frontend then showed a generic
    "Save failed" with no real reason. silent=True + explicit try/except
    guarantees callers always get a dict back."""
    try:
...
def index():
    ...
    data = domain_map.get(host)
    if data:
        return serve_html_project(data["project_dir"], "")
...
# ─────────────────────────────────────────────────────────────
# 🌐 HTML Project Serving — /site/<uid>/<name>/
# ─────────────────────────────────────────────────────────────

def serve_html_project(pdir: str, subpath: str):
    if not subpath or subpath == "/":
        subpath = "index.html"
    subpath = subpath.lstrip("/")
    target = os.path.realpath(os.path.join(pdir, subpath))
    ...
    return send_file(target, mimetype=mime or "text/html")

def _get_project_custom_domain(uid: int, name: str) -> str:
    """Return custom domain for this project if one is configured."""
    pdir_check = os.path.join(os.path.dirname(__file__), "projects", str(uid), name)
    pdir_real  = os.path.realpath(pdir_check)
    for domain, info in domain_map.items():
        if os.path.realpath(info.get("project_dir", "")) == pdir_real:
            return domain
    return ""

@app.route("/site/<int:uid>/<name>/", defaults={"subpath": ""})
@app.route("/site/<int:uid>/<name>/<path:subpath>")
def html_site(uid, name, subpath):
    PROJECTS_ROOT = os.path.join(os.path.dirname(__file__), "projects")
    pdir = os.path.join(PROJECTS_ROOT, str(uid), name)
    if not os.path.isdir(pdir):
        abort(404)
    # FIX: If project has a custom domain, redirect /site/ URL to it.
    custom = _get_project_custom_domain(uid, name)
    if custom and request.host.split(":")[0] != custom:
        return Response("", status=301, headers={"Location": "https://" + custom + "/"})
    return serve_html_project(pdir, subpath)

@app.route("/<path:subpath>")
def domain_catch_all(subpath):
    host = request.host.split(":")[0]
    data = domain_map.get(host)
    if data and not subpath.startswith("fm/") and not subpath.startswith("wh/"):
        return serve_html_project(data["project_dir"], subpath)
    abort(404)

# ─────────────────────────────────────────────────────────────
# 🔗 GitHub Webhook — /wh/<secret>
# ─────────────────────────────────────────────────────────────

@app.route("/wh/<secret>", methods=["POST"])
def github_webhook(secret):
    # FIX: HMAC-SHA256 signature verification
    sig_header = request.headers.get("X-Hub-Signature-256", "")
    if sig_header:
        import hmac as _hmac, hashlib as _hashlib
        expected = "sha256=" + _hmac.new(
            secret.encode(), request.data, _hashlib.sha256
        ).hexdigest()
        if not _hmac.compare_digest(expected, sig_header):
            return jsonify({"error": "Invalid signature"}), 403
    data = webhook_secrets.get(secret)
    if not data:
        abort(404)
    pdir = data.get("project_dir", "")
    if not pdir or not os.path.isdir(pdir):
        return jsonify({"ok": False, "error": "project dir not found"}), 400
    ...
    def _do_deploy():
        try:
            ...
            pending_webhooks.append({
                ...
                "project_dir":  pdir,
                "pull_output":  result.stdout[:500] + result.stderr[:200],
            })
...
function detectMode(name){
  ...
  return ({py:"python",js:"javascript",ts:"javascript",jsx:"javascript",
    tsx:"javascript",json:"application/json",
