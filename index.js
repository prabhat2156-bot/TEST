/**
 * WhatsApp Group Creator Bot — v3.0 FREEZE-FIXED
 *
 * KEY FIXES:
 * 1. All Baileys calls in whatsapp-manager now wrapped in withTimeout() — no more freezes
 * 2. JID phone extraction: device suffix ":5" stripped correctly
 * 3. phoneMatch() helper: flexible country-code-aware number matching
 * 4. extractNumbers(): deduplicates + strips all non-digit chars
 * 5. parseVcf(): handles folded lines, multiple TEL formats, encoding
 * 6. removeAllMembers: batches of 3 with timeout — actually removes now
 * 7. Group creation: members promoted correctly
 */
...
// ─── VCF Parser — v3 FIXED ────────────────────────────────────────────────
// Handles: folded lines, ENCODING=QUOTED-PRINTABLE, multiple TEL fields,
// wsp/\r\n variants, any phone format (+91-9876-543210, (91)98765 43210...)
function parseVcf(content) {
  const contacts = [];
  // Step 1: Normalize line endings
  let text = content.replace(/\r\n/g, "\n").replace(/\r/g, "\n");
  // Step 2: Unfold lines (RFC 6350 — continuation lines start with space/tab)
  text = text.replace(/\n[ \t]/g, "");
  // Step 3: Split into vCards
  const blocks = text.split(/BEGIN:VCARD/i).slice(1);
  for (const block of blocks) {
    // Get display name — try FN first, then N
    const fnMatch = block.match(/^FN[^:]*:(.+)$/mi);
    const nMatch  = block.match(/^N[^:]*:([^;\r\n]+)/mi);
    const name = (fnMatch?.[1] ?? nMatch?.[1] ?? "").trim().replace(/\\/g, "").replace(/;/g, " ").trim() || "";

    // Extract all TEL lines (handles TEL;TYPE=... and plain TEL:)
    const telMatches = [...block.matchAll(/^TEL[^:]*:([^\r\n]+)/gim)];
    for (const m of telMatches) {
      // Strip ALL non-digit chars — handles +, spaces, dashes, dots, parens, encoding
      const raw = m[1].trim();
      // Remove quoted-printable encoding artifacts
      const decoded = raw.replace(/=[\dA-F]{2}/gi, "").replace(/=/g, "");
...
// ─── Extract plain phone numbers from text ────────────────────────────────
// Handles: 919876543210, +91-98765-43210, 91 98765 43210, etc.
...
async function sendMainMenu(ctx) {
  ...
  const text =
    `🤖 *WA Group Creator Bot v3.0 — FIXED*\n` +
    `━━━━━━━━━━━━━━━━━━━━━━━━\n` +
...
// ─── Similar Groups — FIXED: now searches ALL words, not just first ─────
bot.action(/^gs_similar_(.+)$/, async (ctx) => {
  ...
  try {
    ...
    if (!all.length) {
      await reply(ctx, "❌ Koi group nahi mila.", Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]));
      return;
    }

    // FIX: Build word map from ALL words in group name, not just first word
    // This allows matching "Test Group 1", "Test Group 2" etc. by "test" or "group"
    const wordMap = {};
    for (const g of all) {
      // Extract all unique meaningful words (length >= 2)
      const words = [...new Set(
        g.name.toLowerCase().split(/[\s\-_\.]+/).filter((w) => w.length >= 2)
      )];
      for (const word of words) {
        if (!wordMap[word]) wordMap[word] = new Set();
        wordMap[word].add(g.id);
      }
    }

    // Convert sets to arrays and sort by count descending
    const wordMapArr = {};
    for (const [w, set] of Object.entries(wordMap)) wordMapArr[w] = [...set];
    const entries = Object.entries(wordMapArr)
      .sort((a, b) => b[1].length - a[1].length)
      .filter(([, ids]) => ids.length > 1); // Only show words matching 2+ groups

    updateSession(ctx.from.id, {
      featureFlow: { ...getSession(ctx.from.id).featureFlow, feature, allGroups: all, wordGroups: wordMapArr, step: "similar_pick" },
    });
    ...
    await reply(ctx,
      `🔍 *Similar Groups*\n━━━━━━━━━━━━━━━━━━━━\nTotal groups: *${all.length}*\n\n*Auto-detected common words:*\n_Ek word tap karo — us word wale sab groups select honge_`,
...
bot.action("es_apply", async (ctx) => {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      if (result.changes.length) { changed++; details.push(`✅ *${g.name}*: ${result.changes.map(c => c.replace(/^[^:]+:/, "")).join(", ")}`); }
      else { alreadyOk++; details.push(`⏭ *${g.name}*: pehle se same`); }
  ...
  if (feature === "remove_members") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try { await bot.telegram.editMessageText(ctx.chat.id, pm.message_id, undefined,
        `🧹 *Removing...*\n━━━━━━━━━━━━━━━━━━━━\n✅ Done: ${done}/${total}\n⚙️ ${g.name}\n${bar(i, total)}`,
        { parse_mode: "Markdown" }); } catch {}
      try { const n = await removeAllMembers(0, g.id); totalRem += n; done++; details.push(`✅ ${g.name}: ${n} hataye`); }
      catch (err) { failed++; details.push(`❌ ${g.name}: ${err.message}`); }
      await sleep(2000);
    }
  ...
  if (feature === "make_admin") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        totalProm += n; done++;
        details.push(n > 0 ? `✅ ${g.name}: ${n} promoted` : `⚠️ ${g.name}: 0 mila (member nahi ya pending nahi)`);
      } catch (err) { failed++; details.push(`❌ ${g.name}: ${err.message}`); }
      await sleep(1500);
    }
  ...
  if (feature === "demote_admin") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        totalDem += n; done++;
        details.push(n > 0 ? `✅ ${g.name}: ${n} demoted` : `⚠️ ${g.name}: 0 mila (admin nahi)`);
      } catch (err) { failed++; details.push(`❌ ${g.name}: ${err.message}`); }
      await sleep(1200);
    }
...
bot.action("ge_desc",        async (ctx) => { await ctx.answerCbQuery(); updateSession(ctx.from.id,{groupFlow:{...getSession(ctx.from.id).groupFlow,step:"description_edit"}});  await reply(ctx,`📄 *Naya description:*`,{parse_mode:"Markdown",...Markup.inlineKeyboard([[Markup.button.callback("⏭ Clear","ge_desc_clear")],[Markup.button.callback("🔙 Cancel","gf_back_confirm")]])}); });
bot.action("ge_desc_clear",  async (ctx) => { await ctx.answerCbQuery(); updateSession(ctx.from.id,{groupFlow:{...getSession(ctx.from.id).groupFlow,description:"",step:"confirm"}}); await showConfirm(ctx); });
bot.action("ge_photo",       async (ctx) => { await ctx.answerCbQuery(); updateSession(ctx.from.id,{groupFlow:{...getSession(ctx.from.id).groupFlow,step:"photo_edit"}});         await reply(ctx,`🖼️ *Naya photo bhejo:*`,{parse_mode:"Markdown",...Markup.inlineKeyboard([[Markup.button.callback("⏭ Remove Photo","ge_photo_clear")],[Markup.button.callback("🔙 Cancel","gf_back_confirm")]])}); });
bot.action("ge_photo_clear", async (ctx) => { await ctx.answerCbQuery(); updateSession(ctx.from.id,{groupFlow:{...getSession(ctx.from.id).groupFlow,photo:null,step:"confirm"}}); await showConfirm(ctx); });
bot.action("ge_disappearing",async (ctx) => { await ctx.answerCbQuery(); const s=getSession(ctx.from.id); updateSession(ctx.from.id,{groupFlow:{...s.groupFlow,step:"disappearing_edit"}}); await askDisappearing(ctx); });
bot.action("ge_members",     async (ctx) => { await ctx.answerCbQuery(); updateSession(ctx.from.id,{groupFlow:{...getSession(ctx.from.id).groupFlow,step:"members_edit"}});      await reply(ctx,`👥 *Naye members (ek line mein ek):*`,{parse_mode:"Markdown",...Markup.inlineKeyboard([[Markup.button.callback("🔙 Cancel","gf_back_confirm")]])}); });
...
bot.action("gf_create_now", async (ctx) => {
  ...
  const pm = await ctx.reply(
    `➕ *Creating ${total} group(s)...*\n━━━━━━━━━━━━━━━━━━━━\n${bar(0, total)}`,
    { parse_mode: "Markdown" }
  );
  await showCancelBtn(ctx);

  let created = 0, failed = 0, cancelled = false;
  const details = [], links = [];
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      // Create group — Baileys requires at least 1 participant JID
      const firstMember = flow.members.length > 0
        ? `${cleanPhone(flow.members[0])}@s.whatsapp.net`
        : `0000000000@s.whatsapp.net`;
      const result = await createGroup(0, name, [firstMember]);
      const gid = result.id;

      if (flow.description) await updateGroupDescription(0, gid, flow.description).catch(() => {});
      if (flow.photo)       await updateGroupPhoto(0, gid, flow.photo).catch(() => {});
      if (flow.disappearing) await setDisappearingMessages(0, gid, flow.disappearing).catch(() => {});
      await setGroupPermissions(0, gid, flow.permissions).catch(() => {});

      // Add remaining members (if more than 1)
      if (flow.members.length > 1) {
        const extraJids = flow.members.slice(1).map((n) => `${cleanPhone(n)}@s.whatsapp.net`);
        await promoteToAdmin(0, gid, []).catch(() => {}); // noop but ensures socket alive
        for (let mi = 0; mi < extraJids.length; mi += 5) {
          await addMembersToGroup(0, gid, flow.members.slice(mi + 1, mi + 6)).catch(() => {});
          await sleep(1500);
        }
      }

      // Promote all members to admin if requested
      if (flow.makeAdmin && flow.members.length > 0) {
        const allJids = flow.members.map((n) => `${cleanPhone(n)}@s.whatsapp.net`);
        await promoteToAdmin(0, gid, allJids).catch(() => {});
      }

      const link = await getGroupInviteLink(0, gid).catch(() => null);
      if (link) links.push({ name, link });
      created++;
      details.push(`✅ *${name}*${link ? `\n   ${link}` : ""}`);
    } catch (err) {
      failed++;
  ...
  await removeCancelBtn(ctx);
  try { await bot.telegram.editMessageText(ctx.chat.id, pm.message_id, undefined,
    `✅ *Done! ${created}/${total} groups bane*`, { parse_mode: "Markdown" }); } catch {}

  const extraLines = [`➕ *Created: ${created}*`, ...details.slice(0, 20)];
  if (details.length > 20) extraLines.push(`_...aur ${details.length - 20} more_`);
  await sendSummary(ctx, { feature: "create_groups", total, success: created, failed, cancelled, extra: extraLines });
...
bot.action("am_mode_onebyone", async (ctx) => {
  ...
  updateSession(ctx.from.id, { featureFlow: { ...flow, addMode: "onebyone", step: "am_awaiting_vcf" },
    awaitingVcf: { feature: "add_members", step: "am_vcf" } });
  await ctx.reply(
    `📎 *VCF 1/${flow.links.length} bhejo:*`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) }
  );
});
...
async function askNextVcf(ctx) {
  ...
  updateSession(ctx.from.id, { awaitingVcf: { feature: "add_members", step: "am_vcf" }, featureFlow: { ...flow, step: "am_awaiting_vcf" } });
  await ctx.reply(
    `📎 *VCF ${idx + 1}/${flow.links.length} bhejo:*`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([
      [Markup.button.callback("⏭ Skip this group", "am_vcf_skip")],
      [Markup.button.callback("🏠 Main Menu", "back_menu")],
    ]) }
  );
}

bot.action("am_vcf_skip", async (ctx) => {
...
async function runAddMembersFromVcfs(ctx) {
  ...
  const pm = await ctx.reply(`➕ *Adding members — ${total} groups...*\n━━━━━━━━━━━━━━━━━━━━\n${bar(0, total)}`, { parse_mode: "Markdown" });
  await showCancelBtn(ctx);
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (!contacts || !contacts.length) {
      summaryLines.push(`⏭ Group ${i + 1}: VCF skip tha`); continue;
    }
    try {
      await bot.telegram.editMessageText(ctx.chat.id, pm.message_id, undefined,
        `➕ *Adding...*\n━━━━━━━━━━━━━━━━━━━━\n✅ Done: ${doneGroups}/${total}\n⚙️ Group ${i + 1}/${total}\n${bar(i, total)}`,
        { parse_mode: "Markdown" });
    } catch {}
    try {
...
bot.on("text", async (ctx) => {
  ...
  // Make Admin numbers — FIXED: uses extractNumbers() for robust parsing
  if (s.featureFlow?.step === "admin_numbers") {
    const nums = extractNumbers(text);
    if (!nums.length) { await ctx.reply("⚠️ Koi valid number nahi mila.\n_Example: 919876543210 ya +91-9876543210_", {parse_mode:"Markdown"}); return; }
  ...
  // Demote Admin numbers — FIXED: uses extractNumbers()
  if (s.featureFlow?.step === "demote_numbers") {
    const nums = extractNumbers(text);
    if (!nums.length) { await ctx.reply("⚠️ Koi valid number nahi mila.", {parse_mode:"Markdown"}); return; }
    const flow = s.featureFlow;
    updateSession(uid, { featureFlow: { ...flow, adminNumbers: nums, step: "executing" } });
    await ctx.reply(`✅ *${nums.length} numbers mile. Processing...*`, { parse_mode: "Markdown" });
  ...
  if (flow.step === "description" || flow.step === "description_edit") {
    updateSession(uid,{groupFlow:{...flow,description:text.slice(0,512),step: flow.step==="description_edit"?"confirm":"photo"}});
    if (flow.step==="description_edit") await showConfirm(ctx); else await askPhoto(ctx);
    return;
  }
  if (flow.step === "members" || flow.step === "members_edit") {
    // FIX: Use extractNumbers() for robust number parsing
    const nums = extractNumbers(text);
...
async function runChangeNameAsVcf(ctx) {
  ...
  for (let i=0; i<total; i++) {
    ...
    try {
      ...
      // FIX: Use jidToPhone() which correctly strips device suffix :5
      const memberPhones = (info.participants || []).map(p => jidToPhone(p.id));
      const pending = await getPendingForGroup(0, info.id);
      const pendingPhones = pending.map(p => p.phone);
      // FIX: Use phoneMatch() for flexible country-code-aware matching
      const matches = vcfPhones.some(vcfPh =>
        memberPhones.some(mp => phoneMatch(mp, vcfPh)) ||
        pendingPhones.some(pp => phoneMatch(pp, vcfPh))
      );
      if (matches) {
...
app.get("/", (_, res) => res.send(`<html><body style="font-family:sans-serif;text-align:center;padding:50px;background:#111;color:#fff"><h2>✅ WA Group Creator Bot v3.0</h2><p style="color:#4ade80">Running 🟢</p><p>WA: ${getConnectedCount()>0?"Connected ✅":"Disconnected ❌"}</p><small style="color:#888">v3 Fixes: freeze fix (timeouts), number delete, JID phone detect, VCF parser</small></body></html>`));
app.get("/health", (_, res) => res.json({ status:"ok", version:"3.0", whatsapp:getStatus(0), phone:getPhone(0)||null, ts:new Date().toISOString() }));
...
async function main() {
  ...
  console.log(`WA Group Creator Bot v3.0 FREEZE-FIXED running! Owner: ${OWNER_ID||"NOT SET"}`);
}
