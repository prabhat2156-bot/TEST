...
async function runChangeNameAsVcfAuto(ctx, vcfList) {
  ...
  for (let i = 0; i < totalGroups; i++) {
    ...
    try {
      // ── Collect phone numbers: members + pending requests ──────────────
      // Members (LID-aware extraction)
      const allPhones = new Set(
        (g.participants || [])
          .map((p) => extractParticipantPhone(p))
          .filter((n) => n && n.length >= 7)
      );
      // Pending requests (real phones only, LID-safe)
      try {
      ...
      // ── Find best-matching VCF (most contacts found in group) ──────────
      let bestVcf   = null;
      let bestCount = 0;
