...
const {
  ...
  getGroupStats,
  touchAcc,
  getTotalTracked, getMemStats, ensureConnected,
...
const scheduledTasks = new Map(); // uid -> [{ id, feat, groupCount, fireAt, timeLabel, handle }]

function getScheduledList(uid)    { return scheduledTasks.get(uid) || []; }
function addScheduledTask(uid, t) { const l = getScheduledList(uid); l.push(t); scheduledTasks.set(uid, l); }
function removeScheduledTask(uid, id) { scheduledTasks.set(uid, getScheduledList(uid).filter(t => t.id !== id)); }
...
setCallbacks({
  onPairingCode:  async (uid, code) => { const cb = pendingPairingCbs.get(String(uid)); if (cb) { pendingPairingCbs.delete(String(uid)); await cb(code); } },
  onReady:        async (uid)       => { const cb = pendingReadyCbs.get(String(uid));   if (cb) { pendingReadyCbs.delete(String(uid));   await cb();    } },
  onDisconnected: async ()         => {},
});

// ─── Periodic in-memory session cleanup ────────────────────────────────────
// Clears featureFlow/groupFlow for sessions idle > 30 min to save RAM.
const { sessions: _sessionsMap } = require("./src/session");
setInterval(() => {
  const now = Date.now();
  let cleared = 0;
  for (const [uid, s] of _sessionsMap) {
    if (!s) continue;
    if ((s.featureFlow || s.groupFlow || s.joinFlow) && now - (s._lastActive || 0) > 30 * 60 * 1000) {
      _sessionsMap.set(uid, { ...s, featureFlow: null, groupFlow: null, joinFlow: null, cancelPending: false });
      cleared++;
    }
  }
  if (cleared > 0) console.log(`[SessionClean] Cleared ${cleared} idle flow sessions`);
}, 15 * 60 * 1000);
...
bot.action("cancel_exec", async (ctx) => {
  ...
  updateSession(String(ctx.from.id), { cancelPending: true });
...
// makeFakeCtx — creates a ctx-like object for use inside setTimeout callbacks
// where the original Telegraf ctx is stale. Uses bot.telegram directly.
function makeFakeCtx(uid, chatId) {
  return {
    from:    { id: parseInt(uid) || uid, first_name: "User" },
    chat:    { id: chatId },
...
function buildMainMenu(uid) {
  ...
  return Markup.inlineKeyboard([
    ...
    [b("📅 Schedule Task",  "feat_schedule"),       b("📊 Group Stats",      "feat_groupstats")],
    [Markup.button.callback("📋 My Schedules", "sch_view"), Markup.button.callback("📊 Status", "menu_status")],
  ]);
}

async function sendMainMenu(ctx) {
  const uid = String(ctx.from?.id || "0");
  const c = getStatus(uid) === "connected", p = getPhone(uid);
...
bot.action("menu_status", async (ctx) => {
  await ctx.answerCbQuery();
  const uid = String(ctx.from.id);
  const s = getStatus(uid), p = getPhone(uid);
...
bot.action("logout_wa", async (ctx) => {
  ...
  await disconnectAccount(String(ctx.from.id)); await sleep(800); await sendMainMenu(ctx);
});
bot.action("reset_wa", async (ctx) => {
  await ctx.answerCbQuery("Resetting...");
  await disconnectAccount(String(ctx.from.id));
  updateSession(ctx.from.id, { awaitingPhoneForIndex: true });
  await editOrReply(ctx,
...
const FEAT_MAP = {
  ...
  autoaccept: "auto_accept", groupstats: "group_stats",
};

Object.keys(FEAT_MAP).forEach((key) => {
  bot.action(`feat_${key}`, async (ctx) => {
    await ctx.answerCbQuery();
    if (getStatus(String(ctx.from.id)) !== "connected") { await ctx.answerCbQuery("⚠️ WhatsApp not connected!", { show_alert: true }); return; }
...
async function runChangeNameAsVcfAuto(ctx, vcfList) {
  ...
  let allGroups;
  try {
    allGroups = await withRetry(() => getAllGroupsWithDetails(uid));
  } catch (err) {
    ...
    try {
      ...
      try {
        const { jids: pendingJids, phones: pendingPhones } =
          await withTimeout(withRetry(() => getPendingRawJids(uid, g.id), 2, 1500), 10000, "PendingJids");
...
bot.action("feat_schedule", async (ctx) => {
  await ctx.answerCbQuery();
  if (getStatus(String(ctx.from.id)) !== "connected") { await ctx.answerCbQuery("⚠️ WhatsApp not connected!", { show_alert: true }); return; }
  updateSession(ctx.from.id, { featureFlow: { feature: "schedule", step: "sch_pick_feature" }, cancelPending: false });
  await reply(ctx,
    `📅 *Schedule Task*\n━━━━━━━━━━━━━━━━━━━━\n*Konsa feature schedule karna hai?*`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([
      [Markup.button.callback("✅ Approve Pending", "sch_feat_approve_pending"), Markup.button.callback("🚪 Leave Groups", "sch_feat_leave")],
      [Markup.button.callback("🔗 Get Links",       "sch_feat_get_links"),       Markup.button.callback("🔄 Reset Link",   "sch_feat_reset_link")],
      [Markup.button.callback("🔀 Approval Toggle", "sch_feat_approval")],
      [Markup.button.callback("🏠 Main Menu", "back_menu")],
    ]) }
  );
});

Object.keys(SCH_FEATS).forEach((feat) => {
  bot.action(`sch_feat_${feat}`, async (ctx) => {
    await ctx.answerCbQuery();
    const uid = String(ctx.from.id);
    updateSession(uid, { featureFlow: { feature: "schedule", step: "sch_pick_time", schedFeat: feat, allGroups: [], selectedIds: [] } });
    await reply(ctx,
      `📅 *Schedule: ${SCH_FEATS[feat]}*\n━━━━━━━━━━━━━━━━━━━━\n*Kitne minute baad run karna hai?*`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([
        [Markup.button.callback("5 min",  "sch_time_5"),   Markup.button.callback("10 min", "sch_time_10"),  Markup.button.callback("15 min", "sch_time_15")],
        [Markup.button.callback("30 min", "sch_time_30"),  Markup.button.callback("1 hour", "sch_time_60"),  Markup.button.callback("2 hrs",  "sch_time_120")],
        [Markup.button.callback("✏️ Custom (minutes)", "sch_time_custom")],
        [Markup.button.callback("🏠 Main Menu", "back_menu")],
      ]) }
    );
  });
...
bot.action(/^sch_time_(\d+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  const uid = String(ctx.from.id);
  const mins = parseInt(ctx.match[1]);
  const flow = getSession(uid).featureFlow;
  updateSession(uid, { featureFlow: { ...flow, step: "sch_pick_groups", schedMins: mins } });
  await showSchGroupSelect(ctx, flow.schedFeat, mins);
});

bot.action("sch_time_custom", async (ctx) => {
  await ctx.answerCbQuery();
  const uid = String(ctx.from.id);
  updateSession(uid, { featureFlow: { ...getSession(uid).featureFlow, step: "sch_custom_time" } });
  await reply(ctx, `📅 *Custom Duration*\n━━━━━━━━━━━━━━━━━━━━\nKitne minute baad? Sirf number likhein:\n_Example:_ \`45\``,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) });
});

async function showSchGroupSelect(ctx, feat, mins) {
  const timeLabel = mins >= 60 ? `${mins / 60} hour` : `${mins} min`;
  await reply(ctx,
    `📅 *Schedule: ${SCH_FEATS[feat]}*\n_Fire in: *${timeLabel}*_\n━━━━━━━━━━━━━━━━━━━━\n*Groups select karo:*`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([
      [Markup.button.callback("📋 All Groups",     "sch_grp_all")],
      [Markup.button.callback("🔍 Similar Groups", "sch_grp_similar")],
      [Markup.button.callback("🏠 Main Menu", "back_menu")],
    ]) }
  );
}

bot.action("sch_grp_all", async (ctx) => {
  await ctx.answerCbQuery("Loading groups...");
  const uid = String(ctx.from.id);
  const flow = getSession(uid).featureFlow;
  const allGroups = await getAllGroupsWithDetails(uid).catch(() => []);
  if (!allGroups.length) { await ctx.reply("❌ No groups found."); return; }
  const selectedIds = allGroups.map((g) => g.id);
  updateSession(uid, { featureFlow: { ...flow, step: "sch_confirm", allGroups, selectedIds } });
  await showSchConfirm(ctx);
});

bot.action("sch_grp_similar", async (ctx) => {
  await ctx.answerCbQuery();
  updateSession(ctx.from.id, { featureFlow: { ...getSession(ctx.from.id).featureFlow, step: "sch_similar_kw" } });
  await reply(ctx, `📅 *Similar Groups*\n━━━━━━━━━━━━━━━━━━━━\nGroup name ka keyword bhejo:\n_Example:_ \`GOD MADARA\``,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) });
});

async function showSchConfirm(ctx) {
  const uid = String(ctx.from.id);
  const flow = getSession(uid).featureFlow;
  const mins = flow.schedMins;
  const timeLabel = mins >= 60 ? `${mins / 60} hour` : `${mins} min`;
  const fireAt = new Date(Date.now() + mins * 60000).toLocaleTimeString();
  await reply(ctx,
    `📅 *Schedule Task — Confirm*\n━━━━━━━━━━━━━━━━━━━━\n` +
    `Feature  : *${SCH_FEATS[flow.schedFeat]}*\n` +
    `Groups   : *${flow.selectedIds.length}*\n` +
    `Delay    : *${timeLabel}*\n` +
    `Fire at  : *${fireAt}*\n` +
    `━━━━━━━━━━━━━━━━━━━━`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([
      [Markup.button.callback("✅ Confirm & Schedule", "sch_confirm_go")],
      [Markup.button.callback("❌ Cancel", "back_menu")],
    ]) }
  );
}

bot.action("sch_confirm_go", async (ctx) => {
  await ctx.answerCbQuery("Scheduled! ✅");
  const uid    = String(ctx.from.id);
  const chatId = ctx.chat.id;
  const flow   = getSession(uid).featureFlow;
  const { schedFeat: feat, schedMins: mins, allGroups, selectedIds } = flow;
  const timeLabel = mins >= 60 ? `${mins / 60}h` : `${mins}min`;
  const fireAt    = new Date(Date.now() + mins * 60000).toLocaleTimeString();
  const taskId    = Date.now().toString();

  const handle = setTimeout(async () => {
    removeScheduledTask(uid, taskId);
    try {
      await bot.telegram.sendMessage(chatId,
        `⏰ *Scheduled Task Starting!*\n━━━━━━━━━━━━━━━━━━━━\nFeature : *${SCH_FEATS[feat]}*\nGroups  : *${selectedIds.length}*`,
        { parse_mode: "Markdown" }
      );
      // ── Auto-reconnect WA if idle-disconnected ──────────────────────────
      if (getStatus(uid) !== "connected") {
        await bot.telegram.sendMessage(chatId,
          `🔄 *WhatsApp offline — reconnecting automatically...*`, { parse_mode: "Markdown" });
        const ok = await ensureConnected(uid);
        if (!ok) {
          await bot.telegram.sendMessage(chatId,
            `❌ *WhatsApp reconnect failed.*\nPlease /start the bot and connect your WhatsApp again, then re-schedule the task.`);
          return;
        }
        await bot.telegram.sendMessage(chatId,
          `✅ *WhatsApp reconnected! Starting task now...*`, { parse_mode: "Markdown" });
      }
      // ── Use fakeCtx so stale original ctx is never touched ──────────────
      const fakeCtx = makeFakeCtx(uid, chatId);
      updateSession(uid, { featureFlow: defaultFeatureFlow(feat), cancelPending: false });
      await runFeature(fakeCtx, feat, selectedIds, allGroups, []);
    } catch (err) {
      await bot.telegram.sendMessage(chatId, `❌ Scheduled task failed: ${err.message}`).catch(() => {});
    }
  }, mins * 60000);

  addScheduledTask(uid, { id: taskId, feat, groupCount: selectedIds.length, fireAt, timeLabel, handle });
  updateSession(uid, { featureFlow: null });
  ...
  await reply(ctx,
    ...
    `Fire at : *${fireAt}*\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `_Bot automatically run karega. /menu se kuch aur kar sakte ho._`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([
      [Markup.button.callback("📋 My Schedules", "sch_view")],
      [Markup.button.callback("🏠 Main Menu", "back_menu")],
    ]) }
  );
});

bot.action("sch_view", async (ctx) => {
  await ctx.answerCbQuery();
  const uid   = String(ctx.from.id);
  const tasks = getScheduledList(uid);
  if (!tasks.length) {
    await reply(ctx,
      `📋 *My Schedules*\n━━━━━━━━━━━━━━━━━━━━\n_Koi scheduled task nahi hai._`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) }
    );
    return;
  }
  const lines = tasks.map((t, i) =>
    `${i + 1}. *${SCH_FEATS[t.feat]}*\n   Groups: ${t.groupCount} | Fire: ${t.fireAt} | In: ${t.timeLabel}`
  ).join("\n\n");
  await reply(ctx,
    `📋 *My Schedules*\n━━━━━━━━━━━━━━━━━━━━\n${lines}\n━━━━━━━━━━━━━━━━━━━━`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([
      [Markup.button.callback("🗑️ Cancel All Tasks", "sch_cancel_all")],
      [Markup.button.callback("🏠 Main Menu", "back_menu")],
    ]) }
  );
});

bot.action("sch_cancel_all", async (ctx) => {
  await ctx.answerCbQuery("All cancelled.");
  const uid = String(ctx.from.id);
  getScheduledList(uid).forEach((t) => clearTimeout(t.handle));
  scheduledTasks.set(uid, []);
  await reply(ctx,
    `🗑️ *All scheduled tasks cancelled.*`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) }
  );
});
...
async function runFeature(ctx, feature, selectedIds, allGroups, extraNums) {
  ...
  if (feature === "leave") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🚪 Leaving groups...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        await withRetry(() => leaveGroup(uid, g.id));
        done++;
      } catch (err) { failed++; }
      await sleep(D.leave);
    }
  ...
  if (feature === "remove_members") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        const n = await withRetry(() => removeAllMembers(uid, g.id, 1, true));
  ...
  if (feature === "make_admin") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `👑 Making admin...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const n = await makeAdminByNumbers(uid, g.id, extraNums);
        totalProm += n; done++;
  ...
  if (feature === "demote_admin") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `⬇️ Demoting admins...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const n = await demoteAdminInGroup(uid, g.id, extraNums);
        totalDem += n; done++;
  ...
  if (feature === "approval") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🔀 Toggling approval...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const cur = await withRetry(() => getGroupApprovalStatus(uid, g.id)), next = !cur;
        await withRetry(() => setGroupApproval(uid, g.id, next));
        done++;
      } catch (err) { failed++; }
      await sleep(D.approvalToggle);
    }
  ...
  if (feature === "approve_pending") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `✅ Approving pending...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const r = await withRetry(() => approveAllPending(uid, g.id), 2, 5000);
  ...
  if (feature === "member_list") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `📋 Member list...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const members = await withRetry(() => getGroupMembers(uid, g.id));
        grandTotal += members.length;
        boxLines.push(`${g.name}: ${members.length} member`);
        done++;
      } catch { failed++; boxLines.push(`${g.name}: failed`); }
      await sleep(D.memberList);
    }
    boxLines.push(`──────────────────────`);
    boxLines.push(`Total: ${grandTotal} members (${done} groups)`);
  ...
  if (feature === "pending_list") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `⏳ Pending list...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const { list: pending } = await withRetry(() => getGroupPendingRequests(uid, g.id));
        grandPending += pending.length;
        boxLines.push(`${g.name}: ${pending.length} pending request`);
        done++;
      } catch { failed++; boxLines.push(`${g.name}: failed`); }
      await sleep(D.pendingList);
    }
    boxLines.push(`──────────────────────`);
    boxLines.push(`Total: ${grandPending} pending request (${done} groups)`);
  ...
  // ── GROUP STATS ───────────────────────────────────────────────────────
  if (feature === "group_stats") {
    const pm = await startProgress(ctx, uid, `📊 Fetching stats — ${total} group(s)...\n${bar(0, total)}`);
    let done = 0, failed = 0, grandMembers = 0, grandPending = 0, cancelled = false;
    const boxLines = [];
    for (let i = 0; i < total; i++) {
      if (isCancelled(uid)) { cancelled = true; break; }
      const g = sel[i];
      await editProgress(ctx.chat.id, pm.message_id,
        `📊 Group stats...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const stats = await withRetry(() => getGroupStats(uid, g.id));
        grandMembers += stats.membersCount;
        grandPending += stats.pendingCount;
        const approvalIcon = stats.approval ? "✅" : "❌";
        boxLines.push(`${g.name}`);
        boxLines.push(`  Members: ${stats.membersCount} | Pending: ${stats.pendingCount} | Approval: ${approvalIcon}`);
        done++;
      } catch { failed++; boxLines.push(`${g.name}: failed`); }
      await sleep(D.memberList);
    }
    boxLines.push(`──────────────────────`);
    boxLines.push(`Total Members : ${grandMembers}`);
    boxLines.push(`Total Pending : ${grandPending}`);
    boxLines.push(`Groups Done   : ${done}/${total}`);
    await sendSummary(ctx, { feature: "group_stats", total, success: done, failed, cancelled,
      extra: [`Total Groups  : ${total}`, `Total Members : ${grandMembers}`, `Total Pending : ${grandPending}`],
      boxLines });
    updateSession(uid, { featureFlow: null }); return;
  }
}
...
bot.action("gf_create_now",async(ctx)=>{
  ...
  for(let i=0;i<flow.count;i++){
    ...
    try{
      const r=await withRetry(()=>createGroup(uid,gname,jids));const gid=r.id;
      await sleep(3000); // give WA time to register the new group
      if(flow.description){await updateGroupDescription(uid,gid,flow.description).catch(()=>{});await sleep(600);}
      if(flow.photo){
        ...
        await withRetry(()=>updateGroupPhoto(uid,gid,photoBuf),3,3000).catch(()=>{});
        await sleep(800);
      }
      if(flow.disappearing){await setDisappearingMessages(uid,gid,flow.disappearing).catch(()=>{});await sleep(500);}
      if(flow.makeAdmin&&jids.length){await makeAdminByNumbers(uid,gid,flow.members).catch(()=>{});await sleep(1000);}
      await setGroupPermissions(0,gid,flow.permissions).catch(()=>{});
      let link="";try{link=await getGroupInviteLink(uid,gid);}catch{link="(unavailable)";}
...
bot.on("text", async (ctx) => {
  const uid = String(ctx.from.id), s = getSession(uid), text = ctx.message.text.trim();
  if (text.startsWith("/")) return;

  // WA phone input
  if (s.awaitingPhoneForIndex) {
    ...
    connectAccount(uid, phone).catch(async (err) => {
      pendingPairingCbs.delete(uid); pendingReadyCbs.delete(uid);
      await ctx.reply(`❌ Error: \`${err.message}\``,{parse_mode:"Markdown",...Markup.inlineKeyboard([[Markup.button.callback("🔄 Try Again","menu_account")],[Markup.button.callback("🏠 Main Menu","back_menu")]])});
    });
    return;
  }
  ...
  // Schedule Task: custom time input
  if (s.featureFlow?.step === "sch_custom_time") {
    const mins = parseInt(text, 10);
    if (isNaN(mins) || mins < 1) { await ctx.reply("⚠️ Sirf number likhein. Example: `45`", { parse_mode: "Markdown" }); return; }
    const flow = s.featureFlow;
    updateSession(uid, { featureFlow: { ...flow, step: "sch_pick_groups", schedMins: mins } });
    await showSchGroupSelect(ctx, flow.schedFeat, mins);
    return;
  ...
  // Schedule Task: similar groups keyword
  if (s.featureFlow?.step === "sch_similar_kw") {
    const kw = text.toLowerCase().trim();
    const flow = s.featureFlow;
    const allGroups = await getAllGroupsWithDetails(String(uid)).catch(() => []);
    const matched = allGroups.filter((g) => g.name.toLowerCase().includes(kw));
    if (!matched.length) { await ctx.reply(`❌ Koi group nahi mila keyword: *${text}*`, { parse_mode: "Markdown" }); return; }
    const selectedIds = matched.map((g) => g.id);
    updateSession(uid, { featureFlow: { ...flow, step: "sch_confirm", allGroups, selectedIds } });
    await ctx.reply(`✅ *${matched.length} group(s) match:* \`${matched.map(g=>g.name).join(", ")}\``, { parse_mode: "Markdown" });
    await showSchConfirm(ctx);
    return;
  }
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      const resolved = vcfPhones.length
        ? await withTimeout(resolveVcfPhones(uid, vcfPhones), 20000, "ResolveVcf")
        : [];
...
app.get("/", (_, res) => {
  const m = getMemStats();
  res.send(
    `<html><head><meta charset="utf-8"><title>WA Bot</title>
    <style>body{font-family:sans-serif;text-align:center;padding:50px;background:#0d1117;color:#e6edf3}
    h2{color:#58a6ff}.green{color:#3fb950}.yellow{color:#d29922}.red{color:#f85149}
    .card{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:20px;display:inline-block;min-width:220px;margin:8px}
    .label{color:#8b949e;font-size:13px}.val{font-size:22px;font-weight:bold;margin:4px 0}</style>
    </head><body>
    <h2>🤖 WA Group Manager Bot</h2>
    <p class="green">● Running 24/7</p>
    <div class="card"><div class="label">Connected WA Users</div><div class="val">${m.connected}</div></div>
    <div class="card"><div class="label">RAM Used</div><div class="val ${m.rss>450?"red":m.rss>350?"yellow":"green"}">${m.rss} MB</div></div>
    <div class="card"><div class="label">Tracked Sessions</div><div class="val">${m.accounts}</div></div>
    <p style="color:#8b949e;font-size:12px">Last ping: ${new Date().toISOString()}</p>
    </body></html>`
  );
});
...
app.get("/health", (_, res) => {
  const m = getMemStats();
  res.json({ status: "ok", ...m, ts: new Date().toISOString() });
});

app.listen(PORT, () => console.log(`[HTTP] Health server on port ${PORT}`));

// ─── Self-ping every 2 minutes to keep Render free plan alive ─────────────
function selfPing() {
  ...
  const req  = mod.get(`${full}/health`, { timeout: 10000 }, (r) => {
    console.log(`[Ping] ✅ ${r.statusCode} — WA:${getConnectedCount()} RAM:${getMemStats().rss}MB Q:${getQueueSize()}`);
    r.resume();
  });
  req.on("error", (e) => console.error("[Ping] ❌", e.message));
  req.end();
}
// First ping after 30s, then every 2 minutes
setTimeout(() => { selfPing(); setInterval(selfPing, 2 * 60 * 1000); }, 30 * 1000);

async function main() {
  await connectDB();
  await reconnectSavedAccounts();
  await bot.launch({ dropPendingUpdates: true });
  console.log(`WA Group Manager Bot running — PUBLIC mode (multi-user)`);
}
