...
bot.action("gf_create_now", async (ctx) => {
  ...
  for (let i=0;i<flow.count;i++) {
    ...
    try {
      ...
      const gid=r.id;
      await sleep(4000); // wait for group to be fully ready on WhatsApp servers

      if (flow.description)  { await updateGroupDescription(0,gid,flow.description).catch(()=>{});  await sleep(800); }

      // Photo with retry — first groups need extra time before photo can be set
      if (flow.photo) {
        let photoSet = false;
        for (let attempt = 0; attempt < 3 && !photoSet; attempt++) {
          try { await updateGroupPhoto(0,gid,flow.photo); photoSet = true; }
          catch { await sleep(3000); } // wait longer before retry
        }
        await sleep(800);
      }

      if (flow.disappearing) { await setDisappearingMessages(0,gid,flow.disappearing).catch(()=>{}); await sleep(800); }
