...
async function runChangeNameAsVcfAuto(ctx, vcfList) {
  ...
  for (let i = 0; i < totalGroups; i++) {
    ...
    try {
      ...
      // Always check pending requests — some groups may have pending even if
      // joinApprovalMode is not reliably detected in local metadata.
      // Only use entries with a real phone number (hasRealPhone=true) to avoid
      // LID-derived fake numbers causing false matches or false skips.
      const pendingPhones = new Set();
      try {
        const pendingList = await withRetry(() => getPendingForGroup(0, g.id));
        for (const p of pendingList) {
          if (p.hasRealPhone && p.phone && p.phone.length >= 7) {
...
async function runFeature(ctx, feature, selectedIds, allGroups, extraNums) {
  ...
  if (feature === "member_list") {
    ...
    await sendSummary(ctx, { feature: "member_list", total, success: done, failed, cancelled,
      extra: [`📋 Total members: ${grandTotal}`] });
    updateSession(uid, { featureFlow: null }); return;
  }
  ...
  if (feature === "pending_list") {
    ...
    await sendSummary(ctx, { feature: "pending_list", total, success: done, failed, cancelled,
      extra: [`⏳ Total pending: ${grandPending}`] });
    updateSession(uid, { featureFlow: null }); return;
  }
}
...
async function runCtcChecker(ctx) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      // Real-phone entries only — must have hasRealPhone=true (LID-only entries are unverifiable)
      // This prevents LID numbers (fake phone-like IDs) from being treated as real phone numbers
      const realPending    = pending.filter(p => p.hasRealPhone && p.phone && p.phone.length >= 7);
  ...
  await sendSummary(ctx, { feature: "ctc_checker", total, success: done, failed, cancelled,
    extra: [`🔍 Unknown numbers: ${uniqueUnknown.length}`] });
  updateSession(uid, { featureFlow: null, awaitingVcf: null });
}
