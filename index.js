...
function settingsKb(d) {
  return Markup.inlineKeyboard([
    [Markup.button.callback(`💬 Messages (lock)   : ${esFmt(d.announce)}`,      "es_tog_announce")],
...
bot.action("es_apply", async (ctx) => {
  ...
  await doneProgress(ctx.chat.id, pm.message_id,
    `✅ Applied: ${changed}  ⏭ Skipped: ${alreadyOk}  ❌ Failed: ${failed}`);
...
async function runChangeNameRandom(ctx, links, baseName, numbering) {
  ...
  await doneProgress(ctx.chat.id, pm.message_id,
    `✅ Renamed: ${done}/${total}${failed > 0 ? `  ❌ ${failed} failed` : ""}`);
  const extraLines = details.slice(0, 25);
...
bot.action("cn_vcf", async (ctx) => {
  ...
  await reply(ctx,
    `📛 *Change Name — Match VCF Filename*\n━━━━━━━━━━━━━━━━━━━━\n\n*How it works:*\n• Send your VCF files one by one\n• Bot scans ALL your groups automatically\n• Groups where VCF contacts are found (member or pending) get renamed to the VCF filename\n\n*No group links needed!*\n\n📎 *Send first VCF now:*`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) }
  );
});
 ...
 *
 * Matching logic (LID-aware):
 *  1. Check group members from metadata (LID-safe phone extraction)
 *  2. If group has joinApprovalMode ON, also check pending requests
 *  3. If any VCF contact phone matches a member/pending → rename to VCF filename
 *  4. Best-match VCF wins (most matching contacts)
 */
async function runChangeNameAsVcfAuto(ctx, vcfList) {
  ...
  // Pre-build phone sets per VCF for fast lookup
  const vcfSets = vcfList.map((v) => ({
    name: v.name,
    phones: new Set(v.contacts.map((c) => c.phone)),
  }));
  ...
  for (let i = 0; i < totalGroups; i++) {
    ...
    try {
      // ── Collect phone numbers from this group ──────────────────────────
      // Members (LID-aware extraction from existing metadata)
      const memberPhones = new Set(
        (g.participants || [])
          .map((p) => extractParticipantPhone(p))
          .filter((n) => n && n.length >= 7)
      );

      // Pending (only if approval mode is on — saves API calls)
      // Match ALL pending numbers (not just hasRealPhone — user wants full match)
      const pendingPhones = new Set();
      if (g.joinApprovalMode === "on" || g.joinApprovalMode === true) {
        try {
          const pendingList = await withRetry(() => getPendingForGroup(0, g.id));
          for (const p of pendingList) {
            if (p.phone && p.phone.length >= 7) {
              pendingPhones.add(p.phone);
            }
          }
        } catch {}
      ...
      const allPhones = new Set([...memberPhones, ...pendingPhones]);

      // ── Find best-matching VCF ─────────────────────────────────────────
      let bestVcf    = null;
      let bestCount  = 0;

      for (const vcf of vcfSets) {
        let count = 0;
        for (const ph of vcf.phones) {
          if (allPhones.has(ph)) count++;
        }
      ...
      if (bestVcf && bestCount > 0) {
        ...
        skipped++;
        // Don't clutter details with skipped groups
      }
    } catch (err) {
      failed++;
  ...
  if (details.length > 25) extraLines.push(`_...and ${details.length - 25} more_`);
  await sendSummary(ctx, { feature: "change_name", total: totalGroups, success: renamed, failed, cancelled, extra: extraLines });
...
bot.action("aa_start", async (ctx) => {
  ...
  setTimeout(async () => {
    ...
    const totalAccepted = Object.values(stats).reduce((s, v) => s + (v?.accepted || 0), 0);
    const details = sel.map((g) => `• *${g.name}*: ${stats[g.id]?.accepted || 0}`);
...
bot.action("aa_stop", async (ctx) => {
  ...
  const sel     = (flow.allGroups || []).filter((g) => flow.selectedIds.includes(g.id));
  const details = sel.map((g) => `• *${g.name}*: ${stats[g.id]?.accepted || 0}`);
  try { await ctx.editMessageText(`🛑 *Auto Accept Stopped*  Total: *${total}*`, { parse_mode: "Markdown" }); } catch {}
...
async function runFeature(ctx, feature, selectedIds, allGroups, extraNums) {
  ...
  if (feature === "get_links") {
    ...
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Links ready: ${done}/${total}  ❌ ${failed}`);
    // Send ALL links split into chunks of 30 per message (no truncation)
    const CHUNK = 30;
    for (let c = 0; c < results.length; c += CHUNK) {
      const chunk = results.slice(c, c + CHUNK);
      const part  = Math.floor(c / CHUNK) + 1;
      const parts = Math.ceil(results.length / CHUNK);
      const lines = chunk.map((r, j) => `*${c + j + 1}.* ${r.name}\n\`${r.link}\``);
      const header = parts > 1 ? `🔗 *Group Links — Part ${part}/${parts}*\n━━━━━━━━━━━━━━━━━━━━\n` : `🔗 *Group Links (${results.length}):*\n━━━━━━━━━━━━━━━━━━━━\n`;
      await ctx.reply(header + lines.join("\n\n"), { parse_mode: "Markdown" });
      if (c + CHUNK < results.length) await sleep(500);
    }
    if (fails.length) {
      await ctx.reply(`❌ *Failed (${fails.length}):*\n${fails.slice(0, 20).join("\n")}`, { parse_mode: "Markdown" });
    }
    await ctx.reply(
      `📊 *🔗 Get Links*\n━━━━━━━━━━━━━━━━━━━━\n${cancelled ? "🚫 *Cancelled*" : failed === 0 ? "✅ *All done!*" : `⚠️ *${failed} failed*`}\nTotal: ${total}  Success: ${done}  Failed: ${failed}\nTime: ${elapsed(uid)}s`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) }
    );
    startTimes.delete(uid);
  ...
  if (feature === "leave") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🚪 Deleting groups...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        // Step 1: remove all non-admin members first
        await removeAllMembers(0, g.id).catch(() => {});
        await sleep(1500);
        // Step 2: leave (group is now empty = effectively deleted)
        await withRetry(() => leaveGroup(0, g.id));
        done++; details.push(`✅ ${g.name}`);
      } catch (err) { failed++; details.push(`❌ ${g.name}: ${err.message}`); }
      await sleep(D.leave);
    }
    await doneProgress(ctx.chat.id, pm.message_id,
      `✅ Deleted: ${done}/${total}${failed > 0 ? `  ❌ ${failed} failed` : ""}`);
  ...
  if (feature === "remove_members") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        totalRem += n; done++; details.push(`✅ ${g.name}: ${n} removed`);
      } catch (err) { failed++; details.push(`❌ ${g.name}: ${err.message}`); }
      await sleep(D.removeMembers);
    }
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Done — Total removed: ${totalRem}`);
  ...
  if (feature === "make_admin") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        totalProm += n; done++;
        details.push(n > 0 ? `✅ ${g.name}: ${n} promoted` : `⚠️ ${g.name}: not found`);
      } catch (err) { failed++; details.push(`❌ ${g.name}: ${err.message}`); }
      await sleep(D.makeAdmin);
    }
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Done — Promoted: ${totalProm}`);
  ...
  if (feature === "demote_admin") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        totalDem += n; done++;
        details.push(n > 0 ? `✅ ${g.name}: ${n} demoted` : `⚠️ ${g.name}: not an admin`);
      } catch (err) { failed++; details.push(`❌ ${g.name}: ${err.message}`); }
      await sleep(D.demoteAdmin);
    }
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Done — Demoted: ${totalDem}`);
  ...
  if (feature === "reset_link") {
    ...
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Links reset: ${done}/${total}  ❌ ${failed}`);
    // Send ALL new links in chunks of 30 (no truncation)
    const CHUNK = 30;
    for (let c = 0; c < results.length; c += CHUNK) {
      const chunk = results.slice(c, c + CHUNK);
      const part  = Math.floor(c / CHUNK) + 1;
      ...
      const header = parts > 1
        ? `🔄 *New Links — Part ${part}/${parts}*\n━━━━━━━━━━━━━━━━━━━━\n`
        : `🔄 *New Links (${results.length}):*\n━━━━━━━━━━━━━━━━━━━━\n`;
      await ctx.reply(header + lines.join("\n\n"), { parse_mode: "Markdown" });
      if (c + CHUNK < results.length) await sleep(500);
    }
    if (fails.length) {
  ...
  if (feature === "approval") {
    ...
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Toggled: ${done}/${total}`);
    const extraLines = details.slice(0, 30);
    if (details.length > 30) extraLines.push(`_...and ${details.length - 30} more_`);
    await sendSummary(ctx, { feature, total, success: done, failed, cancelled, extra: extraLines });
    updateSession(uid, { featureFlow: null }); return;
  ...
  if (feature === "approve_pending") {
    ...
    await doneProgress(ctx.chat.id, pm.message_id,
      `✅ Done — Approved: ${totApproved}  Joined: ${totJoined}`);
    const extraLines = [
      `Pending: ${totPend}  Approved: ${totApproved}  Joined: ${totJoined}`,
      ...details.slice(0, 20),
    ];
  ...
  if (feature === "member_list") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        rows.push({ name: g.name, count: members.length, ok: true }); done++;
      } catch { failed++; rows.push({ name: g.name, count: 0, ok: false }); }
      await sleep(D.memberList);
    }
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Done: ${done}/${total} groups`);
    // Send member list in chunks — clean "Name: X members" format
    const sorted = [...rows].sort((a, b) => b.count - a.count);
    const listLines = sorted.map((r) => r.ok ? `• *${r.name}*: ${r.count} members` : `• *${r.name}*: ❌ error`);
    const CHUNK = 50;
    for (let c = 0; c < listLines.length; c += CHUNK) {
      const chunk = listLines.slice(c, c + CHUNK);
      const part = Math.floor(c / CHUNK) + 1, parts = Math.ceil(listLines.length / CHUNK);
      const hdr = c === 0
        ? `📋 *Member List* — Total: *${grandTotal}* members\n━━━━━━━━━━━━━━━━━━━━\n`
        : `📋 *Member List (cont. ${part}/${parts})*\n━━━━━━━━━━━━━━━━━━━━\n`;
      await ctx.reply(hdr + chunk.join("\n"), { parse_mode: "Markdown" });
      if (c + CHUNK < listLines.length) await sleep(400);
    }
    await ctx.reply(
      `📊 *📋 Member List*\n━━━━━━━━━━━━━━━━━━━━\n${cancelled ? "🚫 Cancelled" : "✅ Done"}\nGroups: ${total}  Members: ${grandTotal}  Failed: ${failed}\nTime: ${elapsed(uid)}s`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) }
    );
    startTimes.delete(uid);
    updateSession(uid, { featureFlow: null }); return;
  ...
  if (feature === "pending_list") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        rows.push({ name: g.name, count: pending.length, ok: true }); done++;
      } catch { failed++; rows.push({ name: g.name, count: 0, ok: false }); }
      await sleep(D.pendingList);
    }
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Done: ${done}/${total} groups`);
    // Send pending list in chunks — clean "Name: X pending" format
    const listLines = rows.map((r) => r.ok ? `• *${r.name}*: ${r.count} pending` : `• *${r.name}*: ❌ error`);
    const CHUNK = 50;
    for (let c = 0; c < listLines.length; c += CHUNK) {
      const chunk = listLines.slice(c, c + CHUNK);
      const part = Math.floor(c / CHUNK) + 1, parts = Math.ceil(listLines.length / CHUNK);
      const hdr = c === 0
        ? `⏳ *Pending Requests* — Total: *${grandPending}*\n━━━━━━━━━━━━━━━━━━━━\n`
        : `⏳ *Pending (cont. ${part}/${parts})*\n━━━━━━━━━━━━━━━━━━━━\n`;
      await ctx.reply(hdr + chunk.join("\n"), { parse_mode: "Markdown" });
      if (c + CHUNK < listLines.length) await sleep(400);
    }
    await ctx.reply(
      `📊 *⏳ Pending List*\n━━━━━━━━━━━━━━━━━━━━\n${cancelled ? "🚫 Cancelled" : "✅ Done"}\nGroups: ${total}  Pending: ${grandPending}  Failed: ${failed}\nTime: ${elapsed(uid)}s`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) }
    );
    startTimes.delete(uid);
    updateSession(uid, { featureFlow: null }); return;
...
bot.action("gf_create_now",async(ctx)=>{
  ...
  await doneProgress(ctx.chat.id,pm.message_id,`✅ Created: ${created.length}  ❌ Failed: ${failed.length}`);
  const linkLines=created.map((g,i)=>`*${i+1}.* ${g.name}\n   ${g.link}`);
  const extraLines=[];
  if(created.length){extraLines.push(`🔗 *Created Groups & Links:*`);extraLines.push(...linkLines.slice(0,25));if(linkLines.length>25)extraLines.push(`_...and ${linkLines.length-25} more_`);}
...
async function runAddMembersFromVcfs(ctx) {
  ...
  for (let i=0; i<total; i++) {
    ...
    if (!vcfEntry) { doneGroups++; summaryLines.push(`⏭ Group ${i+1}: skipped`); continue; }
    ...
    try {
      ...
      summaryLines.push(`✅ *${info.name}*: +${result.added}  ❌${result.failed}  ⏭${result.skipped}`);
    } catch (err) { failedGroups++; summaryLines.push(`❌ Group ${i+1}: ${err.message}`); }
    await sleep(D.addMembers);
  }
  await doneProgress(ctx.chat.id, pm.message_id,
    `✅ Done — Added: ${totAdded}  Failed: ${totFailed}  Skipped: ${totSkipped}`);
  const extraLines = [
    `➕ Total added: ${totAdded}  ❌ Failed: ${totFailed}  ⏭ Skipped: ${totSkipped}`,
...
bot.on("text", async (ctx) => {
  ...
  if (s.joinFlow?.step === "links") {
    ...
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Joined: ${joined}  ❌ Failed: ${failed}`);
    const extraLines = [];
    if (failedLinks.length) extraLines.push(`❌ Failed:\n${failedLinks.slice(0,10).join("\n")}`);
  ...
  // CTC Checker links → then collect VCFs one per group
  if (s.featureFlow?.step === "ctc_links") {
    ...
    await ctx.reply(
      `🔍 *CTC Checker — ${codes.length} group(s)*\n━━━━━━━━━━━━━━━━━━━━\n*Step 2:* Send VCFs — one per group.\nBot matches: Group 1 ↔ VCF 1, Group 2 ↔ VCF 2...\n\n📎 *Send VCF 1/${codes.length} now:*`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu","back_menu")]]) }
    );
    return;
  }
...
bot.on("document", async (ctx) => {
  ...
  // ── CTC Checker VCF — collect one per group ───────────────────────────
  if (awaitingVcf?.feature === "ctc_checker" && s.featureFlow?.step === "ctc_vcf_collecting") {
    ...
    try {
      ...
      if (!contacts.length) { await ctx.reply("⚠️ No valid numbers in VCF."); return; }
      const flow   = s.featureFlow;
      const idx    = flow.ctcVcfIdx || 0;
      const total  = (flow.links || []).length;
      const newList = [...(flow.vcfList || []), { contacts }];
      const nextIdx = idx + 1;
      if (nextIdx < total) {
        updateSession(uid, { featureFlow: { ...flow, vcfList: newList, ctcVcfIdx: nextIdx } });
        await ctx.reply(
          `✅ *VCF ${idx + 1}/${total} received!* ${contacts.length} numbers.\n\n📎 *Send VCF ${nextIdx + 1}/${total} now:*`,
          { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu","back_menu")]]) }
        );
      } else {
        // All VCFs collected — start checking
        updateSession(uid, { featureFlow: { ...flow, vcfList: newList, ctcVcfIdx: nextIdx, step: "ctc_running" }, awaitingVcf: null });
        await ctx.reply(`✅ *All ${total} VCFs received!*\n\n⏳ *Checking now...*`, { parse_mode: "Markdown" });
...
// LID-only entries → shown as "unverifiable" (NOT counted as unknown)
...
async function runCtcChecker(ctx) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      // Trusted phones for THIS group = corresponding VCF's contacts
      const vcfEntry       = vcfList[i] || { contacts: [] };
      const trustedPhones  = new Set((vcfEntry.contacts || []).map(c => c.phone));

      // Fetch ALL pending requests for this group
      const pending        = await withRetry(() => getPendingForGroup(0, info.id));

      // Real-phone entries only (LID-only entries go to unverifiable count)
      const realPending    = pending.filter(p => p.phone && p.phone.length >= 7);
      const lidOnlyCount   = pending.length - realPending.length;

      // Unknown = number not in this group's VCF
      const unknownPending = realPending.filter(p => !trustedPhones.has(p.phone));

      done++;

      if (unknownPending.length > 0 || lidOnlyCount > 0) {
        allUnknown.push(...unknownPending.map(u => u.phone));
        const parts = [];
        if (unknownPending.length) parts.push(`${unknownPending.length} unknown`);
        if (lidOnlyCount)          parts.push(`${lidOnlyCount} unverifiable`);
        reportLines.push(
          `⚠️ *${info.name}*: ${pending.length} pending, ${parts.join(", ")}\n` +
          (unknownPending.length ? unknownPending.slice(0, 5).map(u => `   +${u.phone}`).join("\n") : "")
        );
      } else {
  ...
  await doneProgress(ctx.chat.id, pm.message_id,
    `🔍 Done — Unknown: ${uniqueUnknown.length}  Groups: ${done}/${total}`);

  // Send report in chunks
  if (reportLines.length > 0) {
    const CHUNK = 20;
    for (let c = 0; c < reportLines.length; c += CHUNK) {
      const chunk = reportLines.slice(c, c + CHUNK);
      const part  = Math.floor(c / CHUNK) + 1, parts = Math.ceil(reportLines.length / CHUNK);
      const hdr   = c === 0
        ? `🔍 *CTC Report*\n━━━━━━━━━━━━━━━━━━━━\nGroups: ${done}  Unknown numbers: ${uniqueUnknown.length}\n━━━━━━━━━━━━━━━━━━━━\n`
        : `🔍 *CTC Report (cont. ${part}/${parts})*\n━━━━━━━━━━━━━━━━━━━━\n`;
      await ctx.reply(hdr + chunk.join("\n"), { parse_mode: "Markdown" });
      if (c + CHUNK < reportLines.length) await sleep(400);
    }
  }

  if (uniqueUnknown.length > 0) {
    const CHUNK = 50;
    for (let c = 0; c < uniqueUnknown.length; c += CHUNK) {
      const nums = uniqueUnknown.slice(c, c + CHUNK).map(p => `+${p}`).join("\n");
      const hdr  = c === 0 ? `📋 *All Unknown Numbers (${uniqueUnknown.length}):*\n━━━━━━━━━━━━━━━━━━━━\n` : `📋 *Unknown Numbers (cont.)*\n━━━━━━━━━━━━━━━━━━━━\n`;
      await ctx.reply(hdr + nums, { parse_mode: "Markdown" });
      if (c + CHUNK < uniqueUnknown.length) await sleep(400);
    }
  }

  await ctx.reply(
    `📊 *🔍 CTC Checker*\n━━━━━━━━━━━━━━━━━━━━\n${cancelled ? "🚫 Cancelled" : "✅ Done"}\nGroups: ${total}  Unknown: ${uniqueUnknown.length}  Failed: ${failed}\nTime: ${elapsed(uid)}s`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) }
  );
  startTimes.delete(uid);
  updateSession(uid, { featureFlow: null, awaitingVcf: null });
