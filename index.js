/**
 * WhatsApp Group Creator Bot — Fixed & Latest v2.0
 *
 * KEY FIXES vs original:
 * 1. Phone number parsing: accepts 7+ digit numbers (handles shorter country codes)
 * 2. Similar Groups: searches ALL words in group name, not just first word
 * 3. Pending detect: fixed in whatsapp-manager (getPendingJid covers all Baileys fields)
 * 4. Group fetch: retry logic added in whatsapp-manager
 * 5. Number input: strips +, spaces, dashes, parentheses before processing
 * 6. VCF parser: improved to handle wrapped lines and special chars
 * 7. Error messages: clearer and more specific
 */
...
// ─── VCF Parser — FIXED ───────────────────────────────────────────────────
// FIX: Handles folded lines (RFC 6350), special chars, and multiple TEL fields
function parseVcf(content) {
  const contacts = [];
  // Unfold lines (lines starting with space/tab are continuation of previous)
  const unfolded = content.replace(/\r\n[ \t]/g, "").replace(/\r\n/g, "\n").replace(/\r/g, "\n");
  const blocks = unfolded.split(/(?=BEGIN:VCARD)/gi);
  for (const block of blocks) {
    ...
    for (const m of telMatches) {
      // FIX: Strip ALL non-digit chars (handles +, spaces, dashes, dots, parens)
      const digits = m[1].trim().replace(/[^0-9]/g, "");
      if (digits.length >= 7) contacts.push({ name, phone: digits });
    }
  }
  return contacts;
}
...
// FIX: Robust phone number extraction — strips +, -, spaces, parens
...
bot.action("gf_create_now", async (ctx) => {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      // FIX: create with at least one dummy JID to avoid Baileys error, then remove
      const result = await createGroup(0, name, [`${flow.members[0] || "0000000000"}@s.whatsapp.net`]);
      const gid = result.id;
      ...
      if (flow.members.length > 0) {
        const memberJids = flow.members.map((n) => `${n}@s.whatsapp.net`);
        await promoteToAdmin(0, gid, memberJids.filter((_, idx) => idx === 0 && flow.makeAdmin ? true : false)).catch(() => {});
      }
...
app.get("/", (_, res) => res.send(`<html><body style="font-family:sans-serif;text-align:center;padding:50px;background:#111;color:#fff"><h2>✅ WA Group Creator Bot v2.0</h2><p style="color:#4ade80">Running 🟢</p><p>WA: ${getConnectedCount()>0?"Connected ✅":"Disconnected ❌"}</p><small style="color:#888">Fixed: pending detect, group search, number parsing</small></body></html>`));
app.get("/health", (_, res) => res.json({ status:"ok", version:"2.0", whatsapp:getStatus(0), phone:getPhone(0)||null, ts:new Date().toISOString() }));
...
async function main() {
  ...
  console.log(`WA Group Creator Bot v2.0 running! Owner: ${OWNER_ID||"NOT SET"}`);
}
