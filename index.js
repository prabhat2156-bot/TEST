...
const { UserRegistry } = require("./src/models");
...
// ══════════════════════════════════════════════════════════════════════════
// ─── MULTI-USER ACCOUNT MANAGEMENT (OPTIMIZED) ───────────────────────────
//
// Architecture:
//   • Each Telegram user gets a PERMANENT account index stored in MongoDB
//   • userAccountMap (RAM) is rebuilt from DB on startup
//   • 1 min inactivity → socket suspended (RAM freed), auth stays in MongoDB
//   • User returns → resume from saved auth (NO re-pairing needed)
//   • Max concurrent connections enforced to prevent OOM crash
//   • Connection queue: new user waits if server is at capacity
//   • Cleanup runs every 20s for fast slot recycling
// ══════════════════════════════════════════════════════════════════════════

const userAccountMap = new Map();   // Telegram userId → WA account index
const indexPhoneMap  = new Map();   // account index → phoneNumber
const connectionQueue = [];         // waiting users when at capacity
let _nextIdx = 0;

// Max simultaneous WhatsApp sockets (tune based on server RAM)
// Render 512MB: 25 | Render 2GB: 80 | VPS 4GB: 150
const MAX_CONCURRENT = parseInt(process.env.MAX_CONCURRENT || "25", 10);

// Inactivity timeout: 1 minute
const SUSPEND_MS = 60 * 1000;

// ─── Get or assign account index for user (persistent via MongoDB) ────────
async function getOrCreateUserIdx(userId) {
  if (userAccountMap.has(userId)) return userAccountMap.get(userId);
  // Try loading from DB first
  const existing = await UserRegistry.findOne({ telegramUserId: userId }).lean();
  if (existing) {
    userAccountMap.set(userId, existing.accountIndex);
    return existing.accountIndex;
  }
  // New user — assign next index
  _nextIdx++;
  const idx = _nextIdx;
  userAccountMap.set(userId, idx);
  await UserRegistry.findOneAndUpdate(
    { telegramUserId: userId },
    { telegramUserId: userId, accountIndex: idx },
    { upsert: true }
  ).catch(() => {});
  return idx;
}

// Sync version (used after async init)
function uidxSync(uid) { return userAccountMap.get(uid) ?? 0; }

// ─── Activity tracking ────────────────────────────────────────────────────
const lastActive = new Map();   // userId → Date.now()

function touch(userId) { lastActive.set(userId, Date.now()); }

// ─── Connection queue: allow waiting users when at capacity ───────────────
async function waitForSlot(userId) {
  if (getConnectedCount() < MAX_CONCURRENT) return; // slot available
  return new Promise((resolve) => {
    connectionQueue.push({ userId, resolve });
  });
}
...
// ─── Cleanup: every 20 seconds suspend inactive users ─────────────────────
setInterval(async () => {
  const now = Date.now();
  for (const [userId, t] of [...lastActive.entries()]) {
    if (now - t > SUSPEND_MS) {
      const idx = userAccountMap.get(userId);
      if (idx !== undefined && getStatus(idx) === "connected") {
        await suspendAccount(idx).catch(() => {});
        releaseSlot(); // free slot for queue
      }
      lastActive.delete(userId);
      resetSession(userId);
    }
  }
  // Force GC hint every cleanup cycle
  if (global.gc) { try { global.gc(); } catch {} }
}, 20_000);

// ─── uidx: async-safe helper used in handlers ─────────────────────────────
async function uidxAsync(uid) { return await getOrCreateUserIdx(uid); }
function uidx(uid) { return uidxSync(uid); }
...
// ─── Global middleware: touch activity + init user index ───────────────────
bot.use(async (ctx, next) => {
  const uid = ctx.from?.id;
  if (!uid) return next();
  touch(uid);
  // Ensure user has an assigned index (async, but non-blocking for speed)
  if (!userAccountMap.has(uid)) {
    getOrCreateUserIdx(uid).catch(() => {});
  }
  return next();
});
...
bot.action("cancel_exec", async (ctx) => {
  await ctx.answerCbQuery("🛑 Cancelling...", { show_alert: false });
  updateSession(ctx.from.id, { cancelPending: true });
  const session = getSession(ctx.from.id);
  const msgId = session?.cancelMsgId;
  if (msgId && ctx.chat?.id) {
    try {
      await bot.telegram.editMessageText(ctx.chat.id, msgId, undefined, "🛑 *Cancelling... Please wait*", { parse_mode: "Markdown" });
    } catch {}
  } else {
    try { await ctx.editMessageText("🛑 *Cancelling... Please wait*", { parse_mode: "Markdown" }); } catch {}
  }
});

function isCancelled(uid) { return getSession(uid).cancelPending === true; }

async function cancellableSleep(ms, uid) {
  const step = 200;
  let elapsed = 0;
  while (elapsed < ms) {
    if (isCancelled(uid)) return true;
...
async function sendSummary(ctx, opts) {
  ...
  if (extra.length) summaryText += `━━━━━━━━━━━━━━━━━━━━\n` + extra.join("\n") + "\n";
  summaryText += `━━━━━━━━━━━━━━━━━━━━`;

  const replyMarkup = Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]);

  // Split boxLines into chunks that each fit within 3800 chars (code block overhead safe)
  const boxChunks = [];
  if (boxLines.length) {
    let current = [];
    let currentLen = 0;
    for (const line of boxLines) {
      ...
      if (currentLen + lineLen > 3600 && current.length > 0) {
        boxChunks.push(current);
        current = [];
        currentLen = 0;
      }
      current.push(line);
    ...
    if (current.length > 0) boxChunks.push(current);
  }

  // Try to fit first box chunk into summary message
  let firstMessage = summaryText;
  let remainingChunks = boxChunks;
  if (boxChunks.length > 0) {
    const firstBox = "\n```\n" + boxChunks[0].join("\n") + "\n```";
    if ((summaryText + firstBox).length <= 4000) {
      firstMessage = summaryText + firstBox;
      remainingChunks = boxChunks.slice(1);
    }
  }

  // Send or edit the first/summary message
  const hasMore = remainingChunks.length > 0;
  const session = uid ? getSession(uid) : null;
  const cancelMsgId = session?.cancelMsgId;

  const sendFirst = async (text, withButton) => {
    const extra2 = withButton ? { parse_mode: "Markdown", ...replyMarkup } : { parse_mode: "Markdown" };
    if (cancelMsgId && ctx.chat?.id) {
      try {
        await bot.telegram.editMessageText(ctx.chat.id, cancelMsgId, undefined, text, {
          parse_mode: "Markdown",
          reply_markup: withButton ? replyMarkup.reply_markup : undefined,
        });
        if (uid) updateSession(uid, { cancelMsgId: null });
      } catch { await ctx.reply(text, extra2); }
    } else {
      await ctx.reply(text, extra2);
    }
  };

  await sendFirst(firstMessage, !hasMore);

  // Send remaining box chunks as separate messages
  for (let i = 0; i < remainingChunks.length; i++) {
    const isLast = i === remainingChunks.length - 1;
    const boxText = "```\n" + remainingChunks[i].join("\n") + "\n```";
    if (isLast) {
      try {
        await ctx.reply(boxText, { parse_mode: "Markdown", ...replyMarkup });
      } catch {
        await ctx.reply(remainingChunks[i].join("\n"), { parse_mode: "Markdown", ...replyMarkup });
      }
    } else {
      try {
        await ctx.reply(boxText, { parse_mode: "Markdown" });
      } catch {
        await ctx.reply(remainingChunks[i].join("\n"));
      }
      await sleep(400);
...
function buildMainMenu(uid) {
  const c = getStatus(uidx(uid)) === "connected", p = getPhone(uidx(uid));
...
async function sendMainMenu(ctx) {
  const uid = ctx.from?.id;
  touch(uid);
  if (uid && !userAccountMap.has(uid)) await getOrCreateUserIdx(uid).catch(() => {});
  const c = getStatus(uidx(uid)) === "connected", p = getPhone(uidx(uid));
...
bot.command("status", async (ctx) => {
  const uid = ctx.from.id; touch(uid);
  const idx = uidx(uid);
  const s = getStatus(idx), p = getPhone(idx);
  const icon = s === "connected" ? "✅" : s === "connecting" ? "⏳" : "❌";
  const total = getConnectedCount();
  await ctx.reply(
    `📊 *Bot Status*\n━━━━━━━━━━━━━━━━━━━━\n` +
    `${icon} Your WhatsApp: *${s}*${s === "connected" ? `\n📞 +${p}` : ""}\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `🌐 Active connections: *${total}/${MAX_CONCURRENT}*\n` +
    `⏱ Auto-suspend: *1 min inactivity*\n` +
    `━━━━━━━━━━━━━━━━━━━━`,
    { parse_mode: "Markdown", ...require("telegraf").Markup.inlineKeyboard([[require("telegraf").Markup.button.callback("🏠 Main Menu", "back_menu")]]) }
  );
});

bot.start(async (ctx) => { const uid=ctx.from.id; touch(uid); resetSession(uid); await sendMainMenu(ctx); });
...
bot.action("menu_status", async (ctx) => {
  await ctx.answerCbQuery();
  const uid = ctx.from.id; touch(uid);
  const s = getStatus(uidx(uid)), p = getPhone(uidx(uid));
  const icon = s === "connected" ? "✅" : s === "connecting" ? "⏳" : "❌";
...
bot.action("menu_account", async (ctx) => {
  await ctx.answerCbQuery();
  const uid = ctx.from.id; touch(uid);
  if (!userAccountMap.has(uid)) await getOrCreateUserIdx(uid).catch(() => {});
  const idx = uidx(uid);
  let status = getStatus(idx), phone = getPhone(idx);
  // If disconnected but has saved auth (was suspended) → auto-resume silently
  if (status === "disconnected" && indexPhoneMap.has(idx)) {
    phone = indexPhoneMap.get(idx);
    await resumeAccount(idx, phone).catch(() => {});
    await sleep(2000);
  ...
  if (status === "connected") {
    ...
    updateSession(uid, { awaitingPhoneForIndex: uidx(uid) });
    await editOrReply(ctx,
...
bot.action("logout_wa", async (ctx) => {
  ...
  await disconnectAccount(uidx(uid)); await sleep(800); await sendMainMenu(ctx);
});
...
bot.action(/^gs_similar_(.+)$/, async (ctx) => {
  ...
  try {
    const all = await getAllGroupsWithDetails(uidx(uid));
...
bot.action(/^gs_all_(.+)$/, async (ctx) => {
  ...
  const feature = ctx.match[1];
  try {
    const groups = await getAllGroupsWithDetails(uidx(uid));
...
bot.action("es_apply", async (ctx) => {
  ...
  for (let i = 0; i < total; i++) {
    ...
    await editProgress(ctx.chat.id, pm.message_id,
      `⚙️ Applying settings...\nDone: ${i}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
    try {
      const result = await withRetry(() => applyGroupSettings(uidx(uid), g.id, d), 2, 2000);
      if (result.changes.length) { changed++; }
      else { alreadyOk++; }
    } catch (err) { failed++; }
    if (await cancellableSleep(D.approvalToggle, uid)) { cancelled = true; break; }
  }
...
async function runChangeNameRandom(ctx, links, baseName, numbering) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      const info = await withTimeout(withRetry(() => getGroupInfoFromLink(uidx(uid), code), 2, 1500), 12000, "GetGroupInfo");
      if (!info) throw new Error("Invalid/expired link");
      await withTimeout(withRetry(() => renameGroup(uidx(uid), info.id, newName), 2, 1500), 12000, "RenameGroup");
      done++; boxLines.push(`${info.name} ➡️ ${newName}`);
    } catch (err) { failed++; boxLines.push(`❌ Group ${i + 1}: ${err.message}`); }
    if (await cancellableSleep(D.changeName, uid)) { cancelled = true; break; }
  }
...
async function runChangeNameAsVcfAuto(ctx, vcfList) {
  ...
  let allGroups;
  try {
    allGroups = await withRetry(() => getAllGroupsWithDetails(uidx(uid)));
  } catch (err) {
  ...
  for (const v of vcfList) {
    ...
    const resolved = phones.length ? await resolveVcfPhones(uidx(uid), phones) : [];
  ...
  for (let i = 0; i < totalGroups; i++) {
    ...
    try {
      ...
      try {
        const { jids: pendingJids, phones: pendingPhones } =
          await withTimeout(withRetry(() => getPendingRawJids(uidx(uid), g.id), 2, 1500), 10000, "PendingJids");
        pendingJids.forEach((j) => groupJids.add(j));
        pendingPhones.forEach((ph) => groupPhones.add(ph));
      } catch {}
      if (await cancellableSleep(D.pendingCheck, uid)) { cancelled = true; break; }
      ...
      if (bestVcf && bestCount > 0) {
        await withTimeout(withRetry(() => renameGroup(uidx(uid), g.id, bestVcf.name), 2, 1500), 12000, "RenameGroup");
        renamed++;
    ...
    if (await cancellableSleep(D.vcfAutoMatch, uid)) { cancelled = true; break; }
  }
...
async function runFeature(ctx, feature, selectedIds, allGroups, extraNums) {
  ...
  if (feature === "get_links") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🔗 Getting links...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try { results.push({ name: g.name, link: await withRetry(() => getGroupInviteLink(uidx(uid), g.id)) }); done++; }
      catch (err) { fails.push(g.name); failed++; }
      if (await cancellableSleep(D.getLinks, uid)) { cancelled = true; break; }
    }
  ...
  if (feature === "leave") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🚪 Leaving groups...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        await withRetry(() => leaveGroup(uidx(uid), g.id));
        done++;
      } catch (err) { failed++; }
      if (await cancellableSleep(D.leave, uid)) { cancelled = true; break; }
    }
  ...
  if (feature === "remove_members") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        const n = await withRetry(() => removeAllMembers(uidx(uid), g.id, 1, true));
        totalRem += n; done++; boxLines.push(`${g.name}: ${n} members removed`);
      } catch (err) { failed++; boxLines.push(`❌ ${g.name}: failed`); }
      if (await cancellableSleep(D.removeMembers, uid)) { cancelled = true; break; }
    }
    await sendSummary(ctx, { feature, total, success: done, failed, cancelled,
  ...
  if (feature === "make_admin") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `👑 Making admin...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const n = await makeAdminByNumbers(uidx(uid), g.id, extraNums);
        totalProm += n; done++;
        boxLines.push(n > 0 ? `${g.name}: ${n} admin set` : `${g.name}: not found`);
      } catch (err) { failed++; boxLines.push(`❌ ${g.name}: failed`); }
      if (await cancellableSleep(D.makeAdmin, uid)) { cancelled = true; break; }
    }
  ...
  if (feature === "demote_admin") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `⬇️ Demoting admins...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const n = await demoteAdminInGroup(uidx(uid), g.id, extraNums);
        totalDem += n; done++;
        boxLines.push(n > 0 ? `${g.name}: ${n} demoted` : `${g.name}: not an admin`);
      } catch (err) { failed++; boxLines.push(`❌ ${g.name}: failed`); }
      if (await cancellableSleep(D.demoteAdmin, uid)) { cancelled = true; break; }
    }
    const numbersStr = extraNums.map((n) => `+${n}`).join(", ");
    await sendSummary(ctx, { feature, total, success: done, failed, cancelled,
  ...
  if (feature === "reset_link") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🔄 Resetting links...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try { results.push({ name: g.name, link: await withRetry(() => resetGroupInviteLink(uidx(uid), g.id)) }); done++; }
      catch { fails.push(g.name); failed++; }
      if (await cancellableSleep(D.resetLink, uid)) { cancelled = true; break; }
    }
    const boxLines = results.map((r) => `${r.name}\n${r.link}`);
    if (fails.length) fails.forEach((n) => boxLines.push(`❌ ${n}: failed`));
  ...
  if (feature === "approval") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🔀 Toggling approval...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const cur = await withRetry(() => getGroupApprovalStatus(uidx(uid), g.id)), next = !cur;
        await withRetry(() => setGroupApproval(uidx(uid), g.id, next));
        done++;
      } catch (err) { failed++; }
      if (await cancellableSleep(D.approvalToggle, uid)) { cancelled = true; break; }
    }
  ...
  if (feature === "approve_pending") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `✅ Approving pending...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const r = await withRetry(() => approveAllPending(uidx(uid), g.id), 2, 5000);
        totPend += r.pendingCount; totApproved += r.approved; done++;
        boxLines.push(`${g.name} = ${r.approved} member add`);
      } catch (err) { failed++; boxLines.push(`❌ ${g.name}: failed`); }
      if (await cancellableSleep(D.approvePending, uid)) { cancelled = true; break; }
    }
    await sendSummary(ctx, { feature, total, success: done, failed, cancelled,
  ...
  if (feature === "member_list") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `📋 Member list...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const members = await withRetry(() => getGroupMembers(uidx(uid), g.id));
        grandTotal += members.length;
        boxLines.push(`${g.name} = ${members.length} members`);
        done++;
      } catch { failed++; boxLines.push(`${g.name}\nfailed`); }
      if (await cancellableSleep(D.memberList, uid)) { cancelled = true; break; }
    }
  ...
  if (feature === "pending_list") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `⏳ Pending list...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const { list: pending } = await withRetry(() => getGroupPendingRequests(uidx(uid), g.id));
        grandPending += pending.length;
        boxLines.push(`${g.name} = ${pending.length} pending`);
        done++;
      } catch { failed++; boxLines.push(`${g.name}\nfailed`); }
      if (await cancellableSleep(D.pendingList, uid)) { cancelled = true; break; }
    }
...
bot.action("gf_create_now",async(ctx)=>{
  ...
  for(let i=0;i<flow.count;i++){
    ...
    try{
      const r=await withRetry(()=>createGroup(uidx(uid),gname,jids));const gid=r.id;
      await sleep(3000); // give WA time to register the new group
      if(flow.description){await updateGroupDescription(uidx(uid),gid,flow.description).catch(()=>{});await sleep(600);}
      if(flow.photo){
        ...
        await withRetry(()=>updateGroupPhoto(uidx(uid),gid,photoBuf),3,3000).catch(()=>{});
        await sleep(800);
      }
      if(flow.disappearing){await setDisappearingMessages(uidx(uid),gid,flow.disappearing).catch(()=>{});await sleep(500);}
      if(flow.makeAdmin&&jids.length){await makeAdminByNumbers(uidx(uid),gid,flow.members).catch(()=>{});await sleep(1000);}
      await setGroupPermissions(uidx(uid),gid,flow.permissions).catch(()=>{});
      let link="";try{link=await getGroupInviteLink(uidx(uid),gid);}catch{link="(unavailable)";}
      created.push({name:gname,link});
    }catch(err){failed.push(gname);}
    if (await cancellableSleep(D.createGroup, uid)) { cancelled = true; break; }
  }
...
async function runAddMembersFromVcfs(ctx) {
  ...
  for (let i=0; i<total; i++) {
    ...
    try {
      const info = await withTimeout(withRetry(() => getGroupInfoFromLink(uidx(uid), links[i]), 2, 1500), 12000, "GetGroupInfo");
      if (!info) throw new Error("Invalid link");
      const result = await addMembersToGroup(uidx(uid), info.id, contacts.map(c=>c.phone), flow.addMode==="onebyone");
    ...
    if (await cancellableSleep(D.addMembers, uid)) { cancelled = true; break; }
  }
...
bot.on("text", async (ctx) => {
  ...
  if (s.awaitingPhoneForIndex !== null && s.awaitingPhoneForIndex !== undefined) {
    ...
    const _idx = await getOrCreateUserIdx(uid);
    // Wait if server is at capacity
    if (getConnectedCount() >= MAX_CONCURRENT) {
      await ctx.reply(
        `⏳ *Server at capacity (${getConnectedCount()}/${MAX_CONCURRENT} connections)*\n\nYou are in queue. Please wait a moment...`,
        { parse_mode: "Markdown" }
      );
      await waitForSlot(uid);
    }
    pendingPairingCbs.set(_idx, async (code) => {
    ...
    pendingReadyCbs.set(_idx, async () => {
      indexPhoneMap.set(_idx, phone);
      // Save telegramUserId mapping in AccountInfo for restart recovery
      const { AccountInfo: AI } = require("./src/models");
      await AI.findOneAndUpdate(
        { accountIndex: _idx },
        { accountIndex: _idx, telegramUserId: uid, phoneNumber: phone, hasAuth: true },
        { upsert: true }
      ).catch(() => {});
      await sendMainMenu(ctx);
    });
    connectAccount(_idx, phone).catch(async (err) => {
      pendingPairingCbs.delete(_idx); pendingReadyCbs.delete(_idx);
  ...
  if (s.joinFlow?.step === "links") {
    ...
    for (let i=0; i<codes.length; i++) {
      ...
      try { await withRetry(() => joinGroupViaLink(uidx(uid), codes[i])); joined++; }
      catch { failed++; }
      if (await cancellableSleep(D.joinGroup, uid)) { cancelled = true; break; }
    }
  ...
  if (s.featureFlow?.step === "similar_query") {
    ...
    try {
      const allGroups = s.featureFlow.allGroups?.length ? s.featureFlow.allGroups : await getAllGroupsWithDetails(uidx(uid));
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      const { jids: pendingJids, phones: pendingPhones, count: pendingCount } =
        await withTimeout(withRetry(() => getPendingRawJids(uidx(uid), info.id), 2, 1500), 12000, "GetPendingJids");

      // Step 2: Resolve VCF phones via onWhatsApp to get real JIDs + LIDs
      const resolved = vcfPhones.length
        ? await withTimeout(resolveVcfPhones(uidx(uid), vcfPhones), 20000, "ResolveVcf")
        : [];
    ...
    if (await cancellableSleep(D.ctcCheck, uid)) { cancelled = true; break; }
  }
...
async function main() {
  await connectDB();
  // Restore userAccountMap from UserRegistry (persistent index mapping)
  const allUsers = await UserRegistry.find({}).lean();
  for (const u of allUsers) {
    userAccountMap.set(u.telegramUserId, u.accountIndex);
    if (u.accountIndex > _nextIdx) _nextIdx = u.accountIndex;
  }
  // Restore phone map from AccountInfo
  const { AccountInfo } = require("./src/models");
  const saved = await AccountInfo.find({ hasAuth: true }).lean();
  for (const a of saved) {
    indexPhoneMap.set(a.accountIndex, a.phoneNumber);
  }
  // Only reconnect up to MAX_CONCURRENT on startup (avoid OOM)
  const toReconnect = saved.slice(0, MAX_CONCURRENT);
