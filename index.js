...
async function sendMainMenu(ctx) {
  const c = getStatus(0) === "connected", p = getPhone(0);
  updateSession(ctx.from?.id, { cancelPending: false, awaitingVcf: null });
  await ctx.reply(
    `🤖 *WA Group Manager*\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `WhatsApp: ${c ? `✅ Connected — +${p}` : "❌ Not Connected"}\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
...
async function runFeature(ctx, feature, selectedIds, allGroups, extraNums) {
  ...
  if (feature === "get_links") {
    ...
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Links ready: ${done}/${total}`);
  ...
  if (feature === "leave") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🚪 Leaving groups...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try { await withRetry(() => leaveGroup(0, g.id)); done++; details.push(`✅ ${g.name}`); }
      catch (err) { failed++; details.push(`❌ ${g.name}: ${err.message}`); }
      await sleep(D.leave);
    }
    await doneProgress(ctx.chat.id, pm.message_id,
      `✅ Left: ${done}/${total}${failed > 0 ? `  ❌ ${failed} failed` : ""}`);
  ...
  if (feature === "reset_link") {
    ...
    await doneProgress(ctx.chat.id, pm.message_id, `✅ Links reset: ${done}/${total}`);
    const linkLines  = results.map((r, i) => `*${i + 1}.* ${r.name}\n   ${r.link}`);
  ...
  if (feature === "member_list") {
    ...
    const extraLines = [`👥 *Grand Total: ${grandTotal} members*`, `━━━━━━━━━━━━━━━━━━━━`, ...tableLines.slice(0, 30)];
  ...
  if (feature === "pending_list") {
    ...
    const extraLines = [`⏳ *Total Pending: ${grandPending}*`, `━━━━━━━━━━━━━━━━━━━━`, ...tableLines.slice(0, 25)];
...
async function runCtcChecker(ctx) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      // Split into verifiable (real phone) and LID-only (unverifiable)
      ...
      // Unknown = real phone but NOT in trusted VCF
      ...
      if (unknownPending.length > 0 || lidOnlyCount > 0) {
        ...
        reportLines.push(`⚠️ *${info.name}*: ${parts.join(", ")}`);
      } else {
  ...
  await doneProgress(ctx.chat.id, pm.message_id,
    `🔍 CTC Done — Unknown: ${uniqueUnknown.length}  Unverifiable (LID): calculated`);

  const extraLines = [
    `Groups checked : ${done}`,
    `Trusted in VCF : ${trustedPhones.size}`,
    `Unknown numbers: ${uniqueUnknown.length}`,
    `━━━━━━━━━━━━━━━━━━━━`,
