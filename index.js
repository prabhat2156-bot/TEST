...
function makeBox(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (Array.isArray(item)) {
      for (const sub of item) lines.push(sub);
    } else {
      lines.push(item);
    }
...
async function sendMainMenu(ctx, edit = false) {
  ...
  const waStatus = c ? `✅  Connected  +${p}` : `❌  Not Connected`;
  const text =
    `🤖 *ᴡꜱ ᴀᴜᴛᴏᴍᴀᴛɪᴏɴ* 🤖\n` +
    `▰▰▰▰▰▰▰▰▰▰▰▰▰\n\n` +
  ...
  if (edit) {
    try { return await ctx.editMessageText(text, { parse_mode: "Markdown", ...keyboard }); } catch {}
  }
...
async function runChangeNameRandom(ctx, links, baseName, numbering) {
  ...
  // Format: OldName ➡️ NewName in box
  const cnBoxItems = details.slice(0, 25).map(d => {
    const m = d.match(/✅ (.+) → *(.+)*/); return m ? `${m[1]} ➡️ ${m[2]}` : d.replace(/[*✅❌]/g, '').trim();
  });
  if (details.length > 25) cnBoxItems.push(`...and ${details.length - 25} more`);
  const extraLines = [makeBox(cnBoxItems)];
...
async function runChangeNameAsVcfAuto(ctx, vcfList) {
  ...
  // Only show renamed groups in box with OldName ➡️ NewName format
  const vcfCnBoxItems = details
    .filter(d => d.startsWith('✅'))
    .slice(0, 25)
    .map(d => {
      const m = d.match(/✅ *(.+)* → *(.+)*/); return m ? `${m[1]} ➡️ ${m[2]}` : d.replace(/[*✅]/g, '').trim();
    });
  if (details.filter(d => d.startsWith('✅')).length > 25) vcfCnBoxItems.push(`...and more`);
  const extraLines = [
    `Groups scanned : ${totalGroups}`,
    `Renamed        : ${renamed}`,
    `No match (skip): ${skipped}`,
    ...(vcfCnBoxItems.length ? [makeBox(vcfCnBoxItems)] : []),
  ];
...
function buildLiveAutoAcceptText(sel, label, endTime, stats) {
  ...
  const groupItems = sel.map((g) => `${g.name}: ${stats[g.id]?.accepted || 0} accepted`);
  return (
    `⏰ *Auto Accept — ACTIVE* 🟢\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `Groups          : *${sel.length}*\n` +
...
bot.action("aa_stop", async (ctx) => {
  ...
  const aaStopBox = sel.map(g => `${g.name}: ${stats[g.id]?.accepted || 0} accepted`);
...
async function runFeature(ctx, feature, selectedIds, allGroups, extraNums) {
  ...
  if (feature === "get_links") {
    ...
    // Send ALL links in box-format chunks of 30 per message
    const CHUNK = 30;
    for (let c = 0; c < results.length; c += CHUNK) {
      ...
      const boxItems = chunk.map(r => [r.name, r.link]);
      const header = parts > 1
        ? `🔗 *Group Links — Part ${part}/${parts}*\n━━━━━━━━━━━━━━━━━━━━\n`
        : `🔗 *Group Links*\n━━━━━━━━━━━━━━━━━━━━\nTotal groups : ${total}  ✅ Success : ${done}\n━━━━━━━━━━━━━━━━━━━━\n`;
      await ctx.reply(header + makeBox(boxItems), { parse_mode: "Markdown" });
    ...
    await sendSummary(ctx, { feature: "get_links", total, success: done, failed, cancelled,
      extra: [`Total groups : ${total}`, `Success      : ${done}`, `Failed       : ${failed}`] });
  ...
  if (feature === "leave") {
    ...
    const extraLines = [`Groups selected : ${total}`, `Left success    : ${done}`, `Failed          : ${failed}`];
  ...
  if (feature === "remove_members") {
    ...
    const removeBoxItems = details.slice(0, 30).map(d => {
      const m = d.match(/✅ (.+): (\d+) removed/); return m ? `${m[1]}: ${m[2]} members removed` : d;
    });
    if (details.length > 30) removeBoxItems.push(`...and ${details.length - 30} more`);
    const extraLines = [
      `Groups selected : ${total}`,
      `Members removed : ${totalRem}`,
      makeBox(removeBoxItems),
    ];
    await sendSummary(ctx, { feature, total, success: done, failed, cancelled, extra: extraLines });
    updateSession(uid, { featureFlow: null }); return;
  }
  ...
  if (feature === "make_admin") {
    ...
    const adminBoxItems = details.slice(0, 25).map(d => {
      const m = d.match(/✅ (.+): (\d+) promoted/); return m ? `${m[1]}: ${m[2]} promoted` : d.replace(/[*✅⚠️❌]/g, '').trim();
    });
    if (details.length > 25) adminBoxItems.push(`...and ${details.length - 25} more`);
    const extraLines = [
      `Groups       : ${total}`,
      `Admin number : ${extraNums.map(n => '+' + n).join(', ')}`,
      `Total promoted : ${totalProm}`,
      makeBox(adminBoxItems),
    ];
    await sendSummary(ctx, { feature, total, success: done, failed, cancelled, extra: extraLines });
    updateSession(uid, { featureFlow: null }); return;
  }
  ...
  if (feature === "demote_admin") {
    ...
    const demoteBoxItems = details.slice(0, 25).map(d => {
      const m = d.match(/✅ (.+): (\d+) demoted/); return m ? `${m[1]}: ${m[2]} demoted` : d.replace(/[*✅⚠️❌⬇️]/g, '').trim();
    });
    if (details.length > 25) demoteBoxItems.push(`...and ${details.length - 25} more`);
    const extraLines = [
      `Groups         : ${total}`,
      `Admin number   : ${extraNums.map(n => '+' + n).join(', ')}`,
      `Total demoted  : ${totalDem}`,
      makeBox(demoteBoxItems),
    ];
    await sendSummary(ctx, { feature, total, success: done, failed, cancelled, extra: extraLines });
    updateSession(uid, { featureFlow: null }); return;
  }
  ...
  if (feature === "reset_link") {
    ...
    // Send ALL new links in box-format chunks of 30
    const CHUNK = 30;
    for (let c = 0; c < results.length; c += CHUNK) {
      const chunk = results.slice(c, c + CHUNK);
      const part  = Math.floor(c / CHUNK) + 1;
      ...
      const header = parts > 1
        ...
        : `🔄 *New Links*\n━━━━━━━━━━━━━━━━━━━━\nGroups selected : ${total}  ✅ Success : ${done}\n━━━━━━━━━━━━━━━━━━━━\n`;
      await ctx.reply(header + makeBox(boxItems), { parse_mode: "Markdown" });
      if (c + CHUNK < results.length) await sleep(500);
    }
    if (fails.length) {
    ...
    await sendSummary(ctx, { feature: "reset_link", total, success: done, failed, cancelled,
      extra: [`Groups selected : ${total}`, `Success         : ${done}`, `Failed          : ${failed}`] });
    updateSession(uid, { featureFlow: null }); return;
  }
  ...
  if (feature === "approval") {
    ...
    const approvalBoxItems = details.slice(0, 30).map(d =>
      d.replace(/[*🔒🔓❌]/g, '').trim()
    );
    if (details.length > 30) approvalBoxItems.push(`...and ${details.length - 30} more`);
    const extraLines = [
      `Groups selected : ${total}`,
      `Success         : ${done}`,
      makeBox(approvalBoxItems),
    ];
    await sendSummary(ctx, { feature, total, success: done, failed, cancelled, extra: extraLines });
    updateSession(uid, { featureFlow: null }); return;
  }
  ...
  if (feature === "approve_pending") {
    ...
    const pendingBoxItems = details.slice(0, 20).map((d, idx) => {
      const m = d.match(/• *(.+)*: (\d+) pending → (\d+) approved/);
      return m ? `${idx + 1}.${m[1]}: ${m[3]} member add` : d.replace(/[*•❌]/g, '').trim();
    });
    if (details.length > 20) pendingBoxItems.push(`...and ${details.length - 20} more`);
    const extraLines = [
      `Groups selected : ${total}`,
      `Total approved  : ${totApproved}`,
      makeBox(pendingBoxItems),
    ];
    await sendSummary(ctx, { feature, total, success: done, failed, cancelled, extra: extraLines });
    updateSession(uid, { featureFlow: null }); return;
  }
  ...
  if (feature === "member_list") {
    ...
    // Send member list in box-format chunks
  ...
  if (feature === "pending_list") {
    ...
    // Send pending list in box-format chunks
    const listLines = rows.map((r) => r.ok
      ? (r.count > 0 ? `${r.name}: ${r.count} pending request` : `${r.name}: no pending`)
      : `${r.name}: error`);
    const CHUNK = 50;
    for (let c = 0; c < listLines.length; c += CHUNK) {
      const chunk = listLines.slice(c, c + CHUNK);
    ...
    await sendSummary(ctx, { feature: "pending_list", total, success: done, failed, cancelled,
      extra: [`Total groups  : ${total}`, `Total pending : ${grandPending}`] });
    updateSession(uid, { featureFlow: null }); return;
  }
}
...
bot.action("gf_create_now",async(ctx)=>{
  ...
  const cgBoxItems = [];
  for (const g of created.slice(0, 25)) { cgBoxItems.push(g.name); cgBoxItems.push(g.link); cgBoxItems.push(""); }
  while (cgBoxItems.length && !cgBoxItems[cgBoxItems.length - 1]) cgBoxItems.pop();
  const extraLines = cgBoxItems.length ? [makeBox(cgBoxItems.map(l => l))] : [];
  if (created.length > 25) extraLines.push(`...and ${created.length - 25} more`);
...
async function runAddMembersFromVcfs(ctx) {
  ...
  const amBoxItems = summaryLines.slice(0, 20).map(d => {
    const m = d.match(/✅ *(.+)*: +(d+)/); return m ? `${m[1]}: ${m[2]} members added` : d.replace(/[*✅❌⏭]/g, '').trim();
  });
  if (summaryLines.length > 20) amBoxItems.push(`...and ${summaryLines.length - 20} more`);
  const extraLines = [
    `Groups selected : ${total}`,
...
bot.on("text", async (ctx) => {
  ...
  if (s.joinFlow?.step === "links") {
    ...
    const jgExtra = [
      `Total links  : ${codes.length}`,
      `Joined       : ${joined}`,
      `Failed       : ${failed}`,
    ];
    if (failedLinks.length) jgExtra.push(makeBox(failedLinks.slice(0, 20)));
    await sendSummary(ctx,{feature:"join_groups",total:codes.length,success:joined,failed,cancelled,extra:jgExtra});
    return;
  }
...
async function runCtcChecker(ctx) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      if (unknownCount > 0) {
        // We know HOW MANY are unknown but can't list their phones (they're LID-only in WA)
        allUnknown.push(...verifiedPhones); // track verified phones globally
        reportLines.push(
          `⚠️ *${info.name}*: ${pendingCount} pending — ` +
          `${verifiedCount} trusted, ${unknownCount} unknown (not in VCF)`
        );
      } else if (pendingCount > 0) {
        reportLines.push(`✅ *${info.name}*: ${pendingCount} pending — all trusted`);
      } else {
        reportLines.push(`✅ *${info.name}*: no pending requests`);
      }
  ...
  if (reportLines.length > 0) {
    ...
    for (let c = 0; c < reportLines.length; c += CHUNK) {
      ...
      // Format each line: remove markdown bold/emoji, produce clean "GroupName: Valid X member, Y wrong contact number"
      const boxItems = chunk.map(l => {
        // "✅ GroupName: N pending — all trusted"
        // "⚠️ GroupName: N pending — V trusted, U unknown"
        const m1 = l.match(/✅ *(.+)*: (d+) pending — all trusted/);
        if (m1) return `${m1[1]}: Valid ${m1[2]} member`;
        const m2 = l.match(/⚠️ *(.+)*: (d+) pending — (d+) trusted, (d+) unknown/);
        if (m2) return `${m2[1]}: Valid ${m2[3]} member, ${m2[4]} wrong contact number`;
        const m3 = l.match(/✅ *(.+)*: no pending/);
        if (m3) return `${m3[1]}: no pending request`;
        return l.replace(/[*✅⚠️❌]/g, '').trim();
      });
      const hdr = c === 0
        ? `🔍 *CTC Report*\n━━━━━━━━━━━━━━━━━━━━\nGroups checked : ${done}\n━━━━━━━━━━━━━━━━━━━━\n`
        : `🔍 *CTC Report (cont. ${part}/${parts})*\n━━━━━━━━━━━━━━━━━━━━\n`;
      await ctx.reply(hdr + makeBox(boxItems), { parse_mode: "Markdown" });
  ...
  await sendSummary(ctx, { feature: "ctc_checker", total, success: done, failed, cancelled,
    extra: [`Groups checked : ${done}`, `Failed         : ${failed}`] });
  updateSession(uid, { featureFlow: null, awaitingVcf: null });
}
