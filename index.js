...
bot.action("cn_vcf", async (ctx) => {
  ...
  await reply(ctx,
    `📛 *Change Name — Match VCF Filename*\n━━━━━━━━━━━━━━━━━━━━\n\n*How it works:*\n• Send multiple VCF files at once\n• Bot scans ALL groups automatically\n• Groups whose *pending requests* match VCF contacts get renamed to that VCF's filename\n• Best-matching VCF wins per group\n\n*No group links needed!*\n\n📎 *Send all VCF files now:*`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu", "back_menu")]]) }
  );
});
 ...
 *
 * Matching logic (pending-only, LID-aware):
 *  1. Fetch pending requests for each group (ONLY pending — not existing members)
 *  2. Filter to real phone numbers only (hasRealPhone=true, ignores LID-only entries)
 *  3. Find the VCF whose contacts have the most matches in pending
...
async function runChangeNameAsVcfAuto(ctx, vcfList) {
  ...
  for (let i = 0; i < totalGroups; i++) {
    ...
    try {
      // ── Collect ONLY pending request phone numbers from this group ─────
      // We only check pending requests (not existing members) because the
      // user wants to rename groups based on who is waiting to join.
      ...
      // ── Find best-matching VCF (most pending numbers found in VCF) ─────
      let bestVcf   = null;
      let bestCount = 0;
      ...
      if (bestVcf && bestCount > 0) {
        ...
        renamed++;
        details.push(`✅ *${g.name}* → *${bestVcf.name}* (${bestCount} pending match${bestCount > 1 ? "es" : ""})`);
      } else {
        skipped++;
      }
    } catch (err) {
...
bot.on("text", async (ctx) => {
  ...
  // CTC Checker links → then collect ALL VCFs at once
  if (s.featureFlow?.step === "ctc_links") {
    ...
    await ctx.reply(
      `🔍 *CTC Checker — ${codes.length} group(s)*\n━━━━━━━━━━━━━━━━━━━━\n*Step 2:* Send ALL ${codes.length} VCF files at once.\n\n📌 Order matters:\nVCF 1 → Group 1\nVCF 2 → Group 2 ...and so on\n\n📎 *Send all VCFs now:*`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu","back_menu")]]) }
    );
    return;
  }
...
bot.on("document", async (ctx) => {
  ...
  // ── CTC Checker VCF — collect ALL VCFs at once (each file = one group in order) ──
  if (awaitingVcf?.feature === "ctc_checker" && s.featureFlow?.step === "ctc_vcf_collecting") {
    ...
    try {
      ...
      if (received < total) {
        // Still waiting for more VCFs — just accumulate silently with a count update
        updateSession(uid, { featureFlow: { ...flow, vcfList: newList, ctcVcfIdx: received } });
        await ctx.reply(
          `✅ *VCF ${received}/${total} received!* (${contacts.length} numbers)\n📎 *Send VCF ${received + 1}/${total} now* — or send all remaining at once:`,
          { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🏠 Main Menu","back_menu")]]) }
        );
      } else {
        // All VCFs collected — start checking immediately
        updateSession(uid, { featureFlow: { ...flow, vcfList: newList, ctcVcfIdx: received, step: "ctc_running" }, awaitingVcf: null });
