...
// For each number:
//   1. Check if already in group (uses cachedParticipants first, then fresh metadata)
...
async function makeAdminByNumbers(index, gid, phones, cachedParticipants = null) {
  ...
  // Use cached participants if provided, otherwise fetch fresh from API
  let currentParticipants = (cachedParticipants && cachedParticipants.length > 0)
    ? cachedParticipants
    : null;
  ...
  if (!currentParticipants) {
    try {
      const meta = await s.groupMetadata(gid);
      currentParticipants = meta.participants || [];
    } catch (e) {
      console.error("[makeAdmin] groupMetadata error:", e.message);
      currentParticipants = [];
    }
  }
  ...
  for (const phone of phones) {
    ...
    try {
      // ── Step 1: Look for this number in the participants list ──
      const member = currentParticipants.find(
      ...
      if (member) {
        ...
        // ── Step 2: Not in cached list → fetch fresh metadata to double-check ──
        let freshMember = null;
        try {
          const freshMeta = await s.groupMetadata(gid);
          currentParticipants = freshMeta.participants || currentParticipants;
          freshMember = currentParticipants.find(
            (p) => normJid(p.id) === normJid(baseJid)
          );
        } catch {}

        if (freshMember) {
          await s.groupParticipantsUpdate(gid, [freshMember.id], "promote").catch((e) =>
            console.error("[makeAdmin] promote error:", e.message)
          );
          promoted++;
        } else {
          // ── Step 3: Not in group → check pending requests ──
          let pendingJid = null;
          try {
          ...
          if (pendingJid) {
            ...
            // Wait for WhatsApp to process the approval (retry up to 3 times, 4s apart)
            let jidToUse = pendingJid;
            for (let attempt = 0; attempt < 3; attempt++) {
              await new Promise((r) => setTimeout(r, 4000));
              try {
                const newMeta = await s.groupMetadata(gid);
                currentParticipants = newMeta.participants || currentParticipants;
                const newMember = currentParticipants.find(
                  (p) => normJid(p.id) === normJid(baseJid)
                );
                if (newMember) { jidToUse = newMember.id; break; }
              } catch {}
            }

            // Now promote — use found real JID or fall back to pendingJid
...
// For each number:
//   1. Search them in group participants (uses cachedParticipants first, then fresh metadata)
...
async function demoteAdminInGroup(index, gid, phones, cachedParticipants = null) {
  ...
  if (!currentParticipants) {
    try {
      ...
      console.error("[demoteAdmin] groupMetadata error:", e.message);
      currentParticipants = [];
    }
  }
  ...
  for (const phone of phones) {
    ...
    try {
      // ── Step 1: Search in the participants list ──
      let member = currentParticipants.find(
        (p) => normJid(p.id) === normJid(baseJid)
      );

      // If not found in cached list → fetch fresh metadata to double-check
      if (!member) {
        try {
          const freshMeta = await s.groupMetadata(gid);
          currentParticipants = freshMeta.participants || currentParticipants;
          member = currentParticipants.find(
            (p) => normJid(p.id) === normJid(baseJid)
          );
        } catch {}
      }
