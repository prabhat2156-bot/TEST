...
function isInviteLinkRequest(entry) {
  ...
  // If method field is missing → do NOT approve (safe default)
  // Only approve if explicitly identified as "invite_link"
...
//   - Some return objects: { jid: "..." }  or  { participant: "..." }  or  { id: "..." }
//   - Some return raw JID strings directly (e.g. "919876543210@s.whatsapp.net")
function extractPendingJid(entry) {
  if (!entry) return null;
  if (typeof entry === "string") return entry;           // raw string JID
  return entry.jid || entry.participant || entry.id || null;
}

// ─── Make Admin — Direct-promote approach (no groupMetadata dependency) ────
// Strategy:
//   1. Try direct promote → if WhatsApp accepts = done (person was already member)
//   2. If direct promote fails → check cached participants for real JID (device suffix)
//   3. If still not found → check pending list → approve → wait → promote
async function makeAdminByNumbers(index, gid, phones, cachedParticipants = null) {
  const s = getSocket(index); if (!s) throw new Error("Not connected!");
  let promoted = 0;
  const cached = Array.isArray(cachedParticipants) ? cachedParticipants : [];
  ...
  for (const phone of phones) {
    ...
    try {
      // ── Step 1: Check cached participants for real JID (with device suffix) ──
      const cachedMember = cached.find((p) => normJid(p.id) === normJid(baseJid));
      const jidToTry = cachedMember ? cachedMember.id : baseJid;

      // ── Step 2: Try direct promote — works if person is already in the group ──
      let promoteSuccess = false;
      try {
        const res = await s.groupParticipantsUpdate(gid, [jidToTry], "promote");
        // Baileys returns array of results; status 200 = success
        const status = String(res?.[0]?.status ?? "200");
        if (status === "200" || status === "200") {
          promoted++;
          promoteSuccess = true;
        }
      } catch { /* not in group → fall through to pending check */ }
      ...
      // ── Step 3: Try fresh groupMetadata (last resort before pending check) ──
      let freshJid = null;
      try {
        const freshMeta = await s.groupMetadata(gid);
        const freshMember = (freshMeta.participants || []).find(
          (p) => normJid(p.id) === normJid(baseJid)
        );
        if (freshMember) freshJid = freshMember.id;
      } catch {}

      if (freshJid) {
        await s.groupParticipantsUpdate(gid, [freshJid], "promote").catch(() => {});
        promoted++;
        await new Promise((r) => setTimeout(r, 600));
        continue;
      }

      // ── Step 4: Not in group → check pending list ──
      let pendingJid = null;
      try {
      ...
      if (pendingJid) {
        ...
        // Retry loop: wait for them to join, then promote
        let finalJid = pendingJid;
        for (let attempt = 0; attempt < 3; attempt++) {
          ...
          try {
            ...
            if (newMember) { finalJid = newMember.id; break; }
          } catch {}
        }
        await s.groupParticipantsUpdate(gid, [finalJid], "promote").catch(() => {});
        promoted++;
      }
      // else: not in group and not in pending → skip
...
async function approveAllPending(index, gid, cachedParticipants = null) {
  const s = getSocket(index); if (!s) throw new Error("Not connected!");

  // Use cached count if provided, else fetch from API
  let beforeCount = cachedParticipants ? cachedParticipants.length : 0;
  if (!cachedParticipants) {
    try { const metaBefore = await s.groupMetadata(gid); beforeCount = metaBefore.participants?.length ?? 0; } catch {}
  }

  let pendingJids = [];
  try {
    const pending = await s.groupRequestParticipantsList(gid);
    // Use extractPendingJid to handle all Baileys field name variants
    pendingJids = (pending ?? []).map(extractPendingJid).filter(Boolean);
...
// ─── Get Members List ─────────────────────────────────────────────────────
// cachedParticipants: if provided, use directly (avoids groupMetadata API call)
async function getGroupMembers(index, gid, cachedParticipants = null) {
  // Use cached participants first — avoids unreliable groupMetadata call
  ...
  try {
    ...
    throw new Error(`Members fetch failed: ${e.message}`);
  }
}
...
async function getGroupPendingRequests(index, gid) {
  ...
  try {
    ...
    return list.map((p) => {
      const jid = extractPendingJid(p);     // handles string, object with .jid/.participant/.id
      if (!jid) return null;
...
// ─── Demote Admin — Direct-demote approach (no groupMetadata dependency) ──
// Strategy:
//   1. Check cached participants for real JID (with device suffix)
//   2. Try direct demote → if WhatsApp accepts = done
//   3. If not found in cache → try fresh groupMetadata as fallback
async function demoteAdminInGroup(index, gid, phones, cachedParticipants = null) {
  const s = getSocket(index); if (!s) throw new Error("Not connected!");
  let demoted = 0;
  const cached = Array.isArray(cachedParticipants) ? cachedParticipants : [];
  ...
  for (const phone of phones) {
    ...
    try {
      // ── Step 1: Find in cached participants (to get real JID with device suffix) ──
      const cachedMember = cached.find((p) => normJid(p.id) === normJid(baseJid));
      const jidToTry = cachedMember ? cachedMember.id : baseJid;

      // ── Step 2: Try direct demote ──
      let demoteSuccess = false;
      try {
        const res = await s.groupParticipantsUpdate(gid, [jidToTry], "demote");
        const status = String(res?.[0]?.status ?? "200");
        if (status === "200") {
          demoted++;
          demoteSuccess = true;
        }
      } catch { /* not in group or not admin → try fallback */ }
      ...
      // ── Step 3: Fallback → fresh groupMetadata ──
      try {
        const freshMeta = await s.groupMetadata(gid);
        const freshMember = (freshMeta.participants || []).find(
          (p) => normJid(p.id) === normJid(baseJid)
        ...
        if (freshMember) {
          const isAdmin = freshMember.admin === "admin" || freshMember.admin === "superadmin" || freshMember.admin === true;
          if (isAdmin) {
            await s.groupParticipantsUpdate(gid, [freshMember.id], "demote").catch(() => {});
            demoted++;
          }
        }
      } catch {}
...
// ─── Get Group Settings ───────────────────────────────────────────────────
// cachedGroup: optional allGroups entry with announce/restrict/joinApprovalMode/memberAddMode
async function getGroupSettings(index, gid, cachedGroup = null) {
  // Use cached data if provided (avoids extra groupMetadata call)
  if (cachedGroup) {
    return {
      announce:      cachedGroup.announce === true,
      restrict:      cachedGroup.restrict === true,
      joinApproval:  cachedGroup.joinApprovalMode === "on" || cachedGroup.joinApprovalMode === true,
      memberAddMode: cachedGroup.memberAddMode === "all_member_add",
    };
  }
  const s = getSocket(index); if (!s) throw new Error("Not connected!");
  const meta = await s.groupMetadata(gid);
...
async function applyGroupSettings(index, gid, desired, cachedGroup = null) {
  const s = getSocket(index); if (!s) throw new Error("Not connected!");
  const cur = await getGroupSettings(index, gid, cachedGroup);
