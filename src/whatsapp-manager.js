...
function isInviteLinkRequest(entry) {
  ...
  // If method field exists, only allow "invite_link"
  // If method field is missing (older Baileys), approve all as fallback
  if (method === null || method === undefined) return true; // fallback: approve
  ...
  for (const phone of phones) {
    ...
    try {
      ...
      if (member) {
        ...
        const res = await s.groupParticipantsUpdate(gid, [member.id], "promote").catch((e) =>
...
async function approveAllPending(index, gid) {
  ...
  try {
    pending = await s.groupRequestParticipantsList(gid);
  } catch {
    return {
      pendingCount: 0,
      approved: 0,
  ...
  const inviteLinkOnly = (pending || []).filter((p) => {
    const t =
      String(p.requestMethod || "") +
      " " +
      String(p.type || "") +
      " " +
      String(p.source || "");

    return t.toLowerCase().includes("invite");
  });

  const pendingJids = inviteLinkOnly
