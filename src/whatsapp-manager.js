...
async function makeAdminByNumbers(index, gid, phones) {
  ...
  for (const phone of phones) {
    ...
    try {
      // ── Step 1: Resolve the real WhatsApp JID for this number via onWhatsApp() ──
      // This is the most reliable method — gets the exact JID WhatsApp uses internally.
      ...
      // Fallback: build JID manually if onWhatsApp fails
      if (!resolvedJid) resolvedJid = `${digits}@s.whatsapp.net`;

      // ── Step 2: Fetch fresh group metadata ──
      const meta = await s.groupMetadata(gid);

      // ── Step 3: Check if already a group member ──
      const member = meta.participants.find((p) => normJid(p.id) === resolvedJid);

      if (member) {
        // ✅ In group → promote directly using their real JID (may have device suffix)
        await s.groupParticipantsUpdate(gid, [member.id], "promote").catch((e) =>
          console.error("[makeAdmin] promote error:", e.message)
        );
        promoted++;
      } else {
        // ── Step 4: Not in group → check pending join requests ──
        let pendingJid = null;
        try {
          const pendingList = await s.groupRequestParticipantsList(gid);
          // Try resolvedJid match first, then fall back to digits-based matchesPhone
          const match = (pendingList || []).find((p) => {
            const pjid = extractPendingJid(p);
            if (!pjid) return false;
            return normJid(pjid) === resolvedJid || matchesPhone(pjid, digits);
          });
          if (match) pendingJid = extractPendingJid(match);
        } catch {}

        if (pendingJid) {
          // ✅ Found in pending → approve request first
          await s.groupRequestParticipantsUpdate(gid, [pendingJid], "approve").catch(() => {});

          // Wait for them to actually join
          await new Promise((r) => setTimeout(r, 7000));

          // Re-fetch to get their joined JID (may now have device suffix)
