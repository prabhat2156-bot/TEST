...
/**
 * Returns true ONLY for "requested to join" entries — person themselves
 * joined via invite link. Explicitly rejects "requested to add" entries
 * where someone else tried to add them (non_admin_add).
 *
 * WhatsApp method values:
 *   "invite_link"   → "+91 XXXXX requested to join"  ✅ accept
 *   "non_admin_add" → "+91 XXXXX requested to add +91 YYYYY" ❌ skip
 *   null/undefined  → old Baileys format, treated as invite_link ✅ accept
 */
function isSelfJoinRequest(entry) {
  const method = entry.method ?? entry.requestMethod ?? null;
  // Explicitly reject "requested to add" (someone else tried to add them)
  if (method === "non_admin_add") return false;
  // Accept only self-join: invite_link or null (old API format = invite_link)
  ...
  autoAcceptTimer = setInterval(async () => {
    ...
    for (const gid of active) {
      ...
      try {
        ...
        // Only accept "requested to join" — skip "requested to add"
        const selfJoinOnly = (pending || []).filter(isSelfJoinRequest);
        const jids = selfJoinOnly.map((p) => p.jid || p.id).filter(Boolean);
        if (jids.length) {
