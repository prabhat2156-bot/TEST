...
// ─── Get Pending for CTC checker (LID-aware) ─────────────────────────────
async function getPendingForGroup(index, gid) {
  ...
  try {
    ...
    return (pending ?? []).map((p) => {
      ...
      // hasRealPhone = true means we have an actual phone number (not a LID)
      return { jid: p.jid || p.id, phone, hasRealPhone: !!phoneJid };
    });
  } catch { return []; }
}
