...
async function makeAdminByNumbers(index, gid, phones) {
  ...
  for (const phone of phones) {
    ...
    try {
      ...
      // ── Step 4: Approve ONLY the exactly matched pending JID ─────────────
      // Never approve all pending — that would promote wrong people.
      if (!matchedPendingJid) {
        // Person not found in group and not in pending requests — skip
        continue;
      }

      await s.groupRequestParticipantsUpdate(gid, [matchedPendingJid], "approve").catch(() => {});
