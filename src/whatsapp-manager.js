...
function findGroupMember(participants = [], number = "") {
  const clean = normalizeNumber(number);

  return participants.find((p) => {
    const jid = extractParticipantJid(p) || "";
    return normalizeNumber(jid).includes(clean);
  });
}
...
//   (contacts show up as "contact" or no method)
function isInviteLinkRequest(entry) {
  const txt = JSON.stringify(entry || {}).toLowerCase();

  // only approve actual invite-link joins
  return (
    txt.includes("invite") ||
    txt.includes("invite_link") ||
    txt.includes("group_invite")
  );
}
  ...
  for (const phone of phones) {
    ...
    try {
      ...
      if (member) {
        ...
        await s.groupParticipantsUpdate(gid, [extractParticipantJid(member)], "promote").catch((e) =>
        ...
        if (pendingJid) {
          ...
          try {
            ...
            await s.groupParticipantsUpdate(gid, [extractParticipantJid({jid: jidToUse})], "promote").catch((e) =>
...
async function demoteAdminInGroup(index, gid, phones) {
  ...
  for (const phone of phones) {
    ...
    try {
      ...
      await s.groupParticipantsUpdate(gid, [extractParticipantJid(member)], "demote").catch((e) =>
