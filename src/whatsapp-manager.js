 ...
 *
 * Har user ka apna alag WhatsApp socket hota hai.
 * accountId = "wa_<telegramId>" format mein.
 *
 * Memory optimization:
 *  - getMessage: () => undefined  (message store disabled — saves ~100MB RAM per session)
 *  - syncFullHistory: false
 *  - generateHighQualityLinkPreview: false
 *
 * 6hr auto-logout: index.js mein setInterval se handle hota hai.
 */
...
// accountId → { socket, status, phoneNumber, callbacks }
const sockets = new Map();

// Global callbacks (per-user overrides possible)
...
function setCallbacks(opts) {
  if (opts.onPairingCode)  globalCallbacks.onPairingCode  = opts.onPairingCode;
  if (opts.onReady)        globalCallbacks.onReady        = opts.onReady;
  if (opts.onDisconnected) globalCallbacks.onDisconnected = opts.onDisconnected;
}

function getEntry(accountId) {
  if (!sockets.has(accountId)) {
    sockets.set(accountId, { socket: null, status: "disconnected", phoneNumber: "" });
  }
  return sockets.get(accountId);
}

function getStatus(accountId)  { return getEntry(accountId).status; }
function getPhone(accountId)   { return getEntry(accountId).phoneNumber; }
function getSocket(accountId)  {
  const e = getEntry(accountId);
  if (!e.socket || e.status !== "connected") return null;
  return e.socket;
}
...
// ─── Auto Accept ──────────────────────────────────────────────────────────
// Per-accountId auto accept maps
const autoAcceptMaps  = new Map(); // accountId → Map<gid, {accepted}>
const autoAcceptTimers = new Map(); // accountId → timer
...
function isSelfJoinRequest(entry) {
  ...
  if (
    entry.addedBy    || entry.adder      || entry.requestedBy ||
    entry.addRequest || entry.add_request|| entry.addedByJid  ||
    entry.addRequestJid
  ) return false;
  return method === "invite_link" || method === null || method === undefined;
}

function startAutoAcceptForGroups(accountId, groupIds) {
  if (!autoAcceptMaps.has(accountId)) autoAcceptMaps.set(accountId, new Map());
  const groupMap = autoAcceptMaps.get(accountId);
  groupIds.forEach((gid) => groupMap.set(gid, { accepted: 0 }));
  if (autoAcceptTimers.has(accountId)) return;

  const timer = setInterval(async () => {
    const active = [...groupMap.keys()];
    if (!active.length) {
      clearInterval(timer);
      autoAcceptTimers.delete(accountId);
      return;
    }
    const s = getSocket(accountId);
    if (!s) return;
  ...
  autoAcceptTimers.set(accountId, timer);
}

function stopAutoAcceptForGroups(accountId, groupIds) {
  const groupMap = autoAcceptMaps.get(accountId);
  if (!groupMap) return;
  groupIds.forEach((gid) => groupMap.delete(gid));
  if (groupMap.size === 0) {
    const t = autoAcceptTimers.get(accountId);
    if (t) { clearInterval(t); autoAcceptTimers.delete(accountId); }
  }
}

function getAutoAcceptStats(accountId, groupIds) {
  const groupMap = autoAcceptMaps.get(accountId) || new Map();
  const result = {};
  groupIds.forEach((gid) => { result[gid] = groupMap.get(gid) || { accepted: 0 }; });
  return result;
}

// ─── Connection ───────────────────────────────────────────────────────────
async function connectAccount(accountId, phoneNumber, freshStart = true) {
  const entry = getEntry(accountId);
  if (entry.socket) { try { entry.socket.end(undefined); } catch {} entry.socket = null; }

  if (freshStart) await clearMongoAuth(accountId);
  ...
  const socket = makeWASocket({
    ...
    // Disables in-memory message store — saves ~100MB RAM per session
  ...
  socket.ev.on("connection.update", async (update) => {
    ...
    if (qr && !pairingRequested) {
      ...
      await _requestPairingWithRetry(socket, accountId, clean);
    }
    if (connection === "open") {
      entry.status = "connected";
      await globalCallbacks.onReady(accountId);
    }
    if (connection === "close") {
      ...
      if (code === DisconnectReason.loggedOut) {
        ...
        entry.phoneNumber = "";
        await globalCallbacks.onDisconnected(accountId);
      } else if (entry.phoneNumber) {
        setTimeout(() => connectAccount(accountId, entry.phoneNumber, false).catch(() => {}), 5000);
      }
    }
  });
}

async function _requestPairingWithRetry(socket, accountId, clean, attempt = 1) {
  try {
    const code = await socket.requestPairingCode(clean);
    if (code) {
      const formatted = code.replace(/[^A-Z0-9]/gi, "").match(/.{1,4}/g)?.join("-") ?? code;
      await globalCallbacks.onPairingCode(accountId, formatted);
    }
  } catch {
    if (attempt < 3) {
      await new Promise((r) => setTimeout(r, 4000));
      await _requestPairingWithRetry(socket, accountId, clean, attempt + 1);
    } else {
      await globalCallbacks.onPairingCode(accountId, null);
    }
  }
}
...
async function reconnectSavedAccount(accountId, phoneNumber) {
  if (!phoneNumber) return;
  await connectAccount(accountId, phoneNumber, false).catch(() => {});
}

// ─── Group utilities ──────────────────────────────────────────────────────
async function getAllGroupsWithDetails(accountId) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
...
async function createGroup(accountId, name, jids) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  return await s.groupCreate(name, jids);
}
async function updateGroupDescription(accountId, gid, desc) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  await s.groupUpdateDescription(gid, desc);
}
async function updateGroupPhoto(accountId, gid, buf) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  await s.updateProfilePicture(gid, buf);
}
async function setDisappearingMessages(accountId, gid, sec) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  await s.groupToggleEphemeral(gid, sec);
}
async function promoteToAdmin(accountId, gid, jids) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  await s.groupParticipantsUpdate(gid, jids, "promote");
}
async function getGroupInviteLink(accountId, gid) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  const c = await s.groupInviteCode(gid);
  return `https://chat.whatsapp.com/${c}`;
}
async function joinGroupViaLink(accountId, code) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  return await s.groupAcceptInvite(code);
}
async function leaveGroup(accountId, gid) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  await s.groupLeave(gid);
}
async function getGroupInfoFromLink(accountId, code) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  try {
...
async function renameGroup(accountId, gid, newName) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  await s.groupUpdateSubject(gid, newName);
}
async function setGroupPermissions(accountId, gid, p) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
...
async function removeAllMembers(accountId, gid, batchSize = 5, skipAdmins = false) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  const meta  = await s.groupMetadata(gid);
...
async function getGroupMembers(accountId, gid) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  const meta = await s.groupMetadata(gid);
  return (meta.participants || []).map((p) => {
    const number = extractPhoneNumber(p);
    return { id: p.id, jid: p.id, number, phone: number,
      admin: p.admin === "admin" || p.admin === "superadmin",
      superadmin: p.admin === "superadmin" };
  });
}

async function getGroupPendingRequests(accountId, gid) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  const parseEntry = (p, defaultMethod) => {
    ...
    const method = typeof p === "object" && p !== null
      ? (p.method || p.requestMethod || defaultMethod) : defaultMethod;
...
async function makeAdminByNumbers(accountId, gid, phones) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  let promoted = 0;
...
async function demoteAdminInGroup(accountId, gid, phones) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  let demoted = 0;
  for (const phone of phones) {
    const digits = phone.replace(/\D/g, "");
...
async function getGroupApprovalStatus(accountId, gid) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  const meta = await s.groupMetadata(gid);
  return meta.joinApprovalMode === "on" || meta.joinApprovalMode === true;
}
async function setGroupApproval(accountId, gid, enable) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  await s.groupJoinApprovalMode(gid, enable ? "on" : "off");
}

async function approveAllPending(accountId, gid) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
...
async function resetGroupInviteLink(accountId, gid) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
...
async function getGroupSettings(accountId, gid) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  const meta = await s.groupMetadata(gid);
  return {
...
async function applyGroupSettings(accountId, gid, desired) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
  const cur = await getGroupSettings(accountId, gid);
...
async function addMembersToGroup(accountId, gid, phones, oneByOne = false) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
...
async function getPendingRawJids(accountId, gid) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
...
async function resolveVcfPhones(accountId, phones) {
  const s = getSocket(accountId); if (!s) throw new Error("Not connected!");
