...
// Matches even when country code is partial / missing, or JID has device suffix.
// digits = raw numeric string from user input (e.g. "9876543210" or "919876543210")
function matchesPhone(jid, digits) {
  const norm = normJid(jid); // e.g. "919876543210@s.whatsapp.net"
  if (!norm.includes("@s.whatsapp.net")) return false; // skip LID / group JIDs
  const jidDigits = norm.replace("@s.whatsapp.net", "");
  // Exact match: user entered full number with country code
  if (jidDigits === digits) return true;
  // Suffix match: user entered local number without country code prefix
  // e.g. digits="9876543210", jidDigits="919876543210" → ends-with check
  if (jidDigits.endsWith(digits) && digits.length >= 10) return true;
  // Prefix match: JID digits are a suffix of what user entered
  // e.g. digits="919876543210" vs jidDigits="9876543210" (rare but possible)
  if (digits.endsWith(jidDigits) && jidDigits.length >= 10) return true;
  return false;
}
...
async function makeAdminByNumbers(index, gid, phones) {
  ...
  for (const phone of phones) {
    ...
    try {
      ...
      const meta   = await s.groupMetadata(gid);
      const member = meta.participants.find((p) => matchesPhone(p.id, digits));
      ...
      if (member) {
        ...
        try {
          ...
          const match = (pendingList || []).find((p) => {
            ...
            return pjid && matchesPhone(pjid, digits);
          });
        ...
        if (pendingJid) {
          ...
          try {
            ...
            const newMember = newMeta.participants.find((p) => matchesPhone(p.id, digits));
