...
function isInviteLinkRequest(entry) {
  const method =
    entry.method || entry.requestMethod || entry.addedByJid || null;
  ...
  autoAcceptTimer = setInterval(async () => {
    ...
    for (const gid of active) {
      ...
      try {
        ...
        const jids = inviteLinkOnly.map(extractPendingJid).filter(Boolean);
        if (jids.length) {
          await s
...
// ─── Make Admin ────────────────────────────────────────────────────────────
// 1. Check if already in group → promote directly
// 2. If not in group → check pending → approve → wait → promote
