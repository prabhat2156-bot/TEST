...
// ─── JID Normalization ────────────────────────────────────────────────────
// Handles all WhatsApp JID formats:
//   "919876543210@s.whatsapp.net"       standard
//   "+919876543210@s.whatsapp.net"      newer WA versions with + prefix
//   "919876543210:5@s.whatsapp.net"     multi-device suffix
function normJid(jid) {
  return (jid || "")
    .replace(/^\+/, "")        // strip leading + if present
    .replace(/:\d+@/, "@")     // strip device suffix :5@
    .toLowerCase()
    .trim();
}

// ─── Flexible phone-to-JID matching ──────────────────────────────────────
// Handles all JID formats across Baileys/WhatsApp versions:
//   "919876543210@s.whatsapp.net"       (standard)
//   "+919876543210@s.whatsapp.net"      (newer WA with + prefix)
//   "919876543210:5@s.whatsapp.net"     (device suffix — stripped by normJid)
// digits = raw numeric-only string from user input
function matchesPhone(jid, digits) {
  const norm = normJid(jid);
  // Accept both @s.whatsapp.net and any JID that contains digits — be permissive
  const atIdx = norm.indexOf("@");
  if (atIdx === -1) return false; // no domain at all → skip
  // Extract only the numeric part before "@" (strips +, spaces, dots, etc.)
  const jidDigits = norm.slice(0, atIdx).replace(/[^0-9]/g, "");
  if (!jidDigits) return false;
  // Exact match
  if (jidDigits === digits) return true;
  // Suffix match: user entered without country code (e.g. "9876543210" vs "919876543210")
  if (jidDigits.endsWith(digits) && digits.length >= 7) return true;
  // Prefix match: JID has fewer digits than user input (rare)
  if (digits.endsWith(jidDigits) && jidDigits.length >= 7) return true;
  return false;
}
...
// Helper: extract JID from a pending entry.
// Baileys versions differ: entry may be a plain JID string OR an object with .jid/.participant/.id
...
// Helper: check if pending request came from an invite link ONLY.
// WhatsApp pending entry method values:
...
// Logic:
//   1. If an explicit method field is present, use it directly.
//   2. If addedByJid is present (someone added them), it is NOT an invite-link request.
//   3. If no method and no addedByJid → most likely joined via link (no referrer).
function isInviteLinkRequest(entry) {
  // Plain string entries = just a JID, no method info → treat as invite link
  if (!entry || typeof entry === "string") return true;
  // Check all known method field names across Baileys versions
  const method = entry.method || entry.requestMethod || entry.requestJoinMethod || null;
  if (method !== null && method !== undefined) {
    return method === "invite_link";
  }
  // If someone explicitly added them, it is NOT an invite-link request
  if (entry.addedByJid) return false;
  // No method field and no addedByJid → joined via invite link (no referrer present)
  return true;
}
...
// Baileys versions differ: entry may be a plain JID string OR object with .jid/.participant/.id
...
async function makeAdminByNumbers(index, gid, phones) {
  ...
  for (const phone of phones) {
    ...
    try {
      // ── Step 1: Resolve canonical JID via onWhatsApp() ──────────────────
      let resolvedJid = null;
      try {
        const wa = await s.onWhatsApp(digits);
        if (wa && wa.length > 0 && wa[0].exists) resolvedJid = normJid(wa[0].jid);
      } catch {}
      if (!resolvedJid) resolvedJid = `${digits}@s.whatsapp.net`;

      // ── Step 2: Check if already a group member ──────────────────────────
      const meta   = await s.groupMetadata(gid);
      const member = meta.participants.find(
        (p) => normJid(p.id) === resolvedJid || matchesPhone(p.id, digits)
      );

      if (member) {
        // Already in group → promote directly
        await s.groupParticipantsUpdate(gid, [member.id], "promote").catch(() => {});
        promoted++;
        continue;
      }

      // ── Step 3: Not a member — work with pending join requests ───────────
      let allPendingJids = [];
      let matchedPendingJid = null;

      try {
        const pendingList = await s.groupRequestParticipantsList(gid);
        const entries = pendingList || [];

        // Collect ALL pending JIDs (handles string[] or object[] formats)
        allPendingJids = entries.map(extractPendingJid).filter(Boolean);

        // Try to find the specific person
        for (const entry of entries) {
          const pjid = extractPendingJid(entry);
          if (!pjid) continue;
          if (normJid(pjid) === resolvedJid || matchesPhone(pjid, digits)) {
            matchedPendingJid = pjid;
            break;
          }
        }
      } catch (e) {
        console.error("[makeAdmin] groupRequestParticipantsList error:", e.message);
      }

      // ── Step 4: Approve the matched pending JID ──────────────────────────
      // If exact match found → approve just them.
      // If no exact match but pending requests exist → approve ALL (last resort,
      //   handles cases where JID format is unknown/unreadable).
      const jidsToApprove = matchedPendingJid
        ? [matchedPendingJid]
        : allPendingJids.length > 0
          ? allPendingJids   // approve all pending as fallback
          : [];

      if (jidsToApprove.length === 0) {
        // Truly not in group and no pending requests
        continue;
      }

      await s.groupRequestParticipantsUpdate(gid, jidsToApprove, "approve").catch(() => {});

      // Wait for them to actually join
      await new Promise((r) => setTimeout(r, 7000));

      // ── Step 5: Re-fetch and promote ─────────────────────────────────────
      ...
      if (newMember) {
        await s.groupParticipantsUpdate(gid, [newMember.id], "promote").catch(() => {});
        promoted++;
      } else if (!matchedPendingJid && jidsToApprove.length > 0) {
        // We approved all pending as fallback — promote all newly joined
        const beforeJids = new Set(meta.participants.map((p) => normJid(p.id)));
        const newlyJoined = newMeta.participants.filter(
          (p) => !beforeJids.has(normJid(p.id))
        );
        for (const nj of newlyJoined) {
          await s.groupParticipantsUpdate(gid, [nj.id], "promote").catch(() => {});
          promoted++;
        }
      }
    } catch (e) {
...
async function approveAllPending(index, gid) {
  ...
  try {
    const pending = await s.groupRequestParticipantsList(gid);
    pendingJids = (pending ?? []).map((p) => getPendingJid(p)).filter(Boolean);
...
async function getGroupPendingRequests(index, gid) {
  ...
  try {
    ...
    return (list ?? []).map((p) => ({ jid: p.jid, phone: normJid(p.jid).replace("@s.whatsapp.net", "") }));
  } catch { return []; }
}
...
async function demoteAdminInGroup(index, gid, phones) {
  ...
  for (const phone of phones) {
    ...
    try {
      // ── Step 1: Resolve the real WhatsApp JID via onWhatsApp() ──
      let resolvedJid = null;
      try {
        const waResult = await s.onWhatsApp(digits);
        if (waResult && waResult.length > 0 && waResult[0].exists) {
          resolvedJid = normJid(waResult[0].jid);
        }
      } catch {}
      if (!resolvedJid) resolvedJid = `${digits}@s.whatsapp.net`;

      // ── Step 2: Fetch fresh metadata and find this member ──
      const meta   = await s.groupMetadata(gid);
      const member = meta.participants.find(
        (p) => normJid(p.id) === resolvedJid || matchesPhone(p.id, digits)
      );

      if (!member) continue; // Not in group — skip

      // ── Step 3: Must be admin to demote ──
      const isAdmin = member.admin === "admin" || member.admin === "superadmin" || member.admin === true;
      if (!isAdmin) continue; // In group but not an admin — skip

      // ── Step 4: Demote using their actual JID (with device suffix if present) ──
...
async function getPendingForGroup(index, gid) {
  ...
  try {
    const pending = await s.groupRequestParticipantsList(gid);
    return (pending ?? []).map((p) => ({ jid: p.jid, phone: normJid(p.jid).replace("@s.whatsapp.net", "") }));
  } catch { return []; }
}
