...
// ─── Make Admin (fixed JID normalization) ─────────────────────────────────
async function makeAdminByNumbers(index, gid, phones) {
  const s = getSocket(index); if (!s) throw new Error("Not connected!");

  // Build target JIDs from phone numbers
  const targetJids = phones.map((n) => `${n.replace(/[^0-9]/g, "")}@s.whatsapp.net`);
  const targetNorm = targetJids.map(normJid);

  // Fetch group metadata
  const meta = await s.groupMetadata(gid);

  // Find members whose normalized JID matches targets
  // (handles "919876543210:5@s.whatsapp.net" case)
  const memberMatches = meta.participants
    .filter((p) => targetNorm.includes(normJid(p.id)))
    .map((p) => p.id); // use actual JID from group (with device suffix if present)

  // Approve pending requests that match targets
  let approvedJids = [];
  try {
    const pendingList = await s.groupRequestParticipantsList(gid);
    const pendingMatches = (pendingList || []).filter((p) => targetNorm.includes(normJid(p.jid)));
    if (pendingMatches.length) {
      const toApprove = pendingMatches.map((p) => p.jid);
      await s.groupRequestParticipantsUpdate(gid, toApprove, "approve").catch(() => {});
      approvedJids = toApprove;
      // Wait longer for approval to process and users to join (increased from 2500 to 6000ms)
      await new Promise((r) => setTimeout(r, 6000));
      // Re-check they joined — if still not in meta, still try to promote them
      // (some WA versions allow promoting before the metadata propagates)
      try {
        const newMeta = await s.groupMetadata(gid);
        const newNorm = newMeta.participants.map((p) => normJid(p.id));
        const joined = approvedJids.filter((j) => newNorm.includes(normJid(j)));
        // Keep approved JIDs even if not yet visible in metadata (promote anyway)
        approvedJids = approvedJids; // try all approved, not just confirmed joined
      } catch {}
    }
  } catch {}

  // Combine: already members + newly approved
  const allToPromote = [...new Set([...memberMatches, ...approvedJids])];
  if (allToPromote.length) {
    await s.groupParticipantsUpdate(gid, allToPromote, "promote").catch((e) => {
      console.error("[makeAdmin] promote error:", e.message);
    });
  }
  return allToPromote.length;
}
...
// ─── Demote Admin (fixed JID normalization) ────────────────────────────────
async function demoteAdminInGroup(index, gid, phones) {
  const s = getSocket(index); if (!s) throw new Error("Not connected!");

  const targetNorm = phones.map((n) => normJid(`${n.replace(/[^0-9]/g, "")}@s.whatsapp.net`));
  const meta = await s.groupMetadata(gid);

  // Find current admins whose normalized JID matches targets
  const toDemote = meta.participants
    .filter((p) => p.admin && targetNorm.includes(normJid(p.id)))
    .map((p) => p.id); // use actual JID (with device suffix)

  if (toDemote.length) {
    await s.groupParticipantsUpdate(gid, toDemote, "demote").catch((e) => {
