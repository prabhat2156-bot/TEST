...
async def renewal_reminder_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = db.get_orders_needing_reminder(cutoff)
    for row in rows:
        try:
...
async def reseller_wd_start_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """User (reseller) taps 'Request Withdrawal' on their reseller panel."""
    q    = update.callback_query
    await q.answer()
    uid  = q.from_user.id
    lang = db.get_user_language(uid) or "en"
    if not db.is_reseller(uid):
        await q.answer(T(lang,"reseller_not_approved"), show_alert=True)
        return ConversationHandler.END
    summary = db.get_reseller_balance_summary(uid)
    if summary["available"] <= 0:
        await q.answer(T(lang,"wd_nothing_available_alert"), show_alert=True)
        return ConversationHandler.END
    ctx.user_data["wd_available"] = summary["available"]
    await q.edit_message_text(
        T(lang,"wd_amount_prompt", available=fmt(summary["available"])),
        parse_mode="HTML")
    return FEAT_WD_AMT

async def feat_wd_amt(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(T(lang,"wd_amount_invalid")); return FEAT_WD_AMT
    available = ctx.user_data.get("wd_available", 0)
    if amt > available:
        await update.message.reply_text(T(lang,"wd_amount_too_high", available=fmt(available))); return FEAT_WD_AMT
    db.create_withdraw_request(uid, amt)
    ctx.user_data.pop("wd_available", None)
    await update.message.reply_html(
        T(lang,"wd_request_submitted", amount=fmt(amt)),
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(T(lang,"btn_home"), callback_data="home")]]))
    user = db.get_user(uid)
    name = (user.get("username") if user else None) or str(uid)
    for admin_id in set(list(config.ADMIN_IDS) + db.get_extra_admins()):
        try:
            await ctx.bot.send_message(admin_id,
                f"💸 <b>New Withdrawal Request</b>\n{sep()}\n\n"
                f"👤 Reseller: {name} (<code>{uid}</code>)\n"
                f"💰 Amount: {fmt(amt)}",
                parse_mode="HTML")
        except Exception:
            pass
