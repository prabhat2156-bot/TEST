...
def skip_cancel_kb() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        [["⏭ Skip (no group)", BTN_CANCEL]], resize_keyboard=True, one_time_keyboard=True
    )
...
def clean_name(name: str) -> str:
    """Remove non-alphanumeric chars for filename use."""
...
async def vm_format(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip()
    if text == BTN_BACK:
        return await vm_entry(update, context)
    ...
    context.user_data["vm"]["format"] = fmt_label_map[text]
    await update.message.reply_text(
...
async def vm_basename(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip()
    if text == BTN_BACK:
        await update.message.reply_text("Step 2/7 — Choose output format:", reply_markup=format_kb())
        return VM_FORMAT

    context.user_data["vm"]["basename"] = clean_name(text) or "contacts"
    await update.message.reply_text(
        "Step 4/7 — Enter contact base name (e.g. `Customer`):",
        reply_markup=back_cancel_kb(),
        parse_mode="Markdown",
    )
...
async def vm_start_num(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text == BTN_BACK:
        await update.message.reply_text(
            "Step 4/7 — Enter contact base name:",
            reply_markup=back_cancel_kb(),
        )
        return VM_CONTACT_NAME

    try:
        context.user_data["vm"]["start_num"] = int(text)
    except ValueError:
        await update.message.reply_text("Please enter a valid number.", reply_markup=back_cancel_kb())
        return VM_START_NUM

    await update.message.reply_text(
        "Step 6/7 — Contacts per file (e.g. `500`):",
        reply_markup=back_cancel_kb(),
        parse_mode="Markdown",
    )
    return VM_PER_FILE
...
async def vm_per_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text == BTN_BACK:
        await update.message.reply_text(
            "Step 5/7 — Start number:",
            reply_markup=back_cancel_kb(),
        )
        return VM_START_NUM

    try:
        context.user_data["vm"]["per_file"] = int(text)
    except ValueError:
        await update.message.reply_text("Please enter a valid number.", reply_markup=back_cancel_kb())
        return VM_PER_FILE
...
async def vm_group(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip()
    if text == BTN_BACK:
        await update.message.reply_text(
            "Step 6/7 — Contacts per file:",
            reply_markup=back_cancel_kb(),
        )
        return VM_PER_FILE

    if text == "⏭ Skip (no group)":
        context.user_data["vm"]["group"] = ""
    else:
        context.user_data["vm"]["group"] = text
...
async def vm_show_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    confirm_text = (
        ...
        f"📋 Format: *{fmt.upper()}*\n"
        f"📁 Files: `{basename}1.{fmt}` ... `{basename}{total_files}.{fmt}`\n"
...
async def vm_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    vm = context.user_data["vm"]
    status = await update.message.reply_text("⏳ Generating files...")
...
async def sf_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Keep original stem, clean it for filenames
...
async def sf_count(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for chunk_start in range(0, total, count):
        ...
        # Simple filenames: stem + number, no underscores/hyphens
...
async def fe_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Simple filename: stem + "edited" (no underscore/hyphen)
...
async def rc_all_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Simple filename: stem + "renamed" (no underscore/hyphen)
...
async def help_entry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = (
        ...
        "*✂️ Split File*\n"
        "Upload file → enter count per file → get split files (original name + number).\n\n"
        "*🔗 Merge Files*\n"
        ...
        "📁 *Filename Convention:*\n"
        "All filenames are simple alphanumeric (no underscores/hyphens).\n"
        "e.g. madara1.vcf, madara2.vcf, madaraedited.vcf, madararenamed.vcf"
    )
...
def build_vm_conv() -> ConversationHandler:
    return ConversationHandler(
        ...
        states={
            ...
            VM_FORMAT:       [MessageHandler(filters.TEXT & ~filters.COMMAND, vm_format)],
            ...
            VM_GROUP:        [MessageHandler(filters.TEXT & ~filters.COMMAND, vm_group)],
