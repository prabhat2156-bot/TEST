...
import otp_module   # merged OTP-bot feature (TG Account Buy + TG Panel)
import wa_module    # WhatsApp account store (WP Account Buy + WA Panel)
...
(
    ...
    ADM_PRD_DESC,   ADM_PRD_PRICE,  ADM_PRD_DUR,
...
def parse_credential(data: str, lang: str) -> str:
    """Format credential for display with spoiler protection."""
    if ":" in data:
        parts = data.split(":", 1)
        return f"📧 <b>Email:</b> <code>{parts[0]}</code>\n🔑 <b>Password:</b> <code>{parts[1]}</code>"
...
def main_menu_rows(uid: int, lang: str) -> list:
    ...
    rows = [
        ...
        [sb("TG ACCOUNT BUY",      "otp_buy", style="success")],
        [sb("WP ACCOUNT BUY",      "wa_buy", style="success")],
...
async def history_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for o in orders:
        ...
        lines.append(f"{icon} #{o['id']:04d} — <b>{o['product_name']}</b> — {fmt(o['amount_usdt'])} — {(o['created_at'] or '')[:10]}")
    btns  = [[InlineKeyboardButton(f"#{o['id']:04d} {o['product_name'][:20]}", callback_data=f"order_{o['id']}")]
              for o in orders]
    nav = []
    if page > 0:
...
async def order_detail_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    item_row = _mcol("stock_items").find_one({"order_id": int(oid)})
    cred_text = (parse_credential(item_row.get("data"), lang)
                 if item_row and item_row.get("data")
                 else T(lang,"order_no_credential"))
    status  = T(lang,"order_status_completed") if not order.get("refunded") else T(lang,"order_status_refunded")

    # Build buttons: back + optional refund request button
    btns = []
    if not order.get("refunded"):
        existing_req = db.get_user_refund_request_for_order(uid, oid)
        if not existing_req:
            btns.append([InlineKeyboardButton(T(lang,"btn_request_refund"), callback_data=f"refund_req_{oid}")])
        elif existing_req["status"] == "pending":
            btns.append([InlineKeyboardButton(T(lang,"btn_refund_pending"), callback_data="noop")])
    btns.append([InlineKeyboardButton(T(lang,"btn_back"), callback_data="history")])

    await q.edit_message_text(
        T(lang,"order_detail_body",
          oid=oid,
...
async def user_refund_reason_received(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    stock_row = _mcol("stock_items").find_one({"order_id": int(oid)}, {"data": 1})
    cred = (stock_row or {}).get("data") or "—"
    ...
    admin_text = (
        ...
        f"📦 Order #{oid:04d} — {order['product_name']}\n"
    ...
    admin_kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Approve Refund", callback_data=f"adm_rreq_ok_{rid}"),
            InlineKeyboardButton("❌ Reject",          callback_data=f"adm_rreq_no_{rid}"),
        ]
    ])
    app = ctx.application
    for adm_id in list(config.ADMIN_IDS) + db.get_extra_admins():
        try:
            await app.bot.send_message(adm_id, admin_text, parse_mode="HTML", reply_markup=admin_kb)
        except Exception:
            pass
    ...
    try:
        ...
        await ctx.application.bot.send_message(
            ...
            T(_ulang,"refund_approved",
              ...
              amount=fmt(order['amount_usdt'])),
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(T(_ulang,"btn_my_profile"), callback_data="profile")]])
        )
    except Exception:
        pass
...
async def adm_rreq_no_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await ctx.application.bot.send_message(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(T(_ulang,"btn_support_ticket"), callback_data="support")]])
        )
    except Exception:
        pass
    # Update admin message
...
async def shop_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not cats:
        await se(q, T(lang,"shop_empty"), [
            [sb(T(lang,"btn_home"), "home", style="danger", emoji_id=EC.E_HOME)]
        ]); return
    btns = [[sb(f"{c['emoji']} {c['name']}", f"cat_{c['id']}", style="primary")] for c in cats]
    btns.append([sb(T(lang,"btn_home"), "home", style="danger", emoji_id=EC.E_HOME)])
    await se(q, T(lang,"shop_title"), btns)
...
async def render_product_view(q, uid: int, lang: str, pid: int, qty: int = 1):
    ...
    text  = T(lang,"product_detail",
              ...
              desc=p.get("description") or "Premium digital subscription.",
    ...
    rows.append([sb(T(lang,"btn_back"), f"cat_{p['category_id']}", style="danger", emoji_id=EC.E_BACK)])
...
async def do_purchase(q, uid: int, lang: str, pid: int, qty: int = 1):
    ...
    # Debit and reserve conditionally. The old read-then-write sequence could
    # oversell stock or let two concurrent taps spend the same balance.
    if not db.debit_balance(uid, total, "catalog_purchase", "product", pid):
        await q.answer(T(lang,"insufficient",needed=round(total-u["balance"],2)), show_alert=True); return
    items = db.reserve_stock(pid, uid, qty)
    if len(items) < qty:
        for item in items:
            db.release_stock(item["id"])
        db.update_balance(uid, total, "stock_unavailable_refund", "product", pid)
    ...
    try:
        for item in items:
            ...
            last_oid = oid
            created_stock_ids.append(item["id"])
    except Exception:
        # Restore any completed order records and return the remaining debit.
        for oid in created_oids:
            db.refund_order(oid)
        for item in items:
            if item["id"] not in created_stock_ids:
                db.release_stock(item["id"])
        remaining = round(total - (unit * len(created_oids)), 6)
        if remaining > 0:
            db.update_balance(uid, remaining, "purchase_error_refund", "product", pid)
        raise
    ...
    # Public sale broadcast ("Someone just bought Nx X from bot")
    try:
        import sale_feed
        await sale_feed.broadcast_sale(
            q.message.bot, qty=qty, product_name=p["name"],
            product_emoji=p.get("emoji",""), source="bot")
    except Exception as _e:
        logger.debug(f"sale_feed sale skip: {_e}")
...
async def cart_view_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not cart:
        ...
        await q.edit_message_text(T(lang,"cart_empty"),
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(T(lang,"btn_home"), callback_data="home")]]),
    ...
    for item in cart:
        ...
        avail = "✅" if stock >= item["quantity"] else "❌"
        lines.append(f"{avail} {item['emoji']} {item['name']} × {item['quantity']} = {fmt(sub)}")
        btns.append([
            InlineKeyboardButton(f"➖ {item['name'][:20]}", callback_data=f"cartqty_{item['product_id']}_dec"),
            InlineKeyboardButton(f"➕", callback_data=f"cartqty_{item['product_id']}_inc"),
            InlineKeyboardButton(f"🗑️", callback_data=f"cartrem_{item['product_id']}"),
        ])
    ...
    if coupon:
        ...
        lines.append(f"\n{T(lang,'cart_total_line',total=fmt(total))}")
        lines.append(f"🎟️ Coupon <b>{coupon['code']}</b>: −{coupon['discount']}% (−{fmt(discount_amt)})")
        lines.append(f"💵 <b>Total after discount: {fmt(final_total)}</b>")
    else:
        lines.append(f"\n{T(lang,'cart_total_line',total=fmt(total))}")
    lines.append(T(lang,'cart_balance_line',balance=fmt(u['balance'])))
    if coupon:
        btns.append([InlineKeyboardButton("🗑️ Remove Coupon", callback_data="cart_coupon_rm")])
    else:
        btns.append([InlineKeyboardButton("🎟️ Apply Coupon Code", callback_data="cart_coupon_start")])
    if u["balance"] >= final_total:
        btns.append([InlineKeyboardButton(T(lang,"btn_checkout"), callback_data="cart_checkout")])
    else:
        btns.append([InlineKeyboardButton(T(lang,"btn_top_up",needed=round(final_total-u["balance"],2)), callback_data="deposit")])
    btns.append([InlineKeyboardButton(T(lang,"btn_clear_cart"), callback_data="cart_clear"),
                 InlineKeyboardButton(T(lang,"btn_home"),       callback_data="home")])
    await q.edit_message_text("\n".join(lines), reply_markup=InlineKeyboardMarkup(btns), parse_mode="HTML")
...
async def cart_coupon_code_received(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if status == "not_found":
        await update.message.reply_text("❌ Invalid or inactive coupon code. Try again, or /cancel.")
        return CART_COUPON_CODE
    if status == "exhausted":
        await update.message.reply_text("❌ This coupon has reached its usage limit. Try again, or /cancel.")
        return CART_COUPON_CODE
    if status == "already_used":
        await update.message.reply_text("❌ You've already used this coupon. Try again, or /cancel.")
        return CART_COUPON_CODE
    ctx.user_data["cart_coupon"] = {"id": c["id"], "code": c["code"], "discount": c["discount"]}
    await update.message.reply_html(
        f"✅ Coupon <b>{c['code']}</b> applied: −{c['discount']}% off!",
...
async def cart_checkout_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Reserve every line before charging anything. This prevents a cart with
    # several products from charging the first items and then failing halfway
    # through because a later item sold out.
    reservations = []
    for item in cart:
        unit  = round(_cart_item_pricing(uid, u, item["price_usdt"]) * disc_mult, 6)
        qty   = item["quantity"]
        items = db.reserve_stock(item["product_id"], uid, qty)
        if len(items) < qty:
            for reserved_item, _unit in reservations:
                db.release_stock(reserved_item["id"])
            for reserved_item in items:
                db.release_stock(reserved_item["id"])
            await q.answer(T(lang,"cart_item_oos",name=item['name']), show_alert=True); return

        reservations.extend((si, unit) for si in items)

    coupon_claimed = False
    if coupon:
        # Claim the coupon before charging; if any later step fails it is
        # explicitly rolled back below.
        coupon_claimed = db.apply_coupon(coupon["id"], uid)
        if not coupon_claimed:
            for reserved_item, _unit in reservations:
                db.release_stock(reserved_item["id"])
            ctx.user_data.pop("cart_coupon", None)
            await q.answer("This coupon is no longer available.", show_alert=True)
            return

    if not db.debit_balance(uid, total, "cart_purchase", "cart", uid):
        for reserved_item, _unit in reservations:
            db.release_stock(reserved_item["id"])
        if coupon_claimed:
            db.rollback_coupon(coupon["id"], uid)
        await q.answer(T(lang,"insufficient",needed=round(total-u["balance"],2)), show_alert=True)
        return
    ...
    created_total = 0.0
    try:
        for si, unit in reservations:
            item = next(i for i in cart if i["product_id"] == si["product_id"])
            oid = db.create_order(uid, item["product_id"], item["name"], unit, si["id"])
            created_oids.append(oid)
            db.mark_stock_sold(si["id"], uid, oid)
            results.append((item, si, oid))
            created_stock_ids.add(si["id"])
            created_total = round(created_total + unit, 6)
    except Exception:
        for oid in created_oids:
            db.refund_order(oid)
        for si, _unit in reservations:
            if si["id"] not in created_stock_ids:
                db.release_stock(si["id"])
        remaining = round(total - created_total, 6)
        if remaining > 0:
            db.update_balance(uid, remaining, "cart_error_refund", "cart", uid)
        if coupon_claimed:
            db.rollback_coupon(coupon["id"], uid)
        raise
    db.clear_cart(uid)
...
async def dep_submit_hash_start_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not dep or dep["user_id"] != q.from_user.id:
        await q.answer("Not found.", show_alert=True); return ConversationHandler.END
    if dep["status"] != "pending":
        await q.answer("This deposit is no longer pending.", show_alert=True); return ConversationHandler.END
...
async def adm_dep_no_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    _mcol("deposit_requests").update_one({"id": int(dep_id)},
...
async def dep_status_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    dep    = db.get_deposit(dep_id)
    if not dep: await q.answer("Not found.", show_alert=True); return
...
async def ticket_view_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    t    = db.get_ticket(tid)
    if not t or t["user_id"] != uid: await q.answer("Not found.", show_alert=True); return
...
async def _credit_deposit_manual(context, dep: dict, txid: str, credited: float, network_label: str):
    ...
    if txid and txid != "ADMIN":
        ...
        if existing and existing["id"] != dep["id"]:
            ...
            return
    # Only the caller that changes this request from pending → completed may
    # credit the wallet. A retry/race must not credit the same deposit twice.
    completed = db.complete_deposit(dep["id"], txid)
    if not completed:
        logger.info("Deposit #%s was already completed or no longer pending", dep["id"])
        return
    ...
    db.update_balance(
        dep["user_id"], credited + vip_bonus, "deposit_credit",
        "deposit", dep["id"])
...
def _admin_panel_content():
    ...
    btns = [
        ...
        [sb("TG Panel",        "tg_panel",       style="success"),
         sb("WA Panel",        "wa_panel",       style="success")],
...
async def adm_prd_dur(u, c):
    if not is_admin(u.effective_user.id): return ConversationHandler.END
    d = c.user_data
    db.add_product(d["prd_cat"], d["prd_name"], d["prd_emoji"], d.get("prd_desc",""), d["prd_price"], u.message.text.strip())
...
async def adm_stock_data(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if result["added"] > 0:
        ...
        # Public "BACK IN STOCK" broadcast
        try:
            import sale_feed
            await sale_feed.broadcast_restock(
                update.message.bot, product_name=p["name"],
                stock=total_stock,
                price=f"{p.get('price_usdt','')} USDT" if p.get("price_usdt") else "",
                product_emoji=p.get("emoji",""), source="bot")
        except Exception as _e:
...
def build_app() -> Application:
    ...
    # ── OTP-bot merged handlers (Buy Account + TG Panel) ─────────────────────
    otp_module.register_handlers(app)

    # ── WhatsApp store handlers (WP Account Buy + WA Panel) ─────────────────
    wa_module.register_handlers(app)
    ...
    async def _set_commands(a):
        await a.bot.set_my_commands([
            ...
            BotCommand("wallet", "👛 Balance & history"),
...
if __name__ == "__main__":
    # Render web services must listen on the platform-provided PORT. The
    # health server runs in a daemon thread while Telegram polling continues
    # in the main thread.
    from render_server import start_health_server
    start_health_server()
    db.init_db()
    otp_module.init_schema()
    wa_module.init_schema()
    print(f"🤖 {bot_name()} v5.1 (+OTP merged) starting…")
