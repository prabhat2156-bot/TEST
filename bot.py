...
async def init_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        """)
        # Settings table — generic config store
        await db.execute("""
...
async def cmd_challenge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for entity in msg.entities:
        if entity.type == "mention":
            # mention text se username nikalo
            mention_text = msg.text[entity.offset : entity.offset + entity.length].lstrip("@")
            if mention_text.lower() == opponent_mention.lower():
                # Telegram mention se user ID directly milta hai text_mention type mein
                # text mention type ke liye entity.user hota hai
                pass
...
async def callback_accept(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if role == "creator":
        if user.id != c["creator_id"]:
            ...
            return
        await update_challenge(ch_id, creator_accepted=1)
        accepted_field = "creator_accepted"
    else:
        # Opponent ID 0 tha agar text mention nahi tha, toh username se match karo
        if c["opponent_id"] != 0 and user.id != c["opponent_id"]:
            await query.answer(
                f"❌ Yeh button sirf @{c['opponent_username']} ke liye hai!",
                show_alert=True,
            )
            return
        elif c["opponent_id"] == 0:
            # First time opponent accept karta hai toh ID save karo
            await update_challenge(ch_id, opponent_id=user.id, opponent_username=user.username or user.first_name)
            await upsert_user(user.id, user.username or user.first_name)

    # Track accepted flags via a simple approach in settings key
    async with aiosqlite.connect(DB_PATH) as db:
        # accepted_flags column nahi hai schema mein, use settings table
...
async def cmd_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/cancel CHID — Challenge cancel karo (owner ya creator pending mein)."""
    msg = update.message
    user = update.effective_user
    ...
    # Creator bhi cancel kar sakta hai agar pending hai
    if user.id != OWNER_ID and user.id != c["creator_id"]:
        await msg.reply_text("❌ Sirf owner ya creator cancel kar sakte hain.")
        return

    if user.id == c["creator_id"] and c["status"] != "pending":
        await msg.reply_text("❌ Creator sirf pending challenge cancel kar sakta hai.")
        return
...
async def fetch_ipl_matches() -> list[dict]:
    ...
    try:
        ...
        for match in data.get("data", []):
            ...
            series = match.get("series", "") or match.get("series_id", "")
            name = match.get("name", "")
...
def main():
    ...
    logger.info("🤖 IPL Challenge Bot V2 starting...")
