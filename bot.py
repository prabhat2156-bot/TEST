...
async def history_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    btns  = [[sb(f"#{o['id']:04d} {o['product_name'][:20]}", f"order_{o['id']}", style="primary")]
              for o in orders]
    nav = []
    if page > 0:
...
async def orders_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for o in orders:
        icon = "✅" if not o.get("refunded") else "♻️"
        lines.append(f"{icon} #{o['id']:04d} — <b>{o['product_name']}</b> — {fmt(o['amount_usdt'])} — {(o['created_at'] or '')[:10]}")
    btns = [[InlineKeyboardButton(f"#{o['id']:04d} {o['product_name'][:20]}", callback_data=f"order_{o['id']}")] for o in orders]
...
def build_app() -> Application:
    ...
    for h in [dep_conv, dep_hash_conv, ticket_conv, ticket_reply_conv, adm_reply_conv,
               ...
               refund_chat_conv, reseller_wd_conv]:
        app.add_handler(h)
