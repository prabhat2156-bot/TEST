...
def _attach_username_and_cred(order_docs):
    """Python-side equivalent of the old LEFT JOIN users / stock_items."""
    out = []
    for o in order_docs:
        o = dict(o)
        username, _ = _user_username_fullname(o["user_id"])
        o["username"] = username
        s = stock_items.find_one({"order_id": o["id"]}, {"data": 1})
        o["cred_data"] = s.get("data") if s else None
        out.append(o)
    return out
...
def get_today_deposits():
    ...
    for d in cursor:
        d = dict(d)
        d["username"], _ = _user_username_fullname(d["user_id"])
...
def _attach_username_fullname(cursor):
    ...
    for t in cursor:
        t = dict(t)
        t["username"], t["full_name"] = _user_username_fullname(t["user_id"])
        out.append(_strip_mongo_id(t))
    return out
...
def get_resellers():
    out = []
    for r in resellers.find({"approved": 1}):
        r = dict(r)
        r["username"], r["full_name"] = _user_username_fullname(r["user_id"])
...
def _attach_user_order_info(rr):
    rr = dict(rr)
    rr["username"], rr["full_name"] = _user_username_fullname(rr["user_id"])
    o = orders.find_one({"id": rr["order_id"]}, {"product_name": 1, "amount_usdt": 1})
    rr["product_name"] = o.get("product_name") if o else None
    rr["amount_usdt"]  = o.get("amount_usdt") if o else None
    return _strip_mongo_id(rr)


def get_pending_refund_requests():
    return [_attach_user_order_info(rr) for rr in refund_requests.find({"status": "pending"}).sort("id", 1)]
...
def get_all_refund_requests(limit=1000):
    ...
    return [_attach_user_order_info(rr) for rr in refund_requests.find().sort("id", -1).limit(limit)]
