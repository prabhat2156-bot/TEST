...
# Settings are read on almost every button press. A tiny TTL cache removes
# a round-trip to Mongo Atlas per read, which is the single biggest source of
# UI lag on Render's free plan. Writes invalidate the cache immediately.
_SETTINGS_CACHE = {"at": 0.0, "data": {}}
_SETTINGS_TTL = 20.0


def _settings_map(force=False):
    now = _time.time()
    if force or now - _SETTINGS_CACHE["at"] > _SETTINGS_TTL:
        try:
            _SETTINGS_CACHE["data"] = {
                d["key"]: d.get("value")
                for d in col("settings").find({}, {"_id": 0, "key": 1, "value": 1})
            }
...
def set_setting(key, value):
    ...
    _SETTINGS_CACHE["data"][key] = str(value)
