...
def init_db():
    ...
    orders.create_index("order_code", unique=True, sparse=True)
...
def _gen_order_code() -> str:
    """Random 5-6 digit order code shown to users (harder to guess/enumerate
    than a sequential order number)."""
...
def create_order(user_id, product_id, product_name, amount_usdt, stock_item_id=None,
    ...
    while orders.find_one({"order_code": order_code}):
        order_code = _gen_order_code()
    orders.insert_one({
        "id": oid, "order_code": order_code, "user_id": user_id, "product_id": product_id,
        "product_name": product_name, "amount_usdt": amount_usdt, "status": "completed",
        "stock_item_id": stock_item_id, "coupon_code": coupon_code,
...
def get_order_by_code(order_code):
    """Look up an order by its user-facing 5-6 digit order code (not the
    internal sequential id). Used by history taps and the 'Find Order' feature."""
    return _strip_mongo_id(orders.find_one({"order_code": str(order_code).strip()}))
...
def _attach_user_order_info_batch(cursor):
    ...
    order_map = {
        o["id"]: o for o in orders.find({"id": {"$in": oid_list}}, {"id": 1, "order_code": 1, "product_name": 1, "amount_usdt": 1})
    }
    out = []
    for rr in docs:
        rr = dict(rr)
        ...
        rr["order_code"]   = (o.get("order_code") if o else None) or rr["order_id"]
