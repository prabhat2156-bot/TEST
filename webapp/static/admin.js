...
loaders.dashboard = async function(){
  try{
    ...
    $('#dashRecentOrders').innerHTML = d.recent_orders.length ? d.recent_orders.map(o => `
      <tr><td>#${o.id}</td><td>@${esc(o.username||o.user_id)}</td><td>${esc(o.product_name)}</td>
...
async function loadOrders(){
  $('#ordersBody').innerHTML = `<tr><td colspan="6" class="loading">Loading…</td></tr>`;
  try{
    const d = await apiGet('/orders?scope=' + ordersScope);
    $('#ordersBody').innerHTML = d.orders.length ? d.orders.map(o => `
      <tr><td>#${o.id}</td><td>@${esc(o.username||o.user_id)}</td><td>${esc(o.product_name)}</td>
      <td>${Number(o.amount).toFixed(2)}</td><td>${badge(o.status)}</td><td>${fmtDate(o.created_at)}</td></tr>`).join('')
      : `<tr><td colspan="6" class="empty">No orders found.</td></tr>`;
  }catch(e){ $('#ordersBody').innerHTML = `<tr><td colspan="6" class="empty">Failed to load.</td></tr>`; }
}
...
async function loadRefunds(){
  ...
  try{
    ...
    $('#refundsBody').innerHTML = d.refunds.length ? d.refunds.map(r => `
      <tr><td>#${r.id}</td><td>@${esc(r.username)}</td><td>#${r.order_id} — ${esc(r.product_name||'')}</td>
  ...
  try{
    ...
    $('#refundModalBody').innerHTML = `
      <div class="detail-grid">
        ...
        <div class="k">Order ID</div><div class="v">#${r.order.id}</div>
