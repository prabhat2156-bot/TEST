...
async def single_phone(update, ctx):
    ...
    client = om.make_login_client(phone)
    try:
        sreq = await om.send_login_code(client, phone)
    except Exception as e:
        try: await client.disconnect()
        except: pass
        om._delete_session_files_by_phone(phone)
        await _reply(update, f"❌ <b>OTP send failed:</b>\n<code>{om.explain_login_error(e)}</code>")
        return ConversationHandler.END
...
async def single_otp(update, ctx):
    ...
    try:
        ...
        try:
            ...
            await _reply(update, f"❌ <b>Resend failed:</b>\n<code>{om.explain_login_error(e)}</code>")
            return await _bulk_fail_or_end(update, ctx)
    except Exception as e:
        try: await client.disconnect()
        except: pass
        om._delete_session_files_by_phone(st["phone"])
        await _reply(update, f"❌ <b>Sign-in failed:</b>\n<code>{om.explain_login_error(e)}</code>")
        return await _bulk_fail_or_end(update, ctx)
    st["twofa"] = "None"
...
async def single_2fa(update, ctx):
    ...
    try:
        ...
        om._delete_session_files_by_phone(st["phone"])
        await _reply(update, f"❌ <b>2FA failed:</b>\n<code>{om.explain_login_error(e)}</code>")
        return await _bulk_fail_or_end(update, ctx)
...
async def _single_after_login(update, ctx):
    ...
    phone = st["phone"]
    try:
        om.persist_login_session(st["client"], phone)
    except Exception as e:
        logger.warning("persist_login_session(%s) failed: %s", phone, e)
...
async def _bulk_next(update, ctx):
    ...
    try:
        ...
        om._delete_session_files_by_phone(phone)
        await _reply(update, f"❌ <b>OTP send failed for +{phone}:</b>\n<code>{om.explain_login_error(e)}</code>")
        stats["failed"] += 1
        return await _bulk_next(update, ctx)
    ctx.user_data["tgp_single"] = {"phone": phone, "client": client,
                                    "phone_code_hash": sreq.phone_code_hash}
